...
For a reusable chart type anyone in the project can apply, see [Chart Studio](/visualizations/chart-studio).
...
<Steps>
  ...
  <Step title="Switch to Vega">
    ...
    <Frame>
    ![The chart type menu with Custom selected and the Custom chart type picker showing Vega (JSON editor) under the Built in group](/images/visualizations/vega/custom-charts-drop-down.png)
    </Frame>
  </Step>
...
For true source-to-target flow diagrams, use the built-in [Sankey chart](/visualizations/sankey). The custom template below is an escape hatch for the stacked-bar variation that shows how segment proportions shift over time on a continuous x-axis — something the built-in Sankey doesn't do.
...
<Frame>
![A sankey-style 100% stacked bar chart: two full-height stacked bars with curved ribbons connecting the matching segments across them](/images/visualizations/vega/sankey-style-bar-chart.png)
</Frame>
...
<Frame>
![A line chart with a shaded band around the line, marking the high and low bounds of the confidence interval at each date](/images/visualizations/vega/custom-chart-confidence-interval.png)
</Frame>
...
<Frame>
![A heatmap grid with order weeks along the x-axis and days of the week down the y-axis, each cell shaded darker for a higher order count](/images/visualizations/vega/custom-charts-heatmap.png)
</Frame>
...
<Frame>
![A bubble plot where each point is a circle positioned by two numeric values and sized by a third metric](/images/visualizations/vega/custom-charts-bubble-plot.png)
</Frame>
...
For standard funnels, use the built-in [Funnel chart](/visualizations/funnel-chart) — it handles discrete stages with counts and stage-to-stage percentages out of the box. The custom template below is an escape hatch for styled variations the built-in chart doesn't support (e.g., custom trapezoid coloring, inline conversion-rate labels between stages).
...
<Frame>
![Custom funnel chart](/images/visualizations/vega/custom-charts-funnel-chart.png)
</Frame>
...
<Frame>
![Custom Waterfall](/images/visualizations/vega/custom-charts-waterfall.png)
</Frame>
...
<Frame>
![Custom trellis area chart](/images/visualizations/vega/trellis-area.png)
</Frame>
...
<Frame>
![A bar chart where one bar is filled in an accent colour and the rest are muted, singling that category out](/images/visualizations/vega/custom-charts-highlighted-bar.png)
</Frame>
