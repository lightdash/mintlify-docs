...
Group labels support a `${table_field.granularity}` placeholder that resolves to the active date granularity (for example `day`, `week`, `month`) — including the granularity selected via [date zoom](/explore/date-zoom). Click the **insert date granularity** icon inside the editable group label field to insert the placeholder without typing it manually. See [Referencing the granularity in chart labels](/explore/date-zoom#referencing-the-granularity-in-chart-labels) for the full syntax and supported labels.
