...
The label and comparison label accept the `${table_field.granularity}` placeholder to show the active date granularity — type `$` in the label field to pick from the available date dimensions. See [Referencing the granularity in chart labels](/explore/date-zoom#referencing-the-granularity-in-chart-labels) for the full syntax and supported labels.
