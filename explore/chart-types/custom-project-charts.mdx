...
Save the chart like any other chart. The saved chart remembers which chart type it uses, how its inputs are mapped, and which version of the chart type it was saved with (see [Versions and upgrades](#versions-and-upgrades)).
...
Every follow-up prompt in the builder creates a new version of the chart type. A saved chart pins the version it was saved with, so iterating on a chart type never changes the charts that already use it. Charts saved before pinning existed follow the latest version until they are next edited and saved.

When you edit a chart whose chart type has moved on, the **Configure chart** sidebar shows **Newer version available**. Click **Review upgrade** to see what changed between the chart's version and the latest one: inputs and display options that were added, renamed or removed, and any change to the color palette. Removed inputs lose their mapping and removed options lose their values.

<Frame>
  <img
    alt="The Upgrade chart type dialog over the Explorer, listing what changed since the chart's pinned version"
    className="block dark:hidden"
...
Click **Upgrade** to move the chart to the latest version. Only that chart moves, and nothing is stored until you save it. If the new version added a required input, the chart tells you which inputs to map before it renders again.

The builder has its own upgrade, for the chart type rather than for a chart. When a chart type was built on an older version of the data apps runtime, its header shows **Upgrade available**. Starting that upgrade rebuilds the chart type on the current runtime as a new version, and **History** then shows a summary of what is now active and what you can ask the builder to add. Nothing that changes how the chart type renders is added automatically.
