---
title: "Period-over-period comparisons"
sidebarTitle: Period-over-period
description: "Compare a metric against its previous period, and the ways Lightdash can calculate the shift"
---
...
<Tip>
  You can also ask an [AI agent](/agents/use-ai-agents#period-over-period-comparisons) for comparisons in natural language — for example, *"compare revenue to last month"* or *"year-over-year orders by week"*.
</Tip>
...
>
  <Frame>
  <iframe
    src="https://www.loom.com/embed/e6d1a0442f6442ac8a5f0343d35b73d7"
    frameborder="0"
...
>
  <Frame>
  <iframe
    src="https://www.loom.com/embed/be15f8023f754123a177aa1f928477e6?t=116"
    frameborder="0"
    webkitallowfullscreen
    mozallowfullscreen
    allowfullscreen
...
_This table calculation relies on the `MONTH_NUM` [time interval](/semantic-layer/dimensions#numeric-options)._
...
- Add **week of year** [time dimension](/semantic-layer/dimensions#numeric-options) (1-52) on the X-axis.  
...
<Frame>
  <img src="/images/explore/period-over-period/period-over-period-week-of-year.png" alt="A chart comparing years, with one line per year plotted against week of year and the x-axis labelled with week descriptions rather than numbers"/>
</Frame>
...
- Use **day of month** [time dimension](/semantic-layer/dimensions#numeric-options) (1-31) on the X-axis, grouped by month.
...
2. Add a [formula table calculation](/explore/table-calculations/formulas) that uses the [`LAG` window function](/explore/table-calculations/formulas#window) to pull the same month last year, offset by 12 rows: `=LAG(event_count, 12)`.
3. Add a second formula for the percent change, following the [period-over-period growth pattern](/explore/table-calculations/formulas#examples): `=(event_count - LAG(event_count, 12)) / LAG(event_count, 12) * 100`.
...
Users can easily toggle between different date ranges to see how metrics are tracking over time — no need to rebuild charts or manually adjust filters. For more on working with parameters, check out our [parameters reference guide](/semantic-layer/parameters). 
...
#### Date range period parameter syntax
  ...
  <Step title="Build your chart with the period dimension">
   ...
   <Frame>
     <img src="/images/explore/period-over-period/period-over-period-example.jpg" alt="A chart of order count grouped by the Order date period dimension, with the out of range group filtered out so only the current and previous periods show"/>
   </Frame>
...
<Frame>
  <img src="/images/explore/period-over-period/custom_range_pop.png" alt="The parameter picker in the Lightdash UI listing the custom date range options, with the chart beside it labelled with those period names"/>
</Frame>
