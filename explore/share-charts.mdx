...
Users with access to the containing space can find the chart under **Browse → All saved charts**, or you can send them its URL. Copy the URL from a full-size browser window: on a screen narrower than 768px a saved chart opens in a read-only minimal view, and a link copied there carries `/minimal/` in its path and opens that way for everyone. See [the minimal view](/explore/dashboards/interact#the-minimal-view). To share an individual saved chart without sharing its space, use [Direct access](/workspace-admin/direct-access). That guide also explains how sharing differs for dashboard-owned and reusable charts.
