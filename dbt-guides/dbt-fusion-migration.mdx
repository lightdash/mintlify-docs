...
### How Lightdash works with dbt Fusion

Lightdash compiles your dbt project using dbt Core v1.10+, regardless of whether you run dbt Fusion locally. Since the YAML structure is the same between dbt Core and dbt Fusion, your project remains compatible with Lightdash.

- **Lightdash CLI**: dbt Fusion is supported locally with the Lightdash CLI. You can run Fusion commands in your development environment while Lightdash commands continue to work alongside them.
- **Lightdash Cloud**: The dbt connector in Lightdash uses dbt Core v1.10+ to compile your project. This generates a compatible manifest, which works correctly since you can always run a Fusion project on dbt Core.

<Note>
  Developers using dbt Fusion may need to maintain separate environments—one for running Fusion commands and another for running Lightdash commands based on dbt Core.
</Note>  
