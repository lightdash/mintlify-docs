...
description: "Connect external AI assistants like Claude, ChatGPT, and OpenAI Codex to your Lightdash data over the Model Context Protocol"
doc-type: reference
---
...
<Info>
  **Per-user permission gate.** `create_content` and `edit_content` are also gated on the caller's role holding the `create:ContentAsCode` scope. The check runs at tool registration, so users without the scope don't see these tools in the tools list.
