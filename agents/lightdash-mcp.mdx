...
MCP exposes [AI writeback](/agents/ai-writeback) — editing the dbt project that backs the active Lightdash project and opening a pull request with the change — to any MCP client. Ask the assistant to rename a metric, add a dimension, edit a model's SQL, or fix a YAML description; the assistant calls the tools below and hands back a pull request URL when the run finishes. The [AI writeback prerequisites](/agents/ai-writeback#prerequisites) — feature flag, supported git host, app installation, project Developer permission — apply to the MCP surface too.
