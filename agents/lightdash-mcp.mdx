...
<a id="pin-a-project-with-the-x-lightdash-project-header" />
...
MCP can build [data apps](/data-apps) — interactive applications generated from a brief on top of the active project's semantic layer — from any MCP client. Ask the assistant to build an app, wait a few minutes while it builds, then iterate on it with follow-up briefs. Each build adds a **version** to the app.

- **Generate data app** (`generate_data_app`) - Start building a new app from a name and a brief. Optionally start from an existing dashboard or saved charts, pick a starter template, or apply a theme.
- **Iterate data app** (`iterate_data_app`) - Build a new version of an existing app from a follow-up brief. The app keeps its name, slug, and theme unless you ask for a different theme.
- **List data app themes** (`list_data_app_themes`) - List the organization's [themes](/data-apps/customize-themes) so the assistant can apply one when it builds.
- **Get data app build status** (`get_data_app_build_status`) - Check how a build is going, and get the app's URL once it's ready. The assistant checks this for you while the build runs.

Builds run in the background, so the assistant reports that the build has started and then follows up when it's ready. Once it is, the assistant can open the app for you, read it back with `read_content`, or keep iterating.

An app built over MCP is created as the calling user's **personal app**: at the project root, in no space, visible only to them until they move it into a space from the Lightdash UI. This mirrors what the in-product [AI agent](/agents) does.

<Info>
  **What enables these tools.** The data app tools only appear in the tools list when both of these are true:
...
- _"Build a data app called Revenue Overview that shows monthly revenue and the top 10 customers, then tell me when it's ready."_
- _"Add a region filter to the Revenue Overview app."_
...
Your organization's [custom agent skills](/agents/custom-agent-skills) are available through the same MCP connection. When an AI agent is active in the session, MCP exposes the custom skills attached to that agent. Without an active agent, it exposes the organization skills the caller has permission to view.
...
- **List skills** (`list_skills`) - Return the available built-in and custom skills along with their supporting resource files
...
<Accordion title="Skill resources via MCP (for developers)">
  The Lightdash MCP server exposes built-in and custom skills as MCP resources, so any MCP-compatible client can discover and load them at runtime — no CLI install required.
  ...
  **Available resources:**
  ...
  | `skill://custom/index.json` | `application/json` | Discovery index for the custom skills available to the caller. |
  | `skill://custom/<skill-name>/SKILL.md` | `text/markdown` | A custom skill's instructions and frontmatter. |
  | `skill://custom/<skill-name>/resources/<file>` | `text/markdown` | A custom skill's supporting resource file. |
