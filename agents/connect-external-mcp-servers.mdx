...
Because MCP servers belong to the project, renaming or deleting one affects every agent that uses it. From any agent's **MCP servers** section, click **Manage MCPs** to open the project-wide list. Each server shows its URL, auth type, connection status, and how many agents it is attached to.

- **Rename** — open the server's **⋯** menu and choose **Rename**. Only the name changes; credentials, connections, and agent attachments are untouched.
- **Delete** — open the **⋯** menu and choose **Delete**. After you confirm, the server is detached from every agent, and its shared credential and every user's personal credential are removed. This cannot be undone.

<Note>
  **Remove** on an agent's MCP server card only detaches the server from that agent. The server, its credentials, and its other attachments stay in the project.
</Note>
