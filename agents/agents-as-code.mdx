...
| `enableSelfImprovement` | boolean | Whether the agent captures corrections into [agent memory](/agents/agent-memory). |
...
| `threadRetentionHours` | number \| null (optional) | Per-agent [conversation retention](/agents/conversation-retention) window. Only applies when the feature is enabled for your organization — see [agents as code semantics](/agents/conversation-retention#agents-as-code). |
