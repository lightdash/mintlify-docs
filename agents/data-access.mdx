...
description: How agents reach your data, and how to scope them with tags and user attributes
doc-type: reference
---
...
<Warning>
  [Content editing](/agents/enable-content-tools) requires data access. If you turn data access off, content editing is automatically turned off and locked until you re-enable data access.
</Warning>

To change data access, go to your agent settings and toggle **Data Access**. Turn it off when you want the agent to work only with your data model structure and not see actual data values.

  <Frame>
    <img
      src="/images/agents/enable-data-access.png"
...
<Frame>
  <img
    src="/images/agents/pass-user-information.png"
    alt="Pass user information toggle in AI agent Data Access settings"
  />
</Frame>

<Note>
  This only applies when the agent knows who is asking. In the Lightdash app that's always the logged-in user. On **Slack**, you need to enable the [AI Agents OAuth requirement](/integrations/slack#ai-agents-configuration) so each Slack user is identified individually — otherwise user information is left out and answers stay generic.
</Note>
...
<Frame caption="Preview available explores and fields as you add tags">
  <img
    src="/images/agents/agent-tags-preview.gif"
...
Tagging the base model does not restrict joined models. Once an explore is visible, each joined table is filtered by its own tags only. A joined table with no tags falls into explore-level mode, so all of its fields are visible to the agent. To restrict a joined model's fields, add field-level tags to that model (see Scenarios 4 and 6 below).
...
```yaml customers.yml (joined model)
models:
  - name: customers
    columns:
      - name: customer_id
...
**Scenario 6: Tagged base model with an untagged joined model**
...
Result: If agent has `["ai-enabled"]` tag:
- `orders` table: ALL fields visible (explore-level mode - explore tags match)
- `customers` table: ALL fields visible (explore-level mode - the joined model has no tags anywhere, so nothing restricts it)

<Warning>
  Tagging the base model does not restrict joined models. A tagged base model exposes every field of an untagged joined model to the agent. To restrict a joined model's fields, add field-level tags to that model, as in Scenario 4.
</Warning>

## User attributes and permissions

AI agents automatically respect all data access controls configured through [user attributes](/workspace-admin/user-attributes). This ensures that agents only access data that the user is authorized to see, maintaining your existing security policies.
...
- Metrics defined under restricted columns are also unavailable; model-level metrics are not restricted by the columns they reference (see [metrics and column access control](/workspace-admin/user-attributes#metrics-and-column-access-control))
