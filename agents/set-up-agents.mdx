...
sidebarTitle: Set up agents
description: Enable AI features, create your first agent, scope its data, and connect it to Slack
doc-type: tutorial
---
...
Set up an agent around a clear job, give it access to the right data and context, then test it with real questions before sharing it more widely.

<Steps>
  <Step title="Enable AI features (Admin)">
    An organization admin enables AI features and chooses the models users can access. [Enable AI features](#enable-ai-features).
  </Step>
  <Step title="Plan your agent">
    Define who the agent is for, the questions it should answer, and the data and workflows it needs. [Plan your agent](#plan-your-agent).
  </Step>
  <Step title="Create and configure the agent">
    Add its instructions, context, reusable skills, access controls, and optional tools. [Create your first agent](#create-your-first-ai-agent).
  </Step>
  <Step title="Test and improve its answers">
    Ask representative questions, review the results, and refine the agent before rolling it out. See [Effective analytics with agents](/agents/effective-analytics-with-agents).
  </Step>
  <Step title="Share it with your team (optional)">
    Control who can use the agent and connect it to team workflows such as [Slack](#slack-integration).
  </Step>
</Steps>

## Plan your agent

Start with the smallest useful scope: one audience, one area of the business, and a clear set of questions. A focused agent is easier to configure, test, and trust than one expected to answer everything.

Use each part of the agent configuration for a different job:

| What the agent needs | What to configure |
| --- | --- |
| Behavior that should apply to every answer | Add [instructions](#instructions) for its role, analysis preferences, and communication style. |
| Business context or reference material | Add [knowledge documents](#knowledge-documents-optional) that the agent can consult when relevant. |
| A repeatable, task-specific workflow | Create a [custom skill](/agents/custom-agent-skills) that the agent can load when the request matches. |
| A defined set of data and users | Configure [data access](/agents/data-access), tags, and [user or group access](#user-and-group-access-optional). |
| Permission to create content or work with other systems | Enable only the tools it needs, such as [content editing](/agents/enable-content-tools), [external MCP servers](/agents/connect-external-mcp-servers), or [AI writeback](/agents/ai-writeback). |

You do not need to configure every option at once. Start with the agent's instructions and access scope, test it with questions from its intended users, then add context, skills, or tools when you identify a specific gap. For more guidance on designing focused agents and improving their answers, see [Effective analytics with agents](/agents/effective-analytics-with-agents).
...
- Admins get [visibility into agent activity](/agents/visibility)
...
Instead of using the instance-wide Anthropic and OpenAI keys configured by your Lightdash administrator, org admins can store their organization's own provider API keys and control which models are exposed to users. When a key is set, Ask AI routes that provider's model calls through your org's account — including agent chats, thread-title generation, the Slack agent path, agent readiness checks, doc summarization, the [AI router](/agents/enable-ai-router), ambient AI, [data-app](/data-apps) clarification and sandbox generation, and other agent utility calls.
...
- An org-level Anthropic key cannot be combined with an instance-wide `ANTHROPIC_BASE_URL`. Remove the org key to use the [corporate gateway](/self-host/enterprise-features/ai-agents#corporate-llm-gateways), or remove the gateway URL to keep routing through the org's Anthropic account.
...
<Frame>
  <img
    src="/images/agents/ai-agent-overview-page.png"
    alt="An agent's Setup page, with its name, avatar URL, instructions, quick templates, and data access toggles"
  />
</Frame>
...
<Frame>
  <img
    src="/images/agents/go-to-ask-ai.png"
    alt="The Lightdash top navigation bar with Ask AI highlighted next to New, Browse, and Metrics"
  />
</Frame>
...
<Frame>
  <img
    src="/images/agents/create-new-agent.png"
    alt="The agent picker dropdown listing the project's agents, with Create new agent at the bottom"
  />
</Frame>
...
Upload reference material — glossaries, metric definitions, business context, internal SOPs — that your agent can consult while answering questions. Use this for the kind of context that doesn't fit neatly into a dbt `ai_hint` or your instructions. See [knowledge documents guidance](/agents/effective-analytics-with-agents#knowledge-documents) for guidance on what to upload.

### Skills (optional)

Turn prompts you use repeatedly into skills, especially when they contain a clear sequence, checks, or output requirements for a particular task. The agent can load an attached skill automatically when a request matches, or users can run it directly with `/`. Keep behavior that should apply to every response in the agent's instructions. Skills are shared through your organization's library, so the same workflow can be used by more than one agent. Learn how to [create and use custom agent skills](/agents/custom-agent-skills).

### Data Access
By default, agents have data access enabled so they can analyze actual query results and provide insights based on the data. You can turn it off per agent if you want metadata-only behavior. This section also holds the **Pass user information** toggle, which lets the agent [tailor answers to who is asking](/agents/data-access#personalizing-answers-to-who-is-asking). Learn more in [data access control](/agents/data-access).
...
This controls who can use the agent, which is separate from [data access control](/agents/data-access) (what data the agent can query).
### Tags (optional)
Use tags in the Lightdash metadata to control which metrics and dimensions the agent can access, restricting it to a specific slice of your semantic layer.

1. Tag the dbt model dimensions and metrics you want the agent to reach — it only sees fields with matching tags. See [limiting access to specific explores and fields](/agents/data-access#limiting-access-to-specific-explores-and-fields) for the tagging YAML and how explore-level and field-level tags interact with permissions and schema visibility.
...
<Frame>
  <img
    src="/images/agents/ai-tags.png"
    alt="The agent's Access control section, with a prod tag in the Tags field and a note that 32 explores are available"
  />
</Frame>

### Enable AI writeback (optional)
Allow your agent to edit dbt or native Lightdash YAML models from chat — renaming metrics, adding dimensions, or updating descriptions — and open a pull request for review. See [AI writeback](/agents/ai-writeback) for supported connections and setup.
...
## Slack Integration
Connect your agent to Slack channels so users can interact directly from Slack. Check out all the cool ways to integrate your agent into slack [here](/integrations/slack#using-the-slack-integration)
1. Add Slack to your organization in organization settings. An admin needs to complete this step. See the [how-to guide](/integrations/slack).
2. Add the desired slack channel to your agent integration settings.

<Frame>
  <img
    src="/images/agents/ai-integrations.png"
    alt="The agent's Integrations section, with Slack enabled and four channels selected"
  />
</Frame>
...
<Frame>
  <img
    src="/images/agents/add-lightdash-app-to-slack.png"
    alt="A Slack channel's add-apps dialog, with Lightdash for analytics highlighted in the search results"
  />
</Frame>
...
<Frame>
  <img
    src="/images/agents/use-agent-in-slack.png"
    alt="A Slack channel where a user mentions the agent to ask what kind of data it can access, with replies threaded below"
  />
</Frame>
...
<Frame>
  <img
    src="/images/agents/ai-agent-slack-thread-consent.png"
    alt="The Slack integration settings, with the AI Agents thread access consent toggle switched on"
  />
</Frame>
...
<Frame>
  <img
    src="/images/agents/what-kind-of-data-can-you-access.png"
    alt="An agent answering what data it can access with a breakdown of available tables, metrics, and dimensions"
  />
</Frame>
...
If you want multiple agents in the same channel, set up a **multi-agent channel** at the organization level instead. In a multi-agent channel, you don't pick which agent to talk to — you interact with the single Lightdash Slack app and the [AI Router](/agents/enable-ai-router) automatically picks the best-fit agent for each question.
...
If your organization needs the configured channel list to be a strict allowlist, an admin can turn off [automatic channel linking](/integrations/slack#automatic-channel-linking) in the Slack integration settings.
