...
It's the general-purpose counterpart to [AI writeback](/agents/ai-writeback): where writeback edits the dbt project that backs a Lightdash project and validates the change with `lightdash compile`, the coding agent targets any repo you can write to and delegates verification to that repository's own CI.
...
For changes that belong in the dbt / semantic-layer repository backing a Lightdash project, keep using [AI writeback](/agents/ai-writeback) — it also runs `lightdash compile` to validate the diff before opening the PR.
...
- The **AI coding agent** feature flag (`ai-coding-agent`) must be enabled for your organization.
...
- On self-hosted instances you need a configured [sandbox provider](/self-host/customize-deployment/sandboxes) and your own Anthropic API key, the same way you do for AI writeback. See [self-hosting AI writeback](/self-host/enterprise-features/ai-writeback) — the coding agent uses a separate, leaner sandbox image but the same setup.
...
2. Starts a fresh, isolated sandbox on your configured [sandbox provider](/self-host/customize-deployment/sandboxes) — a **lean** image containing only `git` and the Claude CLI. There is no dbt toolchain and no `lightdash compile` step, so the sandbox has no way to run a build even if it wanted to.
...
- [AI writeback](/agents/ai-writeback) — the same experience for changes to your project's dbt repository, with `lightdash compile` validation built in.
- [Self-hosting AI writeback](/self-host/enterprise-features/ai-writeback) — sandbox provider and Anthropic key setup that also applies to the coding agent.
