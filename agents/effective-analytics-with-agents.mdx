---
title: "Effective analytics with agents"
sidebarTitle: "Effective analytics"
description: "How to configure, scope, and improve your AI agents so they give accurate, governed answers"
doc-type: guide
---
...
- **Give [parameters](/semantic-layer/parameters) clear labels, descriptions, and options** — agents read them when deciding which value to set on a parameter-driven field, and fall back to the default when a question doesn't call for a specific value
...
Descriptions serve humans and agents alike; `ai_hint` metadata adds a layer only the agent sees — where you spell out which field is canonical, how users phrase questions, and the traps that lead to wrong answers. For what makes a good description, how to layer AI hints on top at the model, group, dimension, and metric level, and when to reach for a hint versus a better description, see [Writing useful descriptions in your semantic layer](/semantic-layer/writing-descriptions).
...
- **Anything that belongs in the semantic layer.** If it describes a specific field or model, prefer an [AI hint](/semantic-layer/writing-descriptions#layer-ai-hints-on-top-of-descriptions) — those are tied to the field and survive refactors.
...
## Curating verified answers

[Verified answers](/agents/verified-answers) teach your agent from your best examples. To build a set that actually raises answer quality:
...
## Making evaluations part of your workflow
...
Treat evaluations as an ongoing loop, not a one-off. Encourage your team to use thumbs-up and thumbs-down as they work, build [evaluation suites](/agents/create-evaluation-suites) from the real threads that went wrong, and run them before you ship changes to instructions or your semantic layer — so quality holds as the agent learns and evolves.
