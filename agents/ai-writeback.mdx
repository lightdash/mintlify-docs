...
- The preview also shows up under [Pull requests](/workflow/pull-requests) in project settings, with a **View preview** action on the row.
...
- [dbt write-back](/semantic-layer/dbt/write-back) — write back individual custom metrics, dimensions, and SQL Runner queries from the Lightdash UI.
