...
<Steps>
  <Step title="Rebuild charts against the migrated tables">
    Open each source dashboard screenshot side by side with Lightdash. For each chart, build the equivalent in Lightdash using the migrated metrics and dimensions. See [Configure charts](/explore/configure-charts) and the [chart types gallery](/visualizations).
