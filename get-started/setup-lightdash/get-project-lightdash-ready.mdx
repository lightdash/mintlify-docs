---
title: "Create your first project in Lightdash"
---
...
![](/assets/images/overview-intro-4debf555ace1283cfc3328617f044db1.png)
...
![](/assets/images/tables-view-98bf4db1f75c40cb891b689192f7278d.jpg)

Now that we know what tables are, let's add some and create our first project!

***

## Step 1: Install the Lightdash CLI tool[​](#step-1-install-the-lightdash-cli-tool "Direct link to Step 1: Install the Lightdash CLI tool")
...
![](/assets/images/example-table-1ea5c330560afe7d774ac2ef68d109f8.jpg)

<Accordion title="Error: Not a dbt project (or any of the parent directories). Missing dbt_project.yml file">
...
<Accordion title="Generate Tables and dimensions for some of the models in my dbt project:">
  ...
  ```
  {{  config(    materialized='table',    tags=['lightdash']  )}}
  ```
  ...
  ```
  lightdash dbt run -s tag:lightdash # all models with the lightdash taglightdash dbt run -s payments  # just paymentslightdash dbt run -s payments+ # payments and all childrenlightdash dbt run -s +payments # payments and all parentslightdash dbt run -s payments+ +customers tag:lightdash # mix and match
  ```
</Accordion>
...
![](/assets/images/update-connection-323a0f05ee9378aec0755be8ce3328eb.jpg)

[Edit this page](https://github.com/lightdash/lightdash/edit/main/docs/docs/get-started/setup-lightdash/get-project-lightdash-ready.mdx)
