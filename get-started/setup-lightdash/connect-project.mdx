...
<Tip>
  You can see a list of every time your project is compiled by visiting the [project compilation history](/references/workspace/project-compilation-history) page.
</Tip>
...
<Frame>
  <img src="/images/get-started/setup-lightdash/connect-project/screenshot-ip-address-20ea3b322ef6d3efc2fca2637ad5fe72.png" alt="screenshot-ip-address" />
</Frame>
...
Use an SSH tunnel when your Redshift cluster is in a private VPC and cannot be reached directly from Lightdash. Lightdash opens an SSH connection to a bastion/jump host that you run in a public subnet of your VPC, and forwards Redshift traffic through it.

<Accordion title="Setting up an SSH tunnel to a private Redshift cluster">
  The path is **Lightdash Cloud → your bastion host → private Redshift**.

  **1. Run a bastion/jump host.** Provision an EC2 instance (or equivalent) in a public subnet of the VPC that contains your Redshift cluster. The bastion needs a public IP, a user account for Lightdash to log in as, and network access to Redshift on port `5439` (typically via a VPC security group rule that allows the bastion's security group to reach the Redshift security group on `5439`).

  **2. Configure the SSH tunnel in Lightdash.** In your Redshift connection settings, enable **Use SSH tunnel** and fill in:

  - **SSH Remote Host**: the bastion's public hostname or IP.
  - **SSH Remote Port**: the bastion's SSH port, usually `22`.
  - **SSH Username**: the Linux user Lightdash logs in as (for example `ec2-user` or `ubuntu`).

  Lightdash generates a public SSH key for the connection. Copy it.

  **3. Authorize the key on the bastion.** Append the public key to `~/.ssh/authorized_keys` for the SSH user you configured above. Authentication is SSH public-key auth — no separate certificate is needed.

  **4. Point Redshift at its private hostname.** Keep the **Host** field set to the private Redshift endpoint (the one that resolves inside the VPC). Lightdash resolves and connects to it from the bastion, not from the public internet.

  **5. Allow Lightdash to reach the bastion.** Add Lightdash's [static IP addresses](#adding-lightdashs-static-ip-addresses-to-your-allow-list) to the bastion's security group on the SSH port you configured.

  **Encryption between the bastion and Redshift is optional.** The SSH tunnel already encrypts traffic between Lightdash and the bastion. You can additionally enable SSL on the Redshift connection to encrypt the bastion-to-Redshift hop; SSH tunnels work with or without it.
</Accordion>
...
We recommend you connect to Github using OAuth. This gives Lightdash a direct connection to the repo, so it won't lose access when individuals leave the company, and it can create pull requests, which is required for [dbt write-back features](/guides/developer/dbt-write-back). This connection lives at the organization level, so you can map different dbt project repos to each Lightdash project without needing a new access token. 
...
<Check>
  After adding your Github information, fill out the [dbt project details](/references/integrations/dbt-projects#dbt-project-settings) and you're all set!
</Check>
...
<Check>
  After adding your Gitlab information, fill out the [dbt project details](/references/integrations/dbt-projects#dbt-project-settings) and you're all set!
</Check>
...
<Check>
  After adding the Azure DevOps details, fill out the [dbt project details](/references/integrations/dbt-projects#dbt-project-settings) and you're all set!
</Check>
...
<Check>
  After adding your Bitbucket information, fill out the [dbt project details](/references/integrations/dbt-projects#dbt-project-settings) and you're all set!
</Check>
...
* [dbt write-back features](/guides/developer/dbt-write-back)
