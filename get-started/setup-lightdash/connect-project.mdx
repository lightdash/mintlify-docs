...
Enable to input your **SSH Remote Host**, **SSH Remote Port**, **SSH Username**, and to generate a public SSH key. Use this when your Postgres database is in a private VPC and cannot be reached directly from Lightdash. See [Connect through an SSH tunnel](/get-started/setup-lightdash/ssh-tunnel) for the full setup.
...
Enable to input your **SSH Remote Host**, **SSH Remote Port**, **SSH Username**, and to generate a public SSH key. Use this when your Redshift cluster is in a private VPC and cannot be reached directly from Lightdash. See [Connect through an SSH tunnel](/get-started/setup-lightdash/ssh-tunnel) for the full setup.
...
<Info>
**MetricFlow metrics are translated automatically.** If your dbt project defines [MetricFlow](https://docs.getdbt.com/docs/build/about-metricflow) semantic models and metrics, Lightdash pulls them from the dbt Cloud Discovery API on each refresh and translates the supported ones into Lightdash metrics — no dbt Semantic Layer configuration needed. See [dbt MetricFlow metrics](/references/integrations/dbt-metricflow#using-the-dbt-cloud-connection) for what translates and for dbt Cloud–specific limitations.
</Info>
