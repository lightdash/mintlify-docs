...
After saving, users who open the project will see **Sign in with AWS** on the warehouse credentials form. Clicking it opens an AWS IAM Identity Center device-code login in a popup window; once they approve the request in AWS, Lightdash stores the resulting short-lived AWS role credentials as their personal Redshift warehouse credentials and automatically sets them as the project's active credential preference — no separate **Save** step is required.

Because IAM Identity Center issues temporary credentials, users will need to re-run **Sign in with AWS** from their **My warehouse connections** settings when their AWS session expires.

If a user is in an environment without access to the IAM Identity Center popup, they can still switch the credential type on their personal credentials form to **AWS IAM (CLI credentials)** and paste short-lived AWS access keys from their own CLI or SSO profile as a fallback.
