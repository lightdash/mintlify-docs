...
4. The wizard shows **Connected as `<user>` ✓**. Pick a database, warehouse, role, and schema from the session-populated dropdowns (or accept the session defaults), then submit the form to create the project.
