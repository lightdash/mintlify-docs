...
2. [How dbt configuration macros resolve in Lightdash](/get-started/setup-lightdash/connect-project#2-how-dbt-configuration-macros-resolve-in-lightdash)
3. [Connect to your dbt project](/get-started/setup-lightdash/connect-project#3-import-a-dbt-project)
...
## 2. How dbt configuration macros resolve in Lightdash

When Lightdash connects to your dbt project, it compiles a `profiles.yml` file based on the connection settings you configure in the Lightdash UI. This means that any dbt macros in your project that use target settings or environment variables will be executed by Lightdash and resolved according to those settings.

In other words, if you're using macros to dynamically configure aspects of your dbt project based on the environment, Lightdash will respect that same logic when it compiles your dbt project and runs queries.

### Understanding dbt targets
...
<Note>
**dbt Core vs dbt Cloud:** In dbt Core, teams commonly use targets to manage different environments. In dbt Cloud, environment variables are more commonly used for the same purpose.
</Note>

### Environment variables in dbt and Lightdash
...
You can configure environment variables in your Lightdash project's dbt connection settings. Any environment variables you set will be available to your dbt macros when Lightdash compiles your project.
...
### How macros resolve in Lightdash

dbt macros often use the target name or environment variables to determine their behavior. Common examples include macros that set schemas, datasets, warehouses, or other configuration. Lightdash respects this logic when compiling your dbt project.

#### Example: Schema configuration

Let's look at dbt's default `generate_schema_name` macro:
...
<Tip>
The `generate_schema_name` macro is a default dbt macro and its behaviour can be overridden by adding a macro to dbt with the same name. If this macro is not showing up in your dbt project, it means that your project is using the default macro. 
It is a good idea to check your project for custommacros which can alter your project's configurations.  
</Tip>
