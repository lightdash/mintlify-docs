...
**<Icon icon="bolt"/> The Lightdash Way:**
...
<Icon icon="thumb-up"/>: very flexible, totally self-serve (as long as everyone knows SQL)

<Icon icon="thumb-down"/>: everybody has to know sql, everybody needs context of raw data, duplicated work, bound to be errors
...
<Icon icon="thumb-up"/>: only analysts have to have data skills, fewer errors (since only data experts are answering data questions)

<Icon icon="thumb-down"/>: doesn't scale, analysts become a bottleneck, this kind of work sort of sucks :(


### 3. The data builders give you meaningful building blocks to answer your own data questions.

**<Icon icon="bolt"/> The Lightdash Way:** We give your data team the tools they need to build metrics + dimensions that everyone else can use in a user-friendly interface.
...
<Icon icon="thumb-up"/>: you only need a few SQL experts, anyone can ask questions using the data, fewer errors, scaleable

<Icon icon="thumb-down"/>: you need a data team, this system needs maintenance
