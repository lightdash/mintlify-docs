...
<Warning>
iframe embedding supports dashboards and [data apps](/embed/embed-data-apps). Chart embedding requires the [React SDK](/embed/react-sdk).
</Warning>
...
3. **Generate JWT** - See [embedding reference](/embed/reference#jwt-token-structure) for token structure
...
<Info>
  **Beta:** The embed `timezone` param is currently in the [Beta](/help/feature-maturity-levels) phase. Contact support to enable it for your organization.
</Info>
...
<Info>
The embed `timezone` param sits at the top of the [query timezone hierarchy](/workspace-admin/set-project-timezone#how-the-project-query-timezone-is-resolved) for that session and overrides any per-chart timezone pin. It only applies to direct iframe and shareable URL embeds, not the React SDK, where the host app's URL is unrelated to the embed.
</Info>
...
- Ensure JWT payload structure matches [embedding reference](/embed/reference#jwt-token-structure)
...
<CardGroup cols={2}>
  <Card title="Embedding reference" icon="book" href="/embed/reference">
