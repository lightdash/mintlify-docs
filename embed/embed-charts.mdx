...
Pass `user` metadata in the token to attribute queries in [usage analytics](/workspace-admin/usage-analytics#query-tags), and `userAttributes` for [row-level security](/workspace-admin/user-attributes). Styling and localization props are covered in the [React SDK reference](/embed/react-sdk#lightdashchart).
