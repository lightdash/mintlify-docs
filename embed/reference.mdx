...
| --- | --- | --- |
| Dashboard | JWT interactivity flags and their defaults | JWT interactivity flags **OR** the actor's corresponding embed scopes |
...
In `'roles'` mode, either source can grant a capability. For example, `canExportCsv: false` plus `view:EmbedCsvExport` allows CSV export, and `canExportCsv: true` allows it even without the scope. In `'default'` mode, the scope is ignored.

The dashboard scopes correspond to these fields inside `content`:

| JWT field | Embed scope |
| --- | --- |
...
For capabilities you want to manage through roles, set the corresponding JWT flags to `false`:
...
In this example, grant or remove `view:EmbedExplore`, `view:EmbedCsvExport`, and `view:EmbedPagePdfExport` on the actor's role to control those three capabilities. Other options keep their JWT defaults.

<Warning>
Removing a dashboard scope cannot override a JWT flag set to `true`. PDF export also defaults to enabled when `canExportPagePdf` is omitted, so explicitly set it to `false` if you want the PDF scope to control access.
</Warning>
...
Grant `view:EmbedAiAgent` to allow access, or remove its last effective grant to block access. Unlike dashboard capabilities, there is no AI-access boolean flag to combine with this scope. This controls embedded AI access, not access to AI in the main Lightdash app, and does not change AI Explore interactivity flags. Dashboard tokens do not authorize AI agent endpoints.
