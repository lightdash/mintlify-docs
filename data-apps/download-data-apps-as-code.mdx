---
title: "Download data apps as code"
sidebarTitle: "Download as code"
doc-type: tutorial
description: "Download a data app's source, edit it locally, and publish new versions with the Lightdash CLI"
---

<Info>
  Building data apps locally is <Badge icon="building-plus" color="blue" size="sm" shape="pill">Enterprise</Badge> and requires data apps, including the build sandbox, to be enabled on your Lightdash instance. See [Self-hosting data apps](/self-host/enterprise-features/data-apps) if you run your own instance.
</Info>
...
[Custom chart types](/explore/chart-types/custom-project-charts) share this folder layout and build pipeline but are a separate as-code resource: they live in `lightdash/chart-types/` and move with the `--chart-types` flags instead of `--apps`. See [Chart types as code](/explore/chart-types/custom-project-charts#chart-types-as-code).
...
- An **Editor** role or above on the project — uploading requires the `create:DataApp` scope. Non-Editors can be granted `create:DataApp@preview` instead to upload only to preview projects they created themselves; see [Restrict data app uploads to previews](/workspace-admin/custom-roles#restrict-data-app-uploads-to-previews-agentic-development-workflow).
...
<Warning>
  Custom dependencies are <Badge icon="test-pipe-2" color="orange" size="sm" shape="pill">Experimental</Badge> and off by default. Enabling them is a security decision each organization makes for itself — contact Lightdash support (or your admin, on self-hosted instances) to turn on the feature flag. Uploading an app that declares custom dependencies additionally requires the `manage:DataAppDependency` permission, granted only to admins by default.
</Warning>

By default, apps build against a fixed, pre-installed library set that Lightdash vets and ships — edits to `package.json` are ignored. Turning custom dependencies on lets an app pull additional packages from the public npm registry into the build sandbox, which is why it's opt-in:

* A third-party library becomes part of your app. It can shape how the app behaves and touch any data the app is allowed to query, so the main consideration is trust in the libraries you choose. That's also why the upload permission (`manage:DataAppDependency`) is restricted to admins by default.
* Lightdash adds several layers of protection to reduce that risk: every uploaded version is screened against the OSV malicious-package feed, installs use `--frozen-lockfile --ignore-scripts` so lifecycle scripts can't run, the sandbox has no access to Lightdash credentials, and network egress is limited to the npm registry.

With custom dependencies enabled, you can extend the pre-installed set from the app folder:
...
**Additional hardening:** install scripts also never run on your machine — the scaffolded `.npmrc` sets `ignore-scripts=true`; keep it. Self-hosted instances can further tighten the policy by requiring a minimum package release age (`LIGHTDASH_APP_DEPENDENCY_MIN_RELEASE_AGE_DAYS`) or pointing installs at an internal mirror (`LIGHTDASH_APP_DEPENDENCY_REGISTRY_HOSTS`).
