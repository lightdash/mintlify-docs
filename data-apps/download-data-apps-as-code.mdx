...
[Custom chart types](/visualizations/chart-studio) share this folder layout and build pipeline but are a separate as-code resource: they live in `lightdash/chart-types/` and move with the `--chart-types` flags instead of `--apps`. See [Chart types as code](/visualizations/chart-studio#chart-types-as-code).
