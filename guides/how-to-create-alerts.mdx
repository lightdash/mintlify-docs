...
Only users with [admin, developer or edit access](/references/workspace/roles) can create alerts. To create an alert, click on the three-dot menu in the top-right corner of your chart, then click on `Alerts`.
...
## Filter and parameter overrides

You can scope an alert to a subset of the chart's data by overriding the chart's filters or parameters on the `Filters` and `Parameters` tabs of the alert configuration. The overrides are layered on top of the chart's saved query before the threshold is evaluated — the saved chart is left untouched.

This is useful when you want to power many alerts from a single shared chart and route each one to its own destination — for example, one alert per customer, region, or product, each going to a different Slack channel or email address — without cloning the chart for every segment.

On the `Filters` tab you can:

1. Add a new filter on any field in the chart's explore.
2. Update the operator or value of an existing filter (for example, change `customer is "ACME"` to `customer is "Globex"`).
3. Remove a filter you added for this alert.

On the `Parameters` tab you can override any [project parameter](/guides/developer/using-parameters) used by the chart. Click the pencil icon to set a value for the alert, or the revert icon to fall back to the chart's default.

When the alert runs, Lightdash applies your filter and parameter overrides, evaluates the threshold against the resulting first row, and sends a notification if the condition is met.
...
Once you have a webhook, add it to the the list of destinations in the alerts form. 
