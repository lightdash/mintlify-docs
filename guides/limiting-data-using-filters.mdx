...
<Info>
If you want to restrict access to temporary filters, you can disable the `Add filters` button on a dashboard. Head to edit mode, click the 'x' on the `Add filters` dropdown to remove it. This works the same way as [disabling Date Zoom](/get-started/exploring-data/interactive-dashboards#date-zoom). End users will then be restricted to only using filters that have been specifically set up in dashboard edit mode.
</Info>
