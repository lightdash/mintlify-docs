...
<Info>
Locked dashboard filters are currently behind a feature flag. Reach out to support to enable `lock-dashboard-filters` for your organization.
</Info>

You can lock a saved dashboard filter to prevent viewers from changing its value. This is useful when a filter defines critical context for the dashboard — for example, a tenant ID, a region, or a fiscal-year boundary — and changing it would produce misleading numbers.

When a filter is locked:

- Viewers see a closed-padlock indicator on the filter chip and cannot click into it to change the value.
- URL parameters (`?filters=...` and `?tempFilters=...`) that target a locked field are ignored, and a one-time toast tells the viewer the override was dropped.
- [Embed JWT](/references/embedding) `externalFilters` that target a locked field are also ignored — the dashboard's saved value is used instead.
...
2. Hover the filter chip you want to lock — an open-padlock button appears next to the `x`.
3. Click the padlock. The icon switches to closed and stays visible at rest.
4. Click `save` on the dashboard to persist the lock.

To unlock, hover the chip in edit mode and click the closed padlock again.

<Info>
A locked filter that is also marked `Required` must have a default value. If you clear the value while both options are on, the `Apply` button is disabled until you set one.
</Info>
