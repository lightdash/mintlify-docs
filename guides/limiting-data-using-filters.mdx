...
If you want to guarantee that a dashboard always shows data scoped to a specific value (for example, a region, a tenant, or a date range), you can **lock** a saved dashboard filter. A locked filter behaves the same way for everyone who views the dashboard — viewers can see what's filtered, but they can't change the value, and URL or embed overrides targeting a locked field are ignored.

Use a locked filter when you want to:

- Ship a dashboard that is permanently scoped (for example, "EMEA Sales") and prevent viewers from accidentally changing the scope.
- Stop someone from sharing a link with an override that hides or changes a filter you rely on.
- Enforce a filter value on embedded dashboards regardless of what the embedding application passes in `externalFilters`.
...
In view mode, a locked filter chip shows a closed-lock indicator on the right. Clicking the chip does nothing — a tooltip explains that the filter is locked. The locked value still applies to every tile that uses the filter's field, exactly like a normal saved filter.

#### How locking interacts with URL and embed overrides

Locked filters take precedence over every override mechanism:

- **URL overrides** — if a link contains `?filters=` or `?tempFilters=` that target a locked field, the override is dropped on load, the URL is cleaned up, and viewers see a one-time toast explaining that the override was ignored.
- **Embed `externalFilters`** — when an embed JWT passes `externalFilters` that target a locked field, the server uses the dashboard's saved locked value and ignores the override.
- **Required + locked** — if a filter is both `Required` and `locked`, you must give it a value in edit mode. Clearing the value disables **Apply** with the message "A locked, required filter must have a value".
