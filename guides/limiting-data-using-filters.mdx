---
title: "Using Filters"
description: Filters appear at the top of the Explore view or dashboard and allow users to change the data being pulled into your charts.
---
...
### Upload a CSV to add multiple filter values

If you need to add a long list of values to a string filter, you can upload a CSV file instead of entering each value manually. This is useful when you have hundreds or thousands of values to filter by (e.g., a list of customer IDs or product SKUs).

To upload a CSV:

1. Open the filter dropdown for a string field
2. Click the **Upload CSV** button
3. Select a CSV file containing your filter values (one value per row, or a single column)
4. The values from your CSV will be added to the filter
...
<Info>
If you want to restrict access to temporary filters, you can hide the `Add filters` button on a dashboard. Head to edit mode and toggle the **eye icon** next to the `Add filters` dropdown to hide or show it. When hidden, end users are restricted to only using filters that have been specifically set up in dashboard edit mode.
</Info>
...
* They do **not** override chart-level filters on the same field. If a saved chart is filtered to `country` **is** `India` and the dashboard has an empty `country` filter, tiles keep the `India` filter until a viewer picks a value in the dashboard. As soon as a viewer selects a value, normal [dashboard-overrides-chart behaviour](#dashboard-filters-override-filters-applied-in-a-saved-chart-on-the-same-field) resumes.
...
Value-less relative-date operators (`in the current`, `not in the current`) are **not** treated as empty because they get their period from a unit-of-time setting rather than a value. They continue to apply to tiles just like any other configured filter.

<Note>
**Migration note.** Before this change, an empty active dashboard filter would clobber a chart-level filter on the same field at runtime — the tile would run without either filter. Now, the chart-level filter is honored until a viewer sets a value. If you previously used an empty dashboard filter specifically to strip a chart filter, either mark the dashboard filter as required, set an explicit default value, or remove the chart-level filter from the saved chart.
</Note>

##### Required filters and filter requirement groups

Dashboard filters can be marked as **required**, meaning the dashboard won't load any data until a viewer picks a value. You can also combine several filters into a **requirement group** — an "at least one of" rule where setting *any* filter in the group unlocks the dashboard. This is handy when viewers should always narrow down the data, but it doesn't matter whether they do it by `Region` or by `Product line`.

<Info>
Filter requirement groups and the guided setup experience are gated behind the `dashboard-filter-requirements` feature flag. With the flag off, only the per-filter `Required` toggle is available and any saved group configuration is ignored. [Contact Lightdash support](mailto:support@lightdash.com) to enable it for your organization. If you're self-hosting, set `LIGHTDASH_ENABLE_FEATURE_FLAGS=dashboard-filter-requirements`.
</Info>

**Building filter rules (editors)**

While editing a dashboard, you'll find a **Required** toggle in each filter's popover, along with an **Add an alternative filter** button to turn a single required filter into a group. To see and manage all of a dashboard's rules in one place, open the **Filter rules** popover next to the filter bar:

1. Click **Add rule** and pick the filters for the rule. A rule with a single filter is a plain required filter; add more filters to turn it into an "at least one of" group. You can only pick saved filters that have no default value and aren't already part of another rule.
2. Optionally, write a **Note for viewers**. It appears as the subtitle on the guided setup card, so use it to explain *why* the dashboard needs these filters.
3. Click **Save** in the popover to apply your rules, then save the dashboard as usual. Closing the popover without saving discards your changes.

<Frame>
  <img
    alt="The Filter rules popover on a dashboard in edit mode, showing a required filter and an at-least-one-of group"
    className="block dark:hidden"
    src="/images/guides/limiting-data-using-filters/filter-requirements-rules-popover-light.png"
  />
  <img
    alt="The Filter rules popover on a dashboard in edit mode, showing a required filter and an at-least-one-of group"
    className="hidden dark:block"
    src="/images/guides/limiting-data-using-filters/filter-requirements-rules-popover-dark.png"
  />
</Frame>

Filters in a rule can't have a default value — a default would satisfy the rule on its own, defeating the point. If a required filter ends up with a value, Lightdash clears it when you save the dashboard.

**What viewers see**

While any rule is unmet, chart tiles stay locked and don't load any data. Instead, the dashboard shows a **guided setup card** with one section per rule, a picker for each filter, and a footer that counts down the remaining rules. Filters in the filter bar also show a requirement icon while their rule is unmet. The dashboard loads the moment every rule is met — and locks again if a viewer clears a value that was satisfying a rule.

Viewers can dismiss the card with **Set filters in the toolbar instead** and pick values from the filter bar directly; either way, tiles stay locked until every rule is met.

<Frame>
  <img
    alt="The guided setup card shown to viewers while a dashboard's filter rules are unmet"
    className="block dark:hidden"
    src="/images/guides/limiting-data-using-filters/filter-requirements-guided-setup-light.png"
  />
  <img
    alt="The guided setup card shown to viewers while a dashboard's filter rules are unmet"
    className="hidden dark:block"
    src="/images/guides/limiting-data-using-filters/filter-requirements-guided-setup-dark.png"
  />
</Frame>

A rule counts as met when at least one of its filters has a value, **or** uses an operator that filters data without needing a value (`is null`, `is not null`, `in the current`, `not in the current`, `in the period-to-date`). A filter left as `is any value` doesn't count.

On dashboards with [tabs](/get-started/exploring-data/dashboards#add-tabs), rules are checked per tab: a rule only locks the tabs where its filters apply to at least one tile, so viewers can still use tabs the rule doesn't touch.

**Dashboards-as-code**

A single required filter is stored as `required: true` on the filter. Requirement groups are stored on each member filter as a `requiredGroupId` — every filter with the same `requiredGroupId` belongs to the same "at least one of" group. The note for viewers lives on the dashboard config as `requiredFiltersNote`.

```yaml
config:
  requiredFiltersNote: "Pick a region or a product line to load the dashboard."
filters:
  dimensions:
    - id: region-filter
      label: Region
      requiredGroupId: geo-or-product
      # ...
