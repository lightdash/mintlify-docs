...
A rule counts as met when one of its filters has a value or uses an operator that needs none (`is null`, `is not null`, `in the current`, `not in the current`, `in all periods to date`). On dashboards with [tabs](/get-started/exploring-data/dashboards#add-tabs), a rule only locks the tabs its filters apply to.
