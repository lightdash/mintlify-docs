...
<Frame>
  <img
    ...
    src="/images/guides/limiting-data-using-filters/filter-requirements-required-toggle-light.png"
  />
  <img
    alt="A dashboard filter popover in edit mode with the Required toggle switched on for the Payment method filter"
    className="hidden dark:block"
    src="/images/guides/limiting-data-using-filters/filter-requirements-required-toggle-dark.png"
  />
</Frame>
...
A rule counts as met when one of its filters has a value or uses an operator that needs none (`is null`, `is not null`, `in the current`, `not in the current`, `in the period-to-date`). On dashboards with [tabs](/get-started/exploring-data/dashboards#add-tabs), a rule only locks the tabs its filters apply to.
