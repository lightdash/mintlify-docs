...
### Locking saved dashboard filters

<Info>
Locking dashboard filters is rolling out behind the `lock-dashboard-filters` feature flag. If you don't see the lock toggle on your filter chips, ask your Lightdash admin to enable it for your organization.
</Info>

If you want viewers to see the value a filter is applied with but not be able to change it, you can **lock** a saved dashboard filter. Locked filters are useful when a dashboard's results only make sense for a specific scope — for example, a single region, customer tier, or fiscal year — and you want to prevent viewers from accidentally (or intentionally) changing that scope.

When a filter is locked:

- Viewers see the filter chip with a closed-padlock indicator and a tooltip explaining the lock. Clicking the chip does nothing in view mode.
- URL overrides (`?filters=...` and `?tempFilters=...`) that target a locked filter's field are ignored. A one-time toast tells the viewer the override was dropped.
- Embedded dashboards ignore `externalFilters` in the embed JWT when they target a locked field, so the saved value is always used in queries.
- Editors can still change or unlock the filter from dashboard edit mode.

#### To lock a saved dashboard filter:

1. Open your dashboard and click `Edit Dashboard`.
2. Hover over the saved filter chip you want to lock. An open-padlock button appears next to the `×`.
3. Click the padlock. The icon switches to closed and stays visible at rest.
4. Click `Save` to persist the lock.

To unlock a filter, hover the chip in edit mode and click the closed padlock again, then save.

<Info>
A locked filter that is also marked as `Required` must have a default value. If you clear the value of a locked, required filter, `Apply` is disabled with a "A locked, required filter must have a value" message.
</Info>
