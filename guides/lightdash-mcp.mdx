...
4. **Use field value search** when building filters with `search_field_values` to find valid filter values
5. **Execute queries** with `run_metric_query` once you have the correct field IDs
...
- **run_metric_query**: Ensure all dimensions and metrics exist in the selected explore
- **search_field_values**: Provide both table name and fieldId from previous searches
...
- Use `search_field_values` to explore possible values before creating filters
```
...
- **Set active project** - Switch context between different projects (required before accessing any data) and optionally set tags to filter data
- **Get current project** - Check which project is currently active and view selected tags
...
- **Search field values** - Search for unique values of a specific field in a table, useful for building filters or exploring data
- **Run metric query** - Execute metric queries and get results as CSV data for analysis and export

#### Tag filtering

MCP supports the same tag-based filtering system as [AI agents](/guides/ai-agents#limiting-access-to-specific-explores-and-fields), allowing you to control which metrics and dimensions are accessible. When you set a project, you can optionally specify tags to filter explores and fields:

- Tags work identically to AI agent tags - fields with **any** of the specified tags will be accessible
- **Tags are per-project in MCP**: Unlike AI agents where tags are global, MCP tags are specific to each project
- **Project and tag selections persist across threads**: Once you set a project and tags, they will be remembered in future conversations with Claude
- Only fields with the specified tags will be considered when searching or querying data
- This helps focus the AI on relevant data and ensures it works with the appropriate datasets

For example, when setting a project:
```
Set project: Sales Analytics
Tags: ["marketing", "revenue"]
```

This will ensure that only metrics and dimensions tagged with "marketing" or "revenue" are available to the AI assistant. These selections will persist across your threads with Claude, so you don't need to reconfigure them in each conversation. See the [AI agents documentation](/guides/ai-agents#limiting-access-to-specific-explores-and-fields) for detailed instructions on how to add tags to your data models.
...
To get the most value from MCP, ensure your Lightdash data is well-organized and documented. Since MCP uses the same underlying infrastructure as AI agents, the same best practices apply. See our [AI agents best practices guide](/guides/ai-agents#best-practices) for detailed recommendations on:

- Organizing and naming your data models
- Writing effective documentation and AI hints
- **Using tags to control data access** - The same tagging system works for both AI agents and MCP
