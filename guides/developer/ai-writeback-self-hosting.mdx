...
- Projects set to `latest` compile against the newest available version (currently 1.12, where the adapter has a 1.12 release).
- Projects pinned to a version below 1.8 (which is end-of-life upstream) are clamped up to 1.8 inside the sandbox.

At the time of writing, no stable `dbt-core` 1.12 is published to PyPI, so `dbt-core~=1.12.0` resolves to the latest pre-release. Snowflake, BigQuery, and Databricks have 1.12 adapter builds; adapters without a 1.12 release stay on their existing 1.11-era pins.
