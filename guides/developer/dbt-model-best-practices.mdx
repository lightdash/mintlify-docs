...
We recommend materializing your dbt models as **tables** instead of views, especially for models that are frequently queried in Lightdash. Views execute the underlying SQL each time they're queried, which increases query time and warehouse costs.
...
For large datasets, consider using **incremental models** to reduce build times while still maintaining table materialization:
