...
- **[External connections](/guides/data-apps/external-connections) in data apps** — requests to third-party HTTP APIs are runtime fetches through the Lightdash proxy and are not covered by results caching.
...
When a project is configured to [require user credentials](/references/workspace/personal-warehouse-connections), cached results are scoped to each individual user. This means:
