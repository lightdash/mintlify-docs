...
5. **Table calculations** — Liquid is evaluated. `ld.query.fields contains "..."` is true for any column the table calculation can reference, since selected dimensions and metrics make up the result set. `ld.query.filters contains "..."` reflects the dimension and metric filters applied to the underlying query, which aren't otherwise visible in the table calculation's input.
