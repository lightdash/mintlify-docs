...
5. **Table calculations** — Liquid is evaluated, but introspection is rarely useful here. A table calculation already knows the rows and columns it's operating on, so `ld.query.fields contains "..."` mostly tells you what you can already see. Stick to dimensions and metrics unless you have a clear reason.

It does **not** work in:

- **Rich text templates** — those use a different engine (`${value.formatted}`) and run after the query, not before.
...
- **`explore_table_name`** is the name of the model in the explore (the dbt model name, or the joined-table label/key)
