...
A common use of dashboards as code is copying a dashboard from one place to another — from one Lightdash instance to another, or between two projects on the same instance. The steps below walk through the full round-trip.

Prerequisites:

* The [Lightdash CLI](/references/lightdash-cli) installed and authenticated.
...
### Step 1: Download from the source instance
...
lightdash login https://yourcompany.lightdash.cloud
# Or use the shared clouds:
...
# Download the dashboard (charts, virtual views, and data apps are included automatically)
lightdash download -d my-dashboard-slug
```

### Step 2: Upload to the target instance or project

If the target is on a different instance, log in to it first. If it's a different project on the same instance, skip the login and go straight to `set-project`.

```bash theme={null}
# Only needed when moving to a different instance
lightdash login https://yourcompany.lightdash.cloud
...
* If the dashboard uses pivoted charts, run the download with `--strip-pivot-series` so the chart YAML doesn't hardcode pivot values from the source project. See [Make downloaded charts portable across projects](#make-downloaded-charts-portable-across-projects).
