...
## Migrating a dashboard between Lightdash instances or projects

A common use of dashboards as code is copying a dashboard from one place to another — from a project on one Lightdash instance to a project on another instance, or between two projects on the same instance. The steps below walk through the full round-trip.
...
* The [Lightdash CLI](/references/lightdash-cli) installed.
* The target project has the same dbt models and explores that the dashboard's charts reference. If they don't match, the upload will fail validation.
...
See [Make downloaded charts portable across projects](#make-downloaded-charts-portable-across-projects) for details.
...
If the target project is on the same instance, skip the login step — just select the target project and upload:
...
* `--force` is required to create new content that doesn't already exist in the target project.
* `--validate` checks for errors after upload.
* The target project must expose the same dbt models and explores that the charts reference. Missing fields or tables will fail validation.
...
<Note>
  Organization mode cannot be combined with project-content flags such as `--charts`, `--dashboards`, `--project`, `--nested`, or `--force`. Uploads never delete resources in Lightdash — remove unwanted roles, users, groups, or themes in the UI.
</Note>

### Data App themes as code

Themes are strict multi-file packages under `lightdash/themes/<slug>/`, rather than single YAML documents. Organization download exports UI-created themes into the canonical package layout. Organization upload creates or updates themes by manifest slug, skips unchanged packages, and leaves remotely existing themes alone when their local directory is absent.

Theme upload does not change the organization default. Each theme import is atomic, but the complete multi-theme upload is not one transaction. For the manifest, supported assets, validation limits, synchronization behavior, and visual evaluation workflow, see [Manage themes as code](/guides/data-apps/themes#manage-themes-as-code).
