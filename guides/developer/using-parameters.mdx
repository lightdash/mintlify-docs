...
Static `options` accept either plain strings/numbers or `{ label, value }` objects. Use the object form when the text you want users to see in the UI should differ from the value substituted into SQL — for example, when the underlying value is an ID. Plain string/number arrays continue to work when the label and value are the same. If you set a `default` with labeled options, it must match a `value`, not a `label`.
...
<Frame>
  <img src="/images/guides/developer/using-parameters/reserved-parameter-autocomplete-light.png" alt="Custom SQL autocomplete showing the date_zoom reserved parameter labelled as a System variable" className="block dark:hidden" />
  <img src="/images/guides/developer/using-parameters/reserved-parameter-autocomplete-dark.png" alt="Custom SQL autocomplete showing the date_zoom reserved parameter labelled as a System variable" className="hidden dark:block" />
</Frame>
