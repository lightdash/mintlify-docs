...
<Warning>
  Parameters must include either `options` (a list of allowed values) or `options_from_dimension` (to dynamically populate values from a dimension) to work. Without one of these, the parameter will not function correctly.
</Warning>
