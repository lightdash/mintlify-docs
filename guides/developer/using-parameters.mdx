...
{/* TODO(screenshot): custom SQL autocomplete showing the `date_zoom` reserved parameter labelled as a "System variable". Save to `/images/guides/developer/using-parameters/reserved-parameter-autocomplete.png` and replace this comment with a `<Frame>` wrapping the image. */}
