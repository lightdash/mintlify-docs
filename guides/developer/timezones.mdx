...
- **Dimension picker icons** - in the Explore field picker, temporal dimensions show a tooltip describing whether they shift with the chart's timezone. Dimensions that are not affected by the timezone (for example, `date`-typed columns or dimensions with `convert_timezone: false`) display a pinned variant of the calendar or clock icon.
