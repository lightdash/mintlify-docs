...
<Info>
  Several features on this page are [Experimental](/references/workspace/feature-maturity-levels) and need `EnableTimezoneSupport` enabled for your organization (contact support to turn it on). Each section is marked accordingly. Without the setting, the project timezone only adjusts relative date filter boundaries - everything else stays in UTC.
</Info>

### How the project timezone is resolved

Lightdash picks the active project timezone using the hierarchy below (highest priority wins):
...
This is the recommended way to configure the query timezone for most teams. It fixes relative date filter boundaries on its own, and unlocks the rest of the project-timezone behaviors below when `EnableTimezoneSupport` is enabled.
...
- **Display** - table cells, chart axes, tooltips, and CSV/Excel/Google Sheets exports render timestamps in the project timezone. The resolved zone appears as a badge on each chart card.
- **`DATE_TRUNC` grouping** - day, week, month, quarter, and year buckets align to the project-timezone calendar.
- **Calendar extracts** - dimensions like "day of week", "month number", and "hour of day" are computed in the project timezone, so they line up with their `DATE_TRUNC` siblings on the same row.
...
Individual dimensions can opt out of project-timezone formatting with `convert_timezone: false`. See [Per-dimension display opt-out](#per-dimension-display-opt-out) below.
...
By default, every `timestamp` dimension follows the project timezone for display, grouping, and extracts. Some columns - system timestamps, audit logs, pre-converted values - should render in their raw warehouse value instead. Set `convert_timezone: false` on the dimension to opt that single column out:
...
The override propagates to every time-interval child of the dimension (`_day`, `_month`, `_day_of_week_index`, …), so all of them render raw too.
...
**What it doesn't change:** filters. Filters always use the project timezone so every dimension on a row agrees on whether the row matches the filter. For example, a filter like `"between 2026-01-01 and 2026-01-31"` cuts at midnight in the project timezone, even though the column itself renders raw.

### Data timezone

<Info>
  **Experimental** - requires `EnableTimezoneSupport`. The setting only appears in warehouse advanced settings once the feature is enabled. Not available on **BigQuery** or **Athena** ([#7010](https://github.com/lightdash/lightdash/issues/7010)).
</Info>

The **data timezone** answers a different question than the project timezone:

> "What timezone are my NTZ (no-timezone) timestamps actually stored in?"

Set it under **Warehouse connection → Advanced settings → Data timezone**. Lightdash then issues a session command on every query (`SET timezone TO 'America/Chicago'` on Postgres, `ALTER SESSION SET TIMEZONE = 'America/Chicago'` on Snowflake, etc.) so the warehouse interprets ambiguous NTZ values in that zone before comparing them to filter literals or grouping.
...
- **Without it** - the warehouse assumes UTC.
- **With `America/Chicago`** - the warehouse reads it as 6pm Chicago, equivalent to midnight UTC the next day.
...
If your data is stored in UTC and `EnableTimezoneSupport` is enabled for your organization, the simplest path is to set the [project query timezone](#project-query-timezone-ui-setting) to your preferred zone - chart axes, tooltips, table cells, and exports will all render in that zone, and `DATE_TRUNC`/EXTRACT intervals will bucket on its calendar boundaries.
...
Filters on a `convert_timezone: false` column still cut at midnight in the project timezone so every dimension on a row agrees on whether it matches. If you need filter cutoffs in the column's displayed zone, do the conversion in dbt and use `type: date` for day-level granularity.
</Warning>
...
| Single column should keep raw warehouse value | Set `convert_timezone: false` on the dimension | Filters still cut at project-timezone midnights |
