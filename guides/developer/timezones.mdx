...
<Info>
  **Experimental** - requires `EnableTimezoneSupport`. The setting only appears in warehouse advanced settings once the feature is enabled.
</Info>
...
| Timestamps stored in a local timezone (NTZ) | Set the data timezone on the warehouse connection | Experimental |
