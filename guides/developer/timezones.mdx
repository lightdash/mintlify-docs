...
| -------------------------- | ----------- | ---------------------------------------------------------------------------- | ---------------------------------------- |
| **Project query timezone** | Application | "What timezone should my users see data in?"                                 | Project settings → Timezone              |
| **Data timezone**          | Warehouse   | "What timezone are my timestamps without timezone metadata (NTZ) stored in?" | Warehouse connection → Advanced settings |

<Info>
  Several features on this page are [Experimental](/references/workspace/feature-maturity-levels) and need `EnableTimezoneSupport` enabled for your organization (contact support to turn it on). Each section is marked accordingly. Without the setting, the project query timezone only adjusts relative date filter boundaries - everything else stays in UTC.
</Info>
...
- **Display** - table cells, chart axes, tooltips, and CSV/Excel/Google Sheets exports render timestamps in the project query timezone. The resolved zone appears as a badge in the Explore view (when viewing or editing a saved chart).
...
Individual dimensions can opt out of project query timezone formatting with `convert_timezone: false` ([Experimental](/references/workspace/feature-maturity-levels)). See [Per-dimension timezone opt-out](#per-dimension-timezone-opt-out) below.
...
The **data timezone** answers a different question than the project query timezone:

> "What timezone are my NTZ (no-timezone) timestamps actually stored in?"

Set it under **Warehouse connection → Advanced settings → Data timezone**. Lightdash then issues a session command on every query so the warehouse interprets ambiguous NTZ values in that zone before they're compared to filter literals or grouped.
