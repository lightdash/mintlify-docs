...
#### Preview the data timezone

Use the **Preview** button next to the data timezone field to verify the setting before saving. The preview runs a trivial query against the live warehouse - it does not read any of your tables - and walks through the current moment as a three-step journey:

1. The raw value the warehouse returns, before any timezone is assumed.
2. How that value is interpreted under the selected data timezone.
3. How it renders in the project query timezone.

Timestamps that already store a timezone (TZ-aware columns) are unaffected by the setting and are shown as a closing contrast note. The preview works in both the create and edit connection flows; on edit, the stored connection's credentials are reused server side so nothing extra is sent from the browser.
