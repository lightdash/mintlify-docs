...
You can override the query timezone for an individual chart in the Explore view. Open the **Run query settings** popover (next to the row limit on the Run button) and pick a value from the **Timezone** dropdown. The override is saved with the chart and takes the highest priority in the [resolution hierarchy](#how-the-project-query-timezone-is-resolved).

The dropdown offers three kinds of values:

- **Project timezone** - inherit whatever the project query timezone is set to. The dropdown shows the resolved zone next to the label so you can see what it currently resolves to.
- **User timezone** - each viewer sees the chart in their own [user timezone](#user-timezone), falling back to the project timezone if they haven't set one. Only available when user timezones are enabled for your organization.
- **A specific IANA zone** - pin the chart to a fixed zone (for example `America/New_York`). The list shows the UTC offset for each zone and tags your browser's local zone with **Local**.
