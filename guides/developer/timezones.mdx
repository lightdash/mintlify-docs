...
Use the **Preview** button next to the data timezone field to check the setting before saving. The preview queries the warehouse - not your tables - and shows the current moment in three steps:
...
Timestamps that already store a timezone (TZ-aware columns) are unaffected by the setting. Preview works in both the create and edit connection flows.
