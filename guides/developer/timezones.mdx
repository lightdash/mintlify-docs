...
### How the query timezone is resolved

Lightdash picks the active query timezone using the hierarchy below (highest priority wins):

1. **Session override** - a per-session timezone supplied by the host, e.g. a `?timezone=` parameter on an embed URL
2. **Saved chart timezone setting** - one of three values stored on the chart:
   - a specific IANA zone (e.g. `America/New_York`) - the chart is pinned to that zone for every viewer
   - `user_timezone` - the chart resolves to each viewer's profile timezone (falls back to the project timezone if the user has none set, or if the user timezone feature is disabled)
   - `project_timezone` *(default for new and unconfigured charts)* - the chart resolves to the project query timezone for every viewer, and tracks later changes to it
3. **Project query timezone** - set by an admin in project settings, and the value the `project_timezone` setting above resolves to
4. **`LIGHTDASH_QUERY_TIMEZONE`** environment variable - server-wide fallback
5. **UTC** - default if nothing else is configured

<Note>
  Saved charts default to **`project_timezone`** so every viewer sees the same numbers, and the chart automatically picks up later changes to the project query timezone. If you want viewers in different timezones to each see their own day boundaries, set the chart to `user_timezone` explicitly using the [per-chart timezone picker](#per-chart-timezone-picker).
</Note>
...
When building or editing a chart in the Explore view, use the timezone picker to choose how the chart resolves its timezone:

- **Project timezone** *(default)* - the chart uses the project query timezone for every viewer. Saved charts use this setting by default, so the numbers are deterministic across your audience.
- **Viewer's timezone** - the chart re-resolves to each viewer's profile timezone (or the project timezone if they have none). Use this for personal exploration or internal dashboards where each viewer should see their own day boundaries.
- **A specific timezone** - pin the chart to a fixed IANA zone (e.g. `America/New_York`). The chart renders in that zone regardless of viewer or project changes.

The setting is stored on the chart, so it applies to every viewer the same way until you change it.
...
Logged-in users can set a personal default timezone in **Settings > Profile > Default timezone**. This preference only applies to a query when the chart's timezone setting is **Viewer's timezone** - by default, saved charts are pinned to the project query timezone for everyone (see the [resolution hierarchy](#how-the-query-timezone-is-resolved)).

When the chart is set to **Viewer's timezone**:

- The user's preference wins over the project default.
...
- **Charts pinned to the project timezone or a specific IANA zone** - those resolve the same way for every viewer regardless of their profile setting.
- **Anonymous viewers** - embed and JWT sessions don't have a profile, so even charts set to **Viewer's timezone** fall back to the project query timezone.

<Note>
  Scheduled deliveries run the chart as saved, so the chart's timezone setting determines how dates are interpreted. The delivery's own timezone setting only controls **when** the delivery runs.
</Note>
...
- **Set your user timezone:** Set a default timezone on your profile, then set the chart to **Viewer's timezone** so it resolves to your profile zone. See [User timezone](#user-timezone).
...
| | Set a user timezone in Profile settings and set the chart to **Viewer's timezone** | Beta; doesn't apply to embeds |
