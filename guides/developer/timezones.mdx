...
<Warning>
With `EnableTimezoneSupport` enabled, the project query timezone will shift this column again on top of your `convert_timezone()` call. Set [`convert_timezone: false`](#per-dimension-timezone-opt-out) on the additional dimension to opt out. Without `EnableTimezoneSupport`, no opt-out is needed.
