...
- **Date filter inputs** - the datetime picker in absolute date filters can be set to display and interpret values in the project query timezone, so users in different browser timezones agree on what a typed wall-clock value means. See [Date filter inputs in project timezone](#date-filter-inputs-in-project-timezone) below.
...
<Info>
  **Experimental** - requires `EnableTimezoneSupport` and a project query timezone. The toggle is disabled until a project query timezone is set.
</Info>
