...
1. **Embed session timezone** - set by the embedding host via the `?timezone=` URL param (embed sessions only)
2. **Per-chart timezone** - set on an individual chart in the Explore view
3. **User timezone** - set by a logged-in user in their profile
4. **Project query timezone** - set by an admin in project settings
5. **`LIGHTDASH_QUERY_TIMEZONE`** environment variable - server-wide fallback
6. **UTC** - default if nothing else is configured
...
- **Anonymous viewers** - embed and JWT sessions don't have a profile, so they use the project query timezone unless the embedding host sets a [per-session timezone](#embed-session-timezone) on the embed URL.
...
### Embed session timezone

<Info>
  **Beta:** The embed session timezone is currently in the [Beta](/references/workspace/feature-maturity-levels) phase. Contact support to enable it for your organization.
</Info>

Embedded sessions have no logged-in user, so they can't use the [user timezone](#user-timezone) or [per-chart timezone picker](#per-chart-timezone-picker). Instead, the embedding host sets the timezone per session by adding a `?timezone=` query parameter to the [iframe embed](/references/iframe-embedding) URL, alongside the [theming parameters](/references/iframe-embedding#theming-url-parameters):

```
https://app.lightdash.cloud/embed/{projectUuid}?timezone=America%2FLos_Angeles#{jwtToken}
```

The value must be a valid IANA timezone name (e.g. `America/Los_Angeles`). When set, it takes the highest priority in the [resolution hierarchy](#how-the-project-query-timezone-is-resolved), overriding the chart pin and project query timezone for every query in that session - dashboard tiles, inherited totals and subtotals, and filter autocomplete.

- URL-encode the value so the `/` is escaped (`America%2FLos_Angeles`); the unencoded form (`America/Los_Angeles`) is also accepted.
- Only the iframe embed reads this parameter. The [React SDK](/references/react-sdk) does not.
- Without the parameter, embed sessions fall back to the project query timezone.
- An invalid timezone value returns a 400 error.
- The parameter only takes effect when Beta timezone support is enabled. With support off, it is ignored and the session uses the project query timezone.
...
| | Set an embed session timezone via the `?timezone=` URL param | Beta; embed sessions only |
