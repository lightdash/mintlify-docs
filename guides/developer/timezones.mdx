...
1. **Embed `?timezone=` URL param** - set per-session on a direct embed or shareable URL (see [iframe embedding](/references/iframe-embedding#timezone))
...
- **Anonymous viewers** - embed and JWT sessions don't have a profile, so they fall back to the project query timezone unless the embed URL sets one explicitly with the [`?timezone=` param](/references/iframe-embedding#timezone).
...
- **`DATE_TRUNC` grouping** - day, week, month, quarter, and year buckets align to the project query timezone calendar. Day-or-coarser grains return a real `DATE` calendar value (e.g. `2026-01-15`), so table cells, exports, and chart axes show the exact bucket date with no off-by-one in negative-offset timezones. Sub-day grains (hour, minute, second) continue to return a timestamp.
...
<Warning>
  **Breaking change for raw SQL table calculations on day-or-coarser time intervals.** With Beta timezone support enabled, day, week, month, quarter, and year time interval dimensions compile to a real `DATE` (not a `TIMESTAMP`). [Raw SQL table calculations](/guides/table-calculations) that apply timestamp-specific operations to these dimensions - for example, `EXTRACT(HOUR FROM ${order_date_day})`, `${order_date_day} + interval '6 hours'`, `${order_date_day}::timestamp`, or `TIMESTAMP_TRUNC(${order_date_day}, HOUR)` - will error on BigQuery and Snowflake, and silently treat the value as a `TIMESTAMP` on Postgres and Redshift.

  [Formula table calculations](/guides/formula-table-calculations) and [SQL template table calculations](/guides/table-calculations/sql-templates) are unaffected. If you need timestamp-specific math on a truncated date, reference a sub-day grain dimension (e.g. `_hour`) or the base timestamp dimension instead.
</Warning>
