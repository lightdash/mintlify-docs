...
<Info>
URL overrides are ignored when they target a [locked dashboard filter](/guides/limiting-data-using-filters#locking-dashboard-filters). The dashboard's saved locked value is used instead, and viewers see a one-time toast explaining that the override was dropped.
</Info>
