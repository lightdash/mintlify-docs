...
<Frame>
  <img src="/images/guides/custom-fields/custom-metric-deleting.png"/>
</Frame>
...
<Frame>
  <img src="/images/guides/custom-fields/custom-metric-add-filter-while-edting.png"/>
</Frame>
...
<Frame>
  <img src="/images/guides/custom-fields/custom-dimension-sql-add.png"/>
</Frame>
...
#### Generate custom SQL dimensions with AI

The create and edit forms for a custom SQL dimension include an AI generate control. Describe the dimension you want in plain English and Lightdash drafts the SQL, a suggested label, and the inferred result type (`number`, `string`, `boolean`, `date`, …) for you. Everything the assistant produces is a starting point — the SQL, label, and type are all still editable before you save.

**When to use it**

- You know the calculation you want but don't want to hand-write the `CASE` / arithmetic / string logic.
- You want a quick first draft to iterate on rather than starting from a blank editor.
- You want Lightdash to infer the right result type instead of picking it manually.

**How it works**

1. Open the custom SQL dimension form (either **Add custom dimension → Custom SQL** or **Edit** on an existing one).
2. In the AI prompt input, describe the dimension — for example, `revenue per order`.
3. Type `@` inside the prompt to insert a field mention. The picker lists every valid dimension in the current explore (including joined tables); pick one and it will be resolved into the correct `${table.field}` reference in the generated SQL.
4. Run the generation. Lightdash returns SQL written for your project's warehouse dialect, a suggested **Label**, and a **Result type**.
5. Edit any of the three fields as needed, then save the dimension.

<Info>
AI custom-dimension generations count toward your organization's AI usage, in the same bucket as other AI-assisted features (agents, table calculation generation, data apps, etc.).
</Info>
