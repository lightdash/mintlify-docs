...
<Tip>
  **Hosted by Lightdash and using GitHub?** You can ask your AI agent (Claude, Cursor, Codex, etc.) to set up the preview deploy GitHub Actions for you if you don't have them already. Point it at the [`lightdash/cli-actions`](https://github.com/lightdash/cli-actions) repo and ask it to add the `start-preview.yml` and `close-preview.yml` workflows to your repository, then add the secrets listed above.
</Tip>
