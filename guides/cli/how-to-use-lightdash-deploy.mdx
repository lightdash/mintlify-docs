...
Once you’ve made updates to your dbt project and you’re ready to make those changes available in your Lightdash production project, you have a few options for deploying from the command line or the UI. This guide will walk you through how it works, when to use each option, and what to keep in mind for a safe and scalable workflow.

#Before you deploy: A quick word of caution

<Warning>
Running `lightdash deploy` pulls from your local dbt profile (whatever is set in your `profiles.yml`). That means the deployment will use whatever database target or warehouse credentials you've currently configured locally which, depending on your setup, might not be pointing to production. This approach works, but it doesn't scale well if you're using multiple environments or have a team of developers.
<Warning>
...
Check out our [**guide to setting up CI/CD**](guides/cli/automate-cli-workflow)
...
* Push your dbt project to the Lightdash project you've authenticated with via the CLI
...
Go to your project → "Manage project" → Refresh dbt. Same result as running `lightdash refresh`.