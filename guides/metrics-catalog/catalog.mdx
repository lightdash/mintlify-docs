...
## Setting a default segment and filter for a metric

You can pre-select a segment-by dimension and a filter for a metric so the Metrics Explorer opens with a curated view instead of a single uncut line. Use this when there is one obvious "first question" for a metric — for example, splitting `unique_order_count` by `order_source` and limiting it to completed orders.

Set these on the metric's `spotlight` block:

- `default_segment`: dimension name (or `table.dimension` for joined fields) that the Metrics Explorer pre-selects in **Segment by**.
- `default_filter`: a map of `{ dimension: value }` or `{ dimension: [value1, value2] }` that the Metrics Explorer applies as the initial filter rule.

Both fields must reference dimensions that are valid for the Metrics Explorer (segmentable / filterable types) and must already be allowed by the metric's `segment_by` and `filter_by` allowlists when those are set. Invalid references fail at compile time.

Users can clear or change either default at any time in the explorer — they only seed the initial view.
...
`default_filter` accepts the same shorthand as model-level default filters — a single value, an array of values, or an operator object (for example `{ amount: { ">=": 100 } }`).
