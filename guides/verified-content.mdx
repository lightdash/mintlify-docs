...
A typical workflow for promoting verified content from one project to another:

1. Run `lightdash download` from a project where the content is already verified
2. Inspect the YAML — verified items will have `verified: true`
3. Copy or move the files into another project's `lightdash/` directory
4. Run `lightdash upload --force` as an admin user to push the content (with verification) to the destination project

To unverify content via the CLI, edit `verified: true` to `verified: false` in the YAML and re-upload.
