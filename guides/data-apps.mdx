...
Next to the send button in the chat input, you can pick which Claude model builds the next version:

- **Sonnet** (default) - balanced quality and speed. Good fit for most apps and iterations.
- **Opus** - highest quality. Use it for complex apps, tricky layouts, or prompts where Sonnet is missing the mark.
- **Haiku** - fastest. Use it for small tweaks where you want a quick turnaround.

You can change the model on every prompt - including mid-iteration. Switching models does not reset the agent's memory of what it already built, so you can start a draft on Sonnet, escalate to Opus for a hard refactor, then drop back to Haiku for small polish passes. Each version records the model that built it, so older versions keep working even if you switch later.
