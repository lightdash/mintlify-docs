...
By default, the preview pane shows the latest ready version of your app and auto-updates each time a new iteration finishes building. You can pin the preview to any earlier version to compare results or revisit something you liked.

Each assistant message in the chat shows a **v{n}** badge next to a kebab menu (`⋯`). Open the menu and choose **Preview this version** to pin the preview to that version. The active version's badge turns indigo with an eye icon so it's clear which one you're viewing.

When you're pinned to an older version, the preview header shows an orange **Viewing v{n}** badge next to the refresh button. Click the badge to clear the pin and return to the latest version.

A few things to know:

- Pinning is preview-only. It does not change which version is served on the app's public URL, and it does not modify version history.
- The pin clears automatically when a new version finishes building, so a fresh prompt you just authored is never hidden behind an older pinned version.
- The pin also clears when you switch to a different app.
