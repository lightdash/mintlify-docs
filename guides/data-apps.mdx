...
Custom dependencies are gated by an instance-wide flag, so first check with your admin that they're enabled on your Lightdash instance (see [Self-hosting Data apps](/guides/data-apps/self-hosting#custom-dependencies)). Uploads that declare dependencies while the flag is off are rejected with a clear error.
