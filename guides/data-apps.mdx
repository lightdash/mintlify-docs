...
## Iterating on production apps in a preview project

When you create a [preview project](/guides/developer/preview-projects), every data app in the upstream production project is copied into the preview alongside your charts and dashboards. This includes both standalone apps and apps embedded as tiles on a dashboard. Each preview copy:

- Starts at version 1, seeded from the production app's latest ready version, so you can open it and iterate immediately - no rebuild is needed.
- Lands in the preview's mirror of the source app's space. Personal apps stay personal and are owned by their original creator.
- Is linked back to the production app it came from. When you [promote](#promoting-an-app) the preview copy later, the original production app is updated with the new version instead of a duplicate being created.

This means you can pull a preview, change a metric or model in dbt, iterate on a production data app against the new semantic layer, and promote the result back to production - all without touching production until you're ready.

Apps without a ready version in production are skipped (there is nothing to copy yet). If a per-app copy fails, the rest of the preview is created normally and the failing app is omitted from the preview - it can be created fresh in the preview if you need it.
