...
- **Hover a data point** in the preview to highlight the query row it came from in the inspector. Use it when a number looks off and you want to jump straight to the query definition and result set behind it.
...
You'll find the same **Refresh** action in the **⋯** menu on the [standalone viewer page](#shareable-viewer-url) (the full-screen `/view` URL you get from **Continue building** or from a shared link).
...
Every data app has a canonical viewer URL that renders the app full-screen, without the builder chat panel. This is the link you share with teammates, drop into a Slack message, paste into a Notion page, or receive from a scheduled delivery.

- **Latest ready version** — `/projects/:projectUuid/apps/:appUuid/view`
- **Pinned to a specific version** — `/projects/:projectUuid/apps/:appUuid/versions/:version/view`

For example, an app called `orders-explorer` served at its latest ready version looks like:

```
https://app.lightdash.cloud/projects/8f2b1c4d-1111-2222-3333-444455556666/apps/orders-explorer-uuid/view
```

The viewer link is what Lightdash uses everywhere the app is surfaced outside the builder: the spaces and content browser, the **Browse** menu, search, the schedulers list, **Settings → My apps**, the **Continue building** shortcut, and the URLs embedded in scheduled-delivery emails and Slack messages. Whoever opens the link is authenticated as themselves and their [project role and space access](#project-role) still apply — the URL is a pointer, not a bypass.

<Info>
  Older `.../preview` links (from bookmarks, previously delivered emails, or shared messages) still work. Lightdash redirects them to the equivalent `.../view` URL and preserves the query string and hash, so scheduled-delivery links that carry a `?schedulerUuid=` marker keep working unchanged.
</Info>
