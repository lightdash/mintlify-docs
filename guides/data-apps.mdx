...
Use `lightdash download --apps <appUuids...>` to fetch specific apps by UUID, or `--include-apps` to grab every app in a space (capped at the first 10):

```bash
# Download specific apps by UUID (works for apps not in a space)
lightdash download --apps 8f2b1c4d-1111-2222-3333-444455556666

# Download every data app in a space in the project (up to 10)
lightdash download --include-apps
```

To reach apps that are not in a space, or to download more than 10 in one go, you must list them explicitly with `--apps`.
...
2. Optionally run `pnpm install && pnpm build` inside the app folder to check that the source compiles. This is a **local pre-check only** — the server does its own build on upload.
...
In non-interactive shells (CI, scripts) the CLI aborts instead of prompting. Pass `--create-new` to opt in explicitly:

```bash
lightdash upload --apps --project <target-project-uuid> --create-new
```

The destination project creates a new app (with a new UUID on the server) and builds it in its own sandbox. The `appUuid` in your local manifest is unchanged; the next `lightdash download` from the source project still finds the original app. Pass `--create-new` any time you want to fork an app into a new record, even inside the same project.
