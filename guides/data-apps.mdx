...
<Info>
  Data apps are an enterprise feature. [View pricing](https://www.lightdash.com/pricing)
</Info>
...
You create data apps from the **Data apps** section of your project. Click **New app** to open the creation flow.

### Choosing a template

Every new app starts from a template. The template you pick tells the agent what kind of layout you have in mind:
...
- **Dashboards** — attach a dashboard to pull in every chart on it at once, along with the dashboard's filters.
...
Every prompt you send creates a new version of the app. The agent keeps its working state between versions, so follow-up prompts like "make the header smaller" or "swap the bar chart for a line chart" land as targeted changes instead of full rewrites. You can roll back to any earlier version from the version history panel.

If a build is taking too long or going in the wrong direction, you can cancel it and try a different prompt.

### Query inspector

Each app version comes with a query inspector. Open it to see every query the running app is making — the underlying metric query, the fields it's pulling, and the results it gets back. Use it to verify the app is asking for the right data, debug unexpected numbers, or sanity-check performance.
...
Newly created apps are personal — only you can see them. To share an app with your team, move it into a space.
...
- **Don't expect one prompt to be enough.** Most apps get substantially better after a few rounds of iteration. Start with the rough shape, then refine.
...
- **Keep prompts focused.** Smaller, targeted iterations ("change the KPI to revenue") tend to land better than sweeping rewrites in a single prompt.
