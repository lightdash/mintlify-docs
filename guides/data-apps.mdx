...
### Linking to dashboards and URLs

Cards and elements in a data app can navigate the viewer to another Lightdash dashboard or to an external URL. Ask the agent to add the link in plain English — for example:

- "Make each KPI card link to the Revenue dashboard."
- "When I click a row in the customers table, open that customer's profile in a new tab."

Links opened in the same tab navigate the full Lightdash page, so the destination dashboard loads with the viewer's existing session and permissions. Links opened in a new tab escape the app's sandbox and behave like any normal external link. Navigation only happens in response to a real click, so app code can't redirect the page on its own.
