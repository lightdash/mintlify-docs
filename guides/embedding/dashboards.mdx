...
<Info>
If a saved dashboard filter is [locked](/guides/limiting-data-using-filters#locking-saved-dashboard-filters), any `externalFilters` in the embed JWT that target the locked field are ignored and the saved value is used in queries. Lock filters when you want to guarantee viewers always see data scoped to a specific value (for example, a tenant ID or region) regardless of what the embedding application passes in.
</Info>
