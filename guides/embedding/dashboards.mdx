...
By default, the embedded dashboard's tabs and filter bar scroll away with the rest of the page. Set `stickyHeader: true` to pin them to the top of the embed scroll container so they stay visible while users scroll through tiles — matching the behavior of the regular dashboard view in Lightdash.
...
Existing embeds keep the previous scroll-away behavior unless you opt in. You can also toggle this from the **Appearance** section of the embed preview form when previewing a dashboard in the embed settings.
...
const token = jwt.sign({
  content: {
    ...
    // Pin tabs and filter bar while scrolling
    stickyHeader: true,
  },
