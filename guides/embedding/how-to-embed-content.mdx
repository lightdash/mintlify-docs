...
**[iframe embedding](/references/iframe-embedding)** is the simpler option. You generate a JWT, construct a URL, drop it in an `<iframe>` tag — no npm installs, no CORS config, works anywhere (React, Vue, plain HTML). The catch: **dashboards only**. You can't embed individual charts this way.
