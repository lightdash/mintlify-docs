...
{/* TODO(screenshot): custom SQL editor autocomplete showing `${ld.parameters.date_zoom}` listed as a "System variable". Save to `/images/guides/date-zoom/date-zoom-reserved-parameter.png` and replace this comment with a `<Frame>` wrapping the image. */}
