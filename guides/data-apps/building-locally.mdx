...
- The [Lightdash CLI](/references/lightdash-cli) installed and logged in (`lightdash login <your-instance-url>`). You need a version with slug-based app identity and the `apps create` / `apps preview` commands — run `lightdash --version` and upgrade if you're on an older release.
