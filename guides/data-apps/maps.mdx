...
Data apps run in a locked-down iframe: the app can't call external hosts directly, and it can't load map tiles from services like OpenStreetMap or Mapbox. That sounds limiting — but it makes one approach shine. SVG maps drawn with `d3-geo` (already available in every data app, no extra packages) work beautifully: country shapes come in as GeoJSON through an [external connection](/guides/data-apps/external-connections), and d3 projects them into anything from a flat world map to a draggable, auto-rotating globe.

This tutorial builds a "World Atlas" globe: countries colored by a switchable metric, drag to rotate, click a country for details — and then joins live Lightdash data onto the map.

<Frame>
  <img src="/images/guides/data-apps/maps-globe-hero.png" alt="A data app showing an interactive d3 globe with countries colored by population" />
</Frame>
...
<Frame>
  <img src="/images/guides/data-apps/maps-globe-country-panel.png" alt="Clicking a country shows its details and a live 7-day weather forecast" />
</Frame>
...
<Tip>
  **Join on `ADM0_A3`, not `ISO_A3`.** In the Natural Earth 110m file, `ISO_A3` is the placeholder `-99` for several countries — including France and Norway — while `ADM0_A3` is populated for every feature. If your warehouse stores ISO alpha-2 codes (`FR`, `NO`), either map them to alpha-3 in the app or expose an alpha-3 dimension in your model.
</Tip>
