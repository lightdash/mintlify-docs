...
## Open a pull request from your active changeset

If you've already staged semantic-layer edits in Lightdash as a changeset, you can ask the agent to open a pull request directly from those changes instead of describing them again in chat.

Phrase the request around your changeset, for example:

```text
Create a PR from my changesets.
```

```text
Write back my active changeset.
```

When the agent recognizes this intent, it skips composing free-text instructions and reads your project's active changeset on the server. The writeback instructions are built deterministically from the staged structured changes, so the resulting pull request matches what you've already reviewed in Lightdash.

Use this when:

- You've made and reviewed changes in Lightdash that are staged in a changeset.
- You want the pull request to reflect those changes exactly, with no re-interpretation by the agent.

For ad-hoc edits that aren't staged in a changeset, keep describing the change in chat as usual — the agent will compose the writeback prompt itself.
