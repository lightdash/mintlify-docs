...
## Writing back a project's staged changeset

If your project has staged changes — for example, edits saved from the Custom Metric or SQL Runner write-back flows that haven't been pushed yet — you can ask the agent to open a single pull request that contains all of them.

Ask the agent to write back the changeset:

```text
Create a pull request from my changesets.
```

```text
Open a PR with the changes staged on this project.
```

The agent reads the project's active changeset, builds the writeback instructions from the structured changes, and opens one pull request that applies every staged change. You don't need to describe the edits — the agent uses what's already staged.

If the project has no staged changes, the agent will tell you the changeset is empty and won't open a pull request.
