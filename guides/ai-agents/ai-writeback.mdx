...
## Reading the dbt repository (Beta)

<Warning>
  Read-only repo access is gated by a separate **repo file system** feature flag. Contact Lightdash support to enable it for your organization.
</Warning>

When the **repo file system** flag is on, the agent can also read source files in your dbt repository to ground its answers and plan a writeback before opening a pull request. Use it when the question is about how a model is built (its SQL, refs, or `dbt_project.yml` config), or to confirm what the file currently contains before asking for an edit.
...
- The **repo file system** feature flag must be enabled for your organization.
...
- The agent can list directories, read files, and search for strings, but it cannot modify anything. To change a file, it still has to call `proposeWriteback`.

In chat, each repo command shows up as its own sub-step (for example, `Read cat models/orders.sql` or `Searched grep -rln "active_users"`) under a terminal icon, so you can see exactly what the agent looked at before answering or proposing a change.
