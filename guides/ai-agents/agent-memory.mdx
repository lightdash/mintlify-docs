...
Cross-user knowledge is handled separately by [Reviews](/guides/ai-agents/reviews) and knowledge documents — memory is the personal layer.
...
## Memory vs. Reviews vs. knowledge documents

Three related surfaces, each with a different job:

- **Memory** (this page) — personal, reactive, automatic. Captures what *you* teach the agent through everyday use, and is used only in your own conversations.
- **[Reviews](/guides/ai-agents/reviews)** — cross-user, curated. Scans agent answers in the background, groups likely-wrong answers by root cause, and proposes one-click fixes to your semantic layer or to a shared `lightdash.project_context.yml` file that every agent reads.
- **[Knowledge documents](/guides/ai-agents/best-practices#knowledge-documents)** — proactive. Upload the context you already know the agent needs, per-agent or org-wide.

Use memory for in-the-moment course corrections that stick, Reviews to improve agents at the source, and knowledge documents when you can hand the agent the answer upfront.
