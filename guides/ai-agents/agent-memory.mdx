...
<Tip>
  Memory is for things the agent picks up over time from your corrections. If you already know what context the agent needs — a glossary, fiscal calendar conventions, definitions of internal terms — it's faster to give it upfront via [knowledge documents](/guides/ai-agents/best-practices#knowledge-documents) than to wait for it to learn through feedback.
</Tip>

<Note>
  **Memory vs. [Reviews](/guides/ai-agents/reviews):** Memory captures corrections per-agent as they happen in chat — it's reactive and lives in the agent's instructions. [Reviews](/guides/ai-agents/reviews) does the boring work for you: it scans agent turns in the background, groups likely-wrong answers by root cause, and proposes one-click fixes to your semantic layer or to a shared [`lightdash.project_context.yml`](/guides/ai-agents/reviews#the-lightdashproject_contextyml-file) file that every agent reads. Use memory for in-the-moment course corrections; use Reviews when you want admins and developers to improve agents at the source.
</Note>
