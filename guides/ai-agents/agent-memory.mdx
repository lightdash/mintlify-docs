...
<Note>
  **Memory vs. [Issues](/guides/ai-agents/issues):** Memory captures corrections per-agent as they happen in chat — it's reactive and lives in the agent's instructions. [Issues](/guides/ai-agents/issues) is a shared board where AI-filed findings and human-filed issues live side by side, each with a one-click fix to your semantic layer or to a shared [`lightdash.project_context.yml`](/guides/ai-agents/issues#the-lightdashproject_contextyml-file) file that every agent reads. Use memory for in-the-moment course corrections; use Issues when you want admins and developers to improve agents at the source.
</Note>
