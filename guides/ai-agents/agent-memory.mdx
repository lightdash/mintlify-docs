...
Memory is controlled at the organization level and requires **Organization Admin** permissions.

1. Go to **Settings → Organization Settings → Ask AI → General**.
2. Under **Review AI agent turns**, toggle **Enable AI agent memories** on.

Once enabled, future agent turns are eligible to produce memories in the background. Disabling the toggle stops memory learning, retrieval, consolidation, and prompt injection, and hides memory-management entry points. Existing memory data is preserved, so re-enabling later resumes use without data loss.

Newly created organizations have memories enabled by default. Existing organizations stay disabled unless an admin turns the toggle on.
