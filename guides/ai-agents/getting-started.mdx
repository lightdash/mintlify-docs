...
### Access control
Choose who can see and use your agent. Access is set per agent in the agent settings, with three options:

- **Everyone in the project** — All project members can see and use the agent. This is the default.
- **Admins & developers only** — The agent is hidden from everyone else. Use this while you're setting up, testing, or iterating on an agent before opening it to the wider team.
- **Specific users & groups** — Only the users and groups you pick can see and use the agent. Admins and developers always have access.

<Tip>
**Admins & developers only** is useful for safely staging an agent. Configure its instructions, tags, and knowledge documents, validate the behavior, then switch the access mode to **Everyone in the project** or **Specific users & groups** when you're ready to roll it out.
</Tip>

For details on how user permissions and attributes affect what an agent can query, see [user attributes and permissions](/guides/ai-agents/data-access#user-attributes-and-permissions).
### Tags (optional)
