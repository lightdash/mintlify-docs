...
### Starting a thread from a dashboard

While viewing a dashboard, look for the small AI agent icon at the rightmost end of the launcher area, next to any active thread pills. Click it to open the new-thread panel with the current dashboard already pinned as context.

If the dashboard has tabs, the tab you're on is pinned too, so the agent knows which slice of the dashboard you were looking at when you asked. This dashboard and tab context is attached automatically to any prompt you send from the launcher, the full-page agent view, or the AI Router — follow-up prompts inherit whichever dashboard and tab you were on when you started.
...
This opens the scheduled delivery form pre-filled to run that conversation's agent over the chart, so you get the same style of answer, on a schedule, without rebuilding anything. See [AI agent](/guides/how-to-create-scheduled-deliveries#ai-agent) for how the generated message works.
