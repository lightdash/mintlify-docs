...
- **Open pull requests against your dbt repository** - Ask the agent to rename a metric, add a dimension, or edit a YAML file and it can open a pull request for review. See [AI writeback](/guides/ai-agents/ai-writeback).
...
You can share a web AI agent thread with a teammate by sending them a link. Open the thread and click the **Share** icon in the header to generate a shareable URL and copy it to your clipboard.

When the recipient opens the link, Lightdash clones a snapshot of the conversation into their own account so they can continue the analysis as their own thread.

A few things to know:

- **Web threads only.** The share action is available on threads started in the Lightdash web app. To share a Slack conversation, share the original Slack thread instead.
- **Snapshot, not live sync.** The link captures the thread at the moment you created it. Messages you send afterward aren't added to the recipient's copy — regenerate the link to share an updated version.
- **Recipient needs agent access.** The link only works for users who already have access to the same AI agent in the project. If they don't, opening the link shows a not-authorized page.
- **Their own copy.** The recipient becomes the author of their cloned thread, so they can send follow-up messages and the original thread stays unchanged.
