...
When you open `/ai-agents`, Lightdash lands you on your default agent if one is set. To use the auto-routing experience instead — which picks an agent based on your prompt — select **Auto** from the agent selector. Auto remains reachable even when a default is set, and the choice persists across refreshes.
