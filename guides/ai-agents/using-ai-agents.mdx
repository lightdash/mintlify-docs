...
This opens the scheduled delivery form pre-filled to run that conversation's agent over the chart, with the originating thread pinned as context, so you get the same style of answer on a schedule without rebuilding anything. Pick the frequency, format, and destinations as you would for any other delivery. The **Schedule delivery** action is hidden inside embedded views and only shown to users who can create scheduled deliveries in the project.

See [AI-enhanced messages](/guides/how-to-create-scheduled-deliveries#ai-enhanced-messages) for how the generated message works, and how to pick between the fast model and an agent.
