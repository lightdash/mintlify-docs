...
- **Per user** _(coming soon)_ — each user will connect their own account so actions are attributed to the individual user and respect their permissions in the external tool. Per-user OAuth is not yet supported.

<Info>
  Today, all MCP server credentials are shared across users of the agent. Per-user OAuth is planned.
</Info>
