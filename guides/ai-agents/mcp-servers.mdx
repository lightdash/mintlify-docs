...
<Steps>
  ...
  <Step title="Add a new server">
    ...
    - **Client ID** and **Client secret** (OAuth only, optional) — provide these if the MCP server you're connecting to doesn't support [Dynamic Client Registration (DCR)](https://datatracker.ietf.org/doc/html/rfc7591). Register an OAuth client with the MCP provider using the callback URL below, then paste the resulting credentials here. Leave both blank to let Lightdash register a client automatically via DCR.
