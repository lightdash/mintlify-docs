...
Each MCP server shows its current runtime state in the servers list so you can spot problems without having to test it from an agent conversation.

| Status | What it means |
| --- | --- |
| **Not connected** | The server has been added but Lightdash has not yet tried to connect. |
| **Connecting** | Lightdash is establishing the connection or completing the auth handshake. |
| **Connected** | The server is reachable and its tools are available to attached agents. |
| **Error** | The last connection attempt failed. The specific error message is shown next to the status. |

When a server is in **Error**, expand the row to see the underlying message (for example, an expired token, an invalid URL, or a TLS failure). Common fixes:

- **Auth errors** — re-enter the bearer token, or click **Reconnect** to re-run the OAuth flow.
- **Network errors** — verify the URL is reachable from your Lightdash instance and that any firewall allow-lists include the MCP endpoint.
- **Protocol errors** — confirm the server implements the remote MCP transport and is on a compatible version.

The status and last error are persisted, so they remain visible after a page refresh or instance restart until the next connection attempt updates them.
