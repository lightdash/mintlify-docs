...
-   **Build from real interactions** - In the AI Agent Admin Panel, filter for thumbs-down threads and add the failed prompts to an evaluation set. Refine the prompt with your expected repsonse until the eval status is `passed`. This helps test fixes and prevent regressions on real-world issues your team actually encounters
