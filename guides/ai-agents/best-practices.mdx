...
For a detailed guide on what makes a good description for models, dimensions, and metrics — with side-by-side examples — see [Writing useful descriptions in your semantic layer](/references/writing-descriptions).
...
<Info>
  Group hints are stored on the group itself, not copied onto each contained field. Field-level hints continue to take precedence when both are declared, and any group hints are appended as additional context. There is no change to how hints are declared or consumed on individual fields.
</Info>
...
<Frame>
  <img src="/images/guides/ai-agents/knowledge-documents.png" alt="Knowledge documents section in agent settings" className="block dark:hidden" />
  <img src="/images/guides/ai-agents/knowledge-documents-dark.png" alt="Knowledge documents section in agent settings" className="hidden dark:block" />
</Frame>
...
## [Using verified answers](/guides/ai-agents/verified-answers)
...
## [Using evaluations](/guides/ai-agents/evaluations)
...
-   **Build from real interactions** - In the [Threads tab](/guides/ai-agents/visibility#thread-activity), filter for thumbs-down threads and add the failed prompts to an evaluation set. Refine the prompt with your expected repsonse until the eval status is `passed`. This helps test fixes and prevent regressions on real-world issues your team actually encounters
