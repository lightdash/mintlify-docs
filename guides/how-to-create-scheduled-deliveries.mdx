...
## Managing your scheduled deliveries

Each scheduled delivery in the list shows a row of action icons you can use to manage it:

- **Send now** (paper plane icon) - Trigger an immediate delivery without waiting for the next scheduled run.
- **View run history** (clock icon) - Open the run history view for this delivery inside the same modal. See [Viewing run history](#viewing-run-history) below.
- **Edit** (pencil icon) - Open the configuration screen to update the delivery.
- **Delete** (trash icon) - Permanently remove the scheduled delivery.

The status badge next to each delivery is also clickable and opens the same run history view.
...
You can drill into a scheduled delivery's run history directly from the chart or dashboard **Scheduled deliveries** modal — no need to open project settings.

Click the clock icon (or the status badge) next to any delivery to swap the modal to the **Run history** view. From there you can:

- Filter runs by status (completed, failed, partial failure, running, scheduled).
- Filter by delivery destination (Slack, email, MS Teams, etc.).
- Sort runs by scheduled time.
- Click any row to open the **Run details** view, which shows the job configuration, individual sub-job results, and error details for failed jobs.

Use the **Back** button at the top of the run history or run details view to return to the list of scheduled deliveries without closing the modal.

<Info>
  Only users with **manage Scheduled Deliveries** permission for a scheduler can view its run history. Interactive viewers can view history for their own deliveries; admins, developers, and editors can view history for all deliveries they have access to.
</Info>
