...
For dashboards that use [tabs](/get-started/exploring-data/dashboards#add-tabs), an **Each tab on its own page (with dashboard and tab title)** checkbox appears when the format is `PDF`, or `Image` with `Also send as PDF` enabled. It's off by default, so existing schedulers keep delivering a single stacked PDF; turn it on when a recipient would rather flip through one page per tab than scroll a long stacked document (for example, an `Orders Overview` dashboard with `Revenue`, `Fulfilment`, and `Returns` tabs becomes a three-page PDF headed `Orders Overview — Revenue`, `Orders Overview — Fulfilment`, `Orders Overview — Returns`).

- **What you get** — one PDF page per selected tab, each page headed by the dashboard name and the tab name. The dashboard name is also set as the PDF's `Title` metadata, so it shows up in the PDF viewer's title bar and in file managers.
- **Where it applies** — the native `PDF` format and the PDF attachment produced alongside an `Image` delivery (`Also send as PDF`). The stacked image itself is unchanged.
- **Empty tabs** — a selected tab with no tiles still gets its own page, showing the tab header and a `Tab is empty` placeholder. It isn't silently dropped.
- **Orphan tiles** — tiles that predate tabs and aren't attached to any tab render on the first eligible page only, so they aren't duplicated across pages.
- **Uniform page size** — all pages are sized to the tallest selected tab. Shorter tabs get extra bottom whitespace so page breaks stay predictable.
- **Untabbed dashboards** — the checkbox is ignored and you get the single stacked render as before.
- **Failure semantics** — if the rendered page count doesn't match the selected tab count, the delivery is marked failed and retried/alerted rather than silently sending a document with the wrong pages. Check the [run details](#run-details) if a delivery fails.
