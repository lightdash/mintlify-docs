...
<Note>
**Required filters and requirement groups block delivery.** If the dashboard has any [required filters or filter requirement groups](/guides/limiting-data-using-filters#required-filters-and-filter-requirement-groups) that the delivery's filters don't satisfy, the **Create** button is disabled with a tooltip explaining what's missing, and **Send once now** is disabled too. In the delivery's `Filters` tab, set a value on each required filter and on at least one filter in each requirement group — otherwise the delivery would send an unfiltered dashboard.
</Note>
