...
To access the overview page, head to your project settings, then to the `Syncs & Scheduled Deliveries` tab.
...
The **All schedulers** tab shows a list of all scheduled deliveries in your project. Each row displays:

- The scheduled delivery name and format (image, CSV, etc.)
- The resource it's attached to (chart or dashboard)
- Delivery destinations (Slack channels, email addresses, etc.)
- The delivery frequency
- The owner who created it
- The most recent delivery status

You can filter the list by format, resource type, owner, or destination to quickly find specific scheduled deliveries.

### Run history

The **Run history** tab shows a history of all scheduled delivery runs in your project. Each run displays a clear status indicator:

- **Completed successfully**: All jobs in the run completed without errors
- **Failed**: The run failed to complete
- **Partial failure**: Some jobs succeeded while others failed
- **Running**: The run is currently in progress
- **Scheduled**: The run is scheduled but hasn't started yet

Click on any run to open the **Run details** modal, which shows:

- The job configuration (delivery target and schedule)
- A list of all sub-jobs with their individual status, timing, and any error details

This makes it easy to diagnose issues when a scheduled delivery fails or partially fails.

### Reassigning scheduled delivery owners

You can transfer ownership of scheduled deliveries to another user in your organization. This is useful when a team member leaves or when you need to reorganize who manages specific deliveries.

To reassign a single scheduled delivery:

1. In the **All schedulers** tab, click the three-dot menu next to the scheduled delivery
2. Select **Reassign owner**
3. Search for and select the new owner
4. Click **Reassign**

To reassign multiple scheduled deliveries at once:

1. In the **All schedulers** tab, select the checkboxes next to the scheduled deliveries you want to reassign
2. Click the **Reassign owner** button that appears in the toolbar
3. Search for and select the new owner
4. Click **Reassign**
