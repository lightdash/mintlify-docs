...
You'll get a modal with a searchable sidebar of existing scheduled deliveries for your saved chart or dashboard, and the option to start a new one.

<Frame>
  ![](/images/guides/how-to-create-scheduled-deliveries/scheduled-delivery-empty-state.png)
</Frame>

Click `New delivery` to open the scheduled delivery editor. The editor uses a three-pane layout:

- **Section nav (left)** — sections are grouped under **Delivery** (when and where it's sent) and **Content** (what's inside it and how it's written).
- **Section form (middle)** — the fields for the section you selected.
- **Preview rail (right)** — a live delivery card that reflects the format you picked (attachment chips for CSV/XLSX, a dashboard bento placeholder or a real screenshot for image and PDF, plus an AI summary block when AI is on), followed by the **next three run times** computed from your cron and time zone.
...
Under `Content > Data & format`, choose how the content is delivered — `Image`, `CSV`, `.xlsx`, or `PDF` — and, for dashboards, adjust the [saved filters](/guides/limiting-data-using-filters#adding-saved-filters-to-your-dashboard) so different recipients can get a different cut of the same dashboard without you having to duplicate it. The preview rail updates to match: image and PDF show a real screenshot preview, CSV and XLSX show an attachment chip. Row and cell limits for CSV/XLSX exports are covered in [Export limits](/references/workspace/export-limits).
...
The scheduled deliveries modal opens on a master–detail list view. The **sidebar** on the left is a searchable list of every delivery on the chart or dashboard; the **detail pane** on the right shows the delivery you've selected, with:

- **Status and format pills** — active/inactive and the delivery format (Image, CSV, XLSX, PDF).
- **Next runs** — the next few times the delivery will fire, computed from its cron and time zone.
- **Recipient chips** — one chip per destination, with the channel icon for email, Slack (channel names resolved from your workspace), Microsoft Teams, and Google Chat.
- A pinned **footer** with `Send now` (run it immediately), `Edit` (jump into the editor for that delivery), and `Delete`.

You can also toggle a delivery active or inactive from the detail pane, and open its run history to see previous sends and any errors.
...
Alerts use the same editor as scheduled deliveries, so everything above — the three-pane layout, the preview rail, the next-runs list — works the same way. The section nav adapts:

- **Delivery > Alert conditions** replaces `Data & format` and `Message`, and is where you pick the metric to watch, the threshold operator (`is greater than`, `is less than`, `increased by`, `decreased by`), and the threshold value. On charts you can also apply chart filters and parameters here so the alert only fires on the slice of data you care about.
- **Delivery > Setup** is where you set how often the alert checks the data, its time zone, and its recipients — the same as a scheduled delivery.

The preview rail switches to an alert card that shows the threshold you configured and when it will next be evaluated.
