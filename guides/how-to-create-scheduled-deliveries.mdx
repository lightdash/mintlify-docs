...
<Note>
If the dashboard has [required filters or filter requirement groups](/guides/limiting-data-using-filters#required-filters-and-filter-requirement-groups), you can't save or send the delivery until its `Filters` tab satisfies every rule.
</Note>
...
<Info>
**Delivering a [data app](/guides/data-apps)?** Scheduling, recipients, and message options are the same, but what lands in the delivery isn't: an app doesn't declare its queries, so Lightdash captures every query the app runs and sends one file per query. Available formats, row limits, and how you pin a filtered cut all differ — see [Scheduled deliveries and syncs for data apps](/guides/data-apps/deliveries-and-syncs).
</Info>
