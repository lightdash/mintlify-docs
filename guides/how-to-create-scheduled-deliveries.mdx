...
- `.xlsx` sends a downloadable Excel file of the results table from your chart. For dashboards, you can choose how the tiles are packaged using the **XLSX layout** option:
  - `Separate files (ZIP)` (default) puts each dashboard tile in its own XLSX file inside a ZIP archive.
  - `Single workbook` puts every dashboard tile on its own sheet inside one XLSX file.

  The same `values`, `limit`, and row options described for CSV apply to XLSX. The CSV/Excel cell limit is shared — see [Export limits](/references/workspace/export-limits) for details.
...
<Info>Filters are only available for Dashboard scheduled deliveries.</Info>
...
3. Edit the **Instructions** to tell Lightdash what to focus on — for example, *"Call out any category whose weekly revenue changed by more than 15% vs. the prior week."*
4. Optionally, choose an **AI agent** to write the message.

### Fast model vs. AI agent

The toggle enables the **fast model** by default. Picking an agent from the `AI agent (optional)` dropdown switches to **agent** mode. You can go back to the fast model at any time by clearing the selection.

| | Fast model (default) | AI agent |
| --- | --- | --- |
| **What it does** | Single-shot summary written by Lightdash's built-in fast model over the data being delivered. | Runs one of your workspace's [AI agents](/guides/ai-agents) in a fresh thread, with the chart or dashboard as context. |
| **How it queries data** | Re-queries the chart or dashboard with the scheduler's filter and parameter overrides, so the summary matches exactly what is delivered — including for image and PDF formats. | Uses the agent's own tools, so it can pull related data, apply the agent's [instructions](/guides/ai-agents/using-ai-agents) and [content](/guides/ai-agents/content-tools), and follow the agent's [data access rules](/guides/ai-agents/data-access). |
| **When to use it** | You want a quick summary that reflects only the delivered numbers, or the delivery format is a static image or PDF. | You want the message to reason beyond the chart — for example, comparing against a related metric, or in the same voice as an existing agent used by your team. |

An example prompt that works well for both modes on a weekly revenue dashboard for a fictional ecommerce store:

> *"Summarize this week's revenue in two sentences. Highlight the top-performing product category and any category with a week-over-week drop."*

### Turn a conversation into a delivery

You can also start an AI-augmented delivery from an existing AI agent conversation. On any saved chart the agent has produced, open the chart's `⋯` **Quick actions** menu and choose **Schedule delivery**. The scheduled delivery form opens pre-filled to run that conversation's agent, with the originating thread pinned as context so the agent picks up where the conversation left off.

The **Schedule delivery** action is only visible to users who can [create scheduled deliveries](/references/workspace/roles) in the project, and is hidden inside embedded views.

### Fallback message

The static **Delivery message body** you write acts as a fallback. If AI generation fails on a given send, Lightdash still delivers your chart or dashboard with that message and marks the run as a **partial failure** — so a bad AI run never blocks your report.

### Limitations and behavior

- **Google Sheets deliveries skip AI augmentation.** The delivery still runs; only the AI-written message is skipped.
- **Google Chat and Microsoft Teams** support AI-enhanced messages as an inline block on the delivery.
- **Permissions follow the delivery's creator.** The AI run — whether fast model or agent — executes with the permissions of the user who created the scheduled delivery, so their agent access, project role, and any [user attributes](/references/workspace/user-attributes) apply. Editing a scheduled delivery that another user created still works normally; it doesn't transfer ownership, and it doesn't lock you out.
- **Send now** always uses your own identity as the delivery's creator for that ad-hoc run. If you're scripting against the Lightdash API to trigger a send, the AI run will always execute as the authenticated caller regardless of what the request body sets.
