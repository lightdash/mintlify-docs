...
<Note>
**Required filters and requirement groups block delivery.** If the underlying dashboard has any [required filters or filter requirement groups](/guides/limiting-data-using-filters#required-filters-and-filter-requirement-groups) that the scheduler overrides don't satisfy, **Create** and **Send now** are disabled with a tooltip explaining which rule is unmet. Set a value on the required filter, or on at least one filter in each requirement group, before saving the delivery — otherwise the schedule would run against an unfiltered dashboard on every send.
</Note>
