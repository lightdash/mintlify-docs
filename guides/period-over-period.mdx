...
Parameters can also be used to create dynamic period-over-period comparisons in your charts and dashboards. This approach lets users easily toggle between different date ranges or periods in a dashboard to see how metrics are tracking over time, without needing to rebuild charts or manually adjust filters. For more info on how to work with parameters refer to our parameters reference guide [here](guides/using-parameters). 

This guide walks through and example of how to do period-over-period analysis in Lightdash using parameters. A step-by-step guide is also included below the demo. 
...
For example, you want users to be able to select a specific time period such as `yesterday`, `last 7 days` or `last 30 days`, and then compare the selected current period to the previous period as well as the same period a year ago. 

This can be achieved by: 

## 1. Set up your date range / period parameter

Below we have set this parameter in the `dbt_orders` model: 
``` yaml
models:
  - name: dbt_orders
