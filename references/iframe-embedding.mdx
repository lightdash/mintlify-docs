...
```
https://your-instance.lightdash.cloud/embed/{projectUuid}?theme=dark&backgroundColor=1c1c1c&timezone=America/Los_Angeles#{jwtToken}
```
...
Accepts an [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) (e.g. `America/Los_Angeles`, `Europe/Berlin`, `Asia/Tokyo`). The value is used for dashboard tile queries, inherited totals and subtotals, and filter autocomplete in that session.
...
**Example:**
```
https://app.lightdash.cloud/embed/abc-123?timezone=America/Los_Angeles#eyJhbGci...
```

URL-encode the `/` if your URL builder requires it (e.g. `America%2FLos_Angeles`).
...
<Warning>
The embed `timezone` param requires the `EnableTimezoneSupport` feature flag on your project. When the flag is off, the param is ignored. When the flag is on, an invalid IANA value returns a 400 — empty or whitespace-only values fall through to the chart pin or project default.
</Warning>
...
```
https://app.lightdash.cloud/embed/abc-123?theme=dark&backgroundColor=1c1c1c&timezone=America/Los_Angeles#eyJhbGci...
```
