...
URL-encode the value so the `/` is escaped (`America%2FLos_Angeles`); the unencoded form is also accepted.
...
<Note>
When Beta timezone support is off, the param is ignored. When it is on, an invalid timezone value returns a 400, and an empty or whitespace-only value falls through to the chart pin or project default.
</Note>
