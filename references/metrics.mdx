...
models:
  - name: orders_model
    meta:
      metrics:
        revenue_per_user:
          type: number # number type metrics can only reference other METRICS - DIMENSIONS cannot be used in this type 
          sql: ${sum_revenue} / ${distinct_user_ids} # two aggregation metrics from this model are referenced here
        sum_total_paid:
          type: sum # sum or count metrics can only reference DIMENSIONS - METRICS cannot be used in this type
          sql: ${revenue} + ${taxes_paid} # two dimensions are added, then put inside a sum() aggregation
```
