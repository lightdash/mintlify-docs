...
| [convert_timezone](#convert-timezone) | No | boolean | If set to `false`, the dimension opts out of the project timezone for display, grouping, and extracts - the raw warehouse value is rendered instead. Defaults to `true`. Requires the project query timezone to be set. |
...
By default, every `timestamp` dimension follows the project timezone for display, grouping (`DATE_TRUNC` intervals), and calendar component extracts (`DAY_OF_WEEK_INDEX`, `MONTH_NUM`, etc.). Set `convert_timezone: false` to opt a single dimension out - the raw warehouse value is rendered instead, and time-interval children of the dimension (`_day`, `_month`, `_day_of_week_index`, ...) inherit the opt-out automatically.
...
<Tabs>
  <Tab title="dbt v1.9 and earlier">
    ...
    columns:
      - name: created_at_utc
        description: 'Always render in UTC, regardless of project timezone'
        meta:
          dimension:
            type: timestamp
...
`convert_timezone: false` only changes how the column is **displayed and grouped** - table cells, exports, `DATE_TRUNC` buckets, and calendar extracts all show the raw warehouse value. Filters always use the project timezone, so every dimension on the same row agrees on whether the row matches. In practice: a filter like "between 2026-01-01 and 2026-01-31" cuts at midnight in the project timezone, even though the column itself renders raw.
