...
When `case_sensitive` is set to `false`, Lightdash applies `UPPER()` transformations to both the database column values and filter values before comparison, enabling case insensitive matching.
