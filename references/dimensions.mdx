...
<Tip>
  When a results table cell contains JSON (an object, an array, or a JSON-formatted string), you can right-click the cell and select **View JSON** to open an interactive viewer. The viewer has a collapsible tree view and a raw text view, lets you copy the full JSON, and lets you click any node to copy its JSONPath (for example `$.metadata.version`) — useful for building `JSON_VALUE` expressions for your additional dimensions.
</Tip>
