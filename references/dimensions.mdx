...
<Snippet file="liquid-templating.mdx" />
...
Create rich displays with images and links:
          ...
          dimension:
            richText: |
              ...
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <p>
                  <strong>${ value.formatted }</strong><br/>
                  <a href="https://www.linkedin.com/search/results/people/?keywords=${ value.formatted | url_encode }" target="_blank">LinkedIn</a>
                </p>
              </div>
              {% endif %}{% endraw %}
    ```
...
You can reference values from other columns in your rich text using `row.table_name.column_name`:
    ...
    columns:
      - name: amount
        meta:
          dimension:
            richText: |
              ${ value.formatted } ${ row.orders.currency.raw }
    ```
  </Tab>
  <Tab title="dbt v1.10+ and Fusion">
    ```yaml
...
<Info>
For the rich text to work with row references, the referenced column must be included in your results table.
</Info>
