...
<Info>
  **Availability:** The `convert_timezone` dimension override is an [Experimental](/references/workspace/feature-maturity-levels) feature behind the `EnableTimezoneSupport` flag. Has no effect unless the [project query timezone](/guides/developer/timezones#project-query-timezone-ui-setting) is set.
</Info>

Set `convert_timezone: false` on a `timestamp` dimension to opt it out of the project query timezone for display, grouping, and calendar extracts. The raw warehouse value is rendered instead, and time-interval children of the dimension (`_day`, `_month`, `_day_of_week_index`, ...) inherit the opt-out automatically. Filters always use the project query timezone regardless of this setting.
...
See [Per-dimension timezone opt-out](/guides/developer/timezones#per-dimension-timezone-opt-out) in the Timezones guide for the full picture of how this interacts with other timezone settings, full dbt v1.9 / v1.10+ / Lightdash YAML examples, and filter behavior.
