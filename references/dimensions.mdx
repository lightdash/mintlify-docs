...
<Info>
  **Experimental:** The `convert_timezone` dimension override is currently in the [Experimental](/references/workspace/feature-maturity-levels) phase. Contact support to enable it for your organization. Has no effect unless the [project query timezone](/guides/developer/timezones#project-query-timezone-ui-setting) is set.
</Info>
...
See [Per-dimension timezone opt-out](/guides/developer/timezones#per-dimension-timezone-opt-out) in the Timezones guide for how this interacts with other timezone settings and filter behavior.

## Timestamp domain

Lightdash reads whether a `timestamp` column stores an instant (`aware`) or a bare wall clock with no zone (`naive`) from your warehouse catalog, so you usually don't need to declare it. Set `timestamp_domain` on a column when the catalog type doesn't match the data — for example a `TIMESTAMP_NTZ` column that already holds UTC instants, or a view whose column comes back untyped.
...
The YAML value wins over the catalog. It only applies when the dimension resolves to a real column: if you provide custom `sql`, or on additional dimensions, the override is dropped because the expression may change the domain. Interval children (`_day`, `_month`, ...) inherit the domain from their base.

See [Override a column's timestamp domain](/guides/developer/timezones#override-a-columns-timestamp-domain) in the Timezones guide for how naive columns interact with the data timezone.
...
Regardless of which option you use, filter autocomplete always returns `{ value, label }` pairs. The `value` is what gets used in the filter, and the `label` is what's displayed in the dropdown. Parameters that source their options from the same dimension via [`options_from_dimension`](/references/tables#parameter-configuration) automatically pick up these labels too.
...
The referenced dimension must live in the same table. Any [parameter](/references/tables#parameter-configuration) that sources its options from `customer_id` via `options_from_dimension` also picks up the customer name labels automatically.
