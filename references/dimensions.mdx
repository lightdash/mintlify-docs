...
| [filter_autocomplete](#filter-autocomplete) | No | Object with `values`, `fetch_from_warehouse` | Configure the filter autocomplete suggestions for this dimension. Provide a static list of `values` (with optional display `label`s) and/or disable warehouse-based autocomplete by setting `fetch_from_warehouse: false`. |
