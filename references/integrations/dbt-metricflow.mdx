...
`ratio`, `derived`, `cumulative`, and filtered metrics, as well as server-side translation from a connected Git repo, are on the roadmap.
...
Deploying this project gives the `orders` explore two metrics — `total_revenue` (a `sum` on `${TABLE}.amount`, labelled "Total revenue") and `unique_customers` (a `count_distinct` on `${TABLE}.customer_id`, from the `create_metric: true` measure) — with no Lightdash-specific YAML.