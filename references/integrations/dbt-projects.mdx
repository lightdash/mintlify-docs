...
Not sure whether to use `lightdash deploy` or `lightdash refresh`? Read about the [differences between the two commands](/references/lightdash-cli#lightdash-deploy-vs-lightdash-refresh) in the CLI reference.
...
<Info>
  **Private beta.** Multiple dbt sources is off by default and enabled per organization. [Contact us](https://www.lightdash.com/contact) to have it turned on for your org. Self-hosted deployments need Lightdash v0.3275.0 or later (shipped July 2026).
</Info>
...
Until you add a second source, projects behave exactly as before — nothing changes for existing single-source projects.
...
- **Add an additional source** — pick a connection type (GitHub, GitLab, Azure DevOps, Bitbucket, dbt Cloud, etc. — the same types supported for the primary source), give the source a display name, and fill in its repo/branch/subfolder/credentials and any environment variables. Environment variables are validated when the source is saved.
...
**Precedence.** Sources are ordered by precedence. The primary source is always `0` (highest priority). Additional sources are appended after it in the order they were added.

**Name collisions.** If the same model name exists in two sources, the **lowest-precedence source wins** and the superseded sibling is dropped from the merged Explores. Collisions are reported as **non-blocking warnings** naming both the winning and superseded source — compilation does not fail.

**What's merged vs. deduplicated.**

- **Models** — collisions resolved by precedence as above. Models can `ref()` each other and be joined across sources.
...
#### AI agent write-back — fully supported

AI agents that open PRs with dbt changes work across all git-backed sources. Before making changes, the agent resolves which source to target, in this order:
...
#### Explore write-back — primary source only

Writing custom metrics and dimensions back as a PR from the Explore view always opens the PR against the **primary source's repo**. This means:

- For models that come from the **primary source**, Explore write-back works normally.
- For models that come from an **additional source**, Explore write-back will fail — the model's YAML file isn't in the primary repo.

<Warning>
  **Current limitation.** Explore write-back only supports the primary source. To edit models in additional sources, use AI agent write-back or edit the source's dbt project directly.
</Warning>
