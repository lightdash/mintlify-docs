...
<AccordionGroup>
  ...
  <Accordion title="2. lightdash deploy from a GitHub Action">
    Same as #1, but runs inside a CI workflow with credentials stored as GitHub Secrets.

    | Property                              | Value                                                                                                                                  |
    | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
    | dbt code source                       | CI runner's checkout of the repo                                                                                                       |
    | Compile location                      | CI runner                                                                                                                              |
    | `profiles.yml` source                 | Provided by the workflow — either committed as an `env_var()` template, or written to disk from a GitHub Secret at runtime             |
    | Target                                | Default in the CI `profiles.yml`, overridden by `--target`                                                                             |
    | UI Connection Settings respected?     | No                                                                                                                                     |
    | Auth                                  | `LIGHTDASH_API_KEY` from GitHub Secrets                                                                                                |
    ...
    | Remote git connection required?       | No                                                                                                                                     |
    | Workflow template                     | [`deploy.yml`](https://github.com/lightdash/cli-actions) in `lightdash/cli-actions`                                                    |
    ...
    **Trade-offs:** requires duplicating warehouse credentials into CI on top of what's already in Lightdash; needs dbt available in the runner.
    ...
    <Warning>
      Never commit a `profiles.yml` with plaintext credentials. Either use an `env_var()` template committed to the repo, or write the file to disk at runtime from a GitHub Secret.
    </Warning>
  </Accordion>
  ...
  <Accordion title="4. lightdash refresh from a GitHub Action">
    Calls the same endpoint as the "Refresh dbt" button, but programmatically from CI. Lightdash does the work server-side using the project's UI Connection Settings.
    ...
    | Remote git connection required?       | Yes                                                                                         |
    | Workflow template                     | [`refresh.yml`](https://github.com/lightdash/cli-actions) in `lightdash/cli-actions`        |
  ...
  <Accordion title="5. Pinging the refresh API directly from a GitHub Action">
    Same as #4, but bypasses the Lightdash CLI entirely — just a `curl` to the API.
...
- **Production sync** → `lightdash deploy` in GitHub Actions (#2)
- **Previews** → `lightdash start-preview` in GitHub Actions
...
<Tip>
  Mechanisms #4 and #5 are underused. Most existing GitHub Actions examples lead with `lightdash deploy`, which is the heaviest option in CI (requires dbt + warehouse credentials duplicated into the runner). The refresh-based patterns are simpler and often a better fit when the project is git-connected.
</Tip>
