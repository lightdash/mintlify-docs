...
<Info>
  **Per-user permission gate.** `createContent` and `editContent` are also gated on the caller's role holding the `create:ContentAsCode` scope. The check runs at tool registration, so users without the scope don't see these tools in `tools/list`.

  - **Roles that get the scope by default:** Editors, Developers, Admins.
  - **Roles that don't:** Interactive Viewers, Viewers.
  - **Custom roles:** must add `create:ContentAsCode` explicitly — see [custom roles](/references/workspace/custom-roles).
...
- **Create scheduled delivery** (`createScheduledDelivery`) - Schedule an existing chart or dashboard to Slack, email, or another supported destination on a cron schedule. Point the tool at the content by slug and pass the schedule and recipients — the delivery is created against the active project using the caller's permissions.

<Info>
  **Registered independently from content writes.** An organization can enable scheduled delivery over MCP without enabling broader chart/dashboard writes (or vice versa). `createScheduledDelivery` only appears in `tools/list` when both of these are true:

  1. **Allow content changes via MCP** is on (**Settings → Ask AI → General**) — same kill-switch as `createContent` and `editContent`.
...
<Info>
  **Per-user permission gate.**
  ...
  - **Roles that get `create:ScheduledDeliveries` by default:** Interactive Viewers, Editors, Developers, Admins.
  - **Roles that don't:** Viewers.
  - **Custom roles:** must add `create:ScheduledDeliveries` explicitly.

  Project-level authorization still runs at invocation time on top of the registration gate.
</Info>
...
The assistant will call `createScheduledDelivery` with the chart's slug, the cron schedule, and the Slack destination — mirroring the [AI agent](/guides/ai-agents) `createScheduledDelivery` tool but exposed to any MCP client.
