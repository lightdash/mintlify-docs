...
<Info>
  **Run SQL** requires the `manage SqlRunner` permission. The SQL is executed directly against your warehouse, so use the appropriate SQL dialect for your connection (e.g., PostgreSQL, BigQuery, Snowflake).
  ...
  Callers without `manage SqlRunner` (for example, interactive viewers and below) do not see `run_sql` in `tools/list` — the tool is filtered out per-request based on the caller's role. This lets agents fall back cleanly to `run_metric_query` and the other read tools instead of repeatedly attempting `run_sql` and hitting a permission error. The per-project permission check inside SQL execution still runs on every invocation, so tool visibility is a coarse capability gate, not a replacement for it.
</Info>
