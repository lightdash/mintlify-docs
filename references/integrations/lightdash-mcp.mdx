...
Registration is gated on both checks — a user without the scope won't see `create_scheduled_delivery` in `tools/list` at all. When the tool is invoked, project-level authorization still applies on top.
...
The assistant will call `create_scheduled_delivery` with the chart's slug, the cron schedule, and the Slack destination — mirroring the behavior of the [AI agent](/guides/ai-agents) `createScheduledDelivery` tool but exposed to any MCP client.
