...
<Tabs>
  <Tab title="Self-hosted">
    ...
    ```bash
    PGWIRE_PORT=5433
    PGWIRE_HOST=analytics.example.com   # optional: the hostname clients should connect to
    ```
...
Use any Postgres client. The **Project settings → Semantic layer** page shows the exact host, port, database (project UUID), and ready-to-copy `psql`, connection URL, and JDBC snippets for your instance — copy the snippet that matches your client and paste in a token as the password.

The connection parameters are:

| Parameter | Value |
| --- | --- |
| Host | Your Lightdash instance host (shown on the settings page) |
...
| SSL mode | `disable` (TLS is not yet supported) |

### Example: `psql`

```bash
PGPASSWORD=ldsvc_... psql \
  -h analytics.example.com \
  -p 5433 \
  -U lightdash \
  -d ecom-store \
  "sslmode=disable"
...
```
postgresql://lightdash:ldsvc_...@analytics.example.com:5433/ecom-store?sslmode=disable
```

### Example: JDBC (DBeaver / DataGrip)

```
jdbc:postgresql://analytics.example.com:5433/ecom-store?sslmode=disable
```
