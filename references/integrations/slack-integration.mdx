...
The bot reacts to your @mention with two emoji to give you at-a-glance progress without reading the reply text:

- 👀 (`:eyes:`) — added immediately when the bot receives your @mention, before any authentication or agent routing, so you know your message was seen.
- ✅ (`:white_check_mark:`) — added on the same message when the agent successfully opens or updates a writeback pull request from your request.

If the reactions don't appear, your Slack workspace likely needs to re-authorize the Lightdash app to grant the new `reactions:write` scope — open your Slack integration settings in Lightdash and reconnect. The bot still responds and writebacks still run without the reactions; only the visual acknowledgements are skipped.
