...
You can control whether string filters are case sensitive or case insensitive at the table (explore) level. By default, all string filters are case sensitive (`case_sensitive: true`). When set to `false`, string filters on all dimensions in this table will ignore case differences when matching values.
