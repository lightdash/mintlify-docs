...
## Adding Tables to your project[​](#adding-tables-to-your-project "Direct link to Adding Tables to your project")
...
```
version: 2models:  - name: my_table    meta:      label: 'My Custom Table Name'      order_fields_by: 'label'      group_label: 'Revenue'      sql_filter: ${date_dimension} > '2020-01-01'      joins:        - join: my_other_table          sql_on: ${my_table.column_a} = ${my_other_table.column_a}      required_attributes:        is_admin: 'true'
```
...
| [sql\_filter](#sql-filter)                                | string                                  | A permanent filter that will always be applied when querying this table directly. Read more about `sql_filter` [below](#sql-filter)                  |   |
| [sql\_where](#sql-filter)                                 | string                                  | Alias for `sql_filter`                                                                                                                               |   |
| [required\_attributes](#required-attributes)              | Object with \{ user\_attribute, value } | Limits access to users with those attributes. [Read more about user attributes in here](/references/user-attributes)                                 |   |
| group\_details                                            | Object with \{ label, description }     | Describes the groups for dimensions and metrics                                                                                                      |   |
| [required\_filters](#required-filters)                    | Array of \{filter field: value}         | Dimension filters that will be applied when no other filter on those dimension exists. Read more about `required_filters` [below](#required-filters) |   |

### If you've added a new dbt model to your project, you need to do `dbt run` + `dbt refresh` before it will appear in Lightdash[​](#if-youve-added-a-new-dbt-model-to-your-project-you-need-to-do-dbt-run--dbt-refresh-before-it-will-appear-in-lightdash "Direct link to if-youve-added-a-new-dbt-model-to-your-project-you-need-to-do-dbt-run--dbt-refresh-before-it-will-appear-in-lightdash")
...
### Order fields by[​](#order-fields-by "Direct link to Order fields by")
...
```
version: 2models:  - name: my_table    meta:      order_fields_by: 'index'    columns:      - name: user_id      - name: product_id      - name: account_id
```
...
### Group label[​](#group-label "Direct link to Group label")
...
![](/assets/images/table-group-label-a6d42e6d72a08f15c392fb7cb03a2e6a.png)

### SQL filter (row-level security)[​](#sql-filter-row-level-security "Direct link to SQL filter (row-level security)")
...
```
models:  - name: sales    meta:      sql_filter: ${TABLE}.sales_region = 'EMEA'
```
...
```
SELECT  [...]FROM `lightdash`.`prod`.`sales`WHERE sales_region = 'EMEA'
```

#### Row-level security using user attributes[​](#row-level-security-using-user-attributes "Direct link to Row-level security using user attributes")
...
```
models:  - name: sales    meta:      sql_filter: ${TABLE}.sales_region IN (${lightdash.attributes.sales_region})
```

#### `sql_filter` will only be applied when querying tables directly.[​](#sql_filter-will-only-be-applied-when-querying-tables-directly "Direct link to sql_filter-will-only-be-applied-when-querying-tables-directly")
...
#### If you reference a dimension from a joined table in your `sql_filter`, the referenced table will always be joined in your queries.[​](#if-you-reference-a-dimension-from-a-joined-table-in-your-sql_filter-the-referenced-table-will-always-be-joined-in-your-queries "Direct link to if-you-reference-a-dimension-from-a-joined-table-in-your-sql_filter-the-referenced-table-will-always-be-joined-in-your-queries")
...
### Required attributes[​](#required-attributes "Direct link to Required attributes")
...
## Required Filters[​](#required-filters "Direct link to Required Filters")
...
```
version: 2models:  - name: orders    meta:      required_filters:        - date: 'inThePast 14 days'    columns:      - name: date        description: 'Order date'        meta:          dimension:            type: date
```

### If you have many filters in your list, they will be joined using `AND`[​](#if-you-have-many-filters-in-your-list-they-will-be-joined-using-and "Direct link to if-you-have-many-filters-in-your-list-they-will-be-joined-using-and")

For example:

```
version: 2  - name: orders    meta:      required_filters:        - date: 'inThePast 14 days'        - status: "completed"    columns:      - name: date        description: 'Order date'        meta:          dimension:            type: date      - name: status        description: 'Order status - completed, pending, cancelled'        meta:          dimension:            type: string
```

In the example above, the `orders` table will have a default filter of `date` in the past 14 days **and** `status` completed.

### Available filter types[​](#available-filter-types "Direct link to Available filter types")
...
[Edit this page](https://github.com/lightdash/lightdash/edit/main/docs/docs/references/tables.mdx)
