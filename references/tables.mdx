...
A optional `required` flag can be added - in which case the filter _cannot_ be removed. This can be particulalry useful if you have a large table and want to force users to filter on a partitioned date.
...
Note that we do also support a _legacy_ sturcture for defining required filters, see below:
