...
| [parameters](#parameters-configuration) | object | Model-level parameters that can be referenced in SQL queries. [Read about parameters](#parameters-configuration) |
...
The `parameters` section allows you to define model-level parameters that can be referenced in various parts of your model's SQL queries. These parameters are scoped to the specific model where they're defined.
...
Model-level parameters can be referenced in your model's SQL using the syntax `${lightdash.parameters.parameter_name}` or the shorter alias `${ld.parameters.parameter_name}`. 

For example, to reference a parameter named `region` in a dimension:
