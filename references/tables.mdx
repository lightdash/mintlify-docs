...
An optional `required` flag can be added - in which case the filter _cannot_ be removed or modified. This can be particulalry useful if you have a large table and want to force users to filter on a partitioned date.
...
<Info>
  When a filter has `required: true`, the filter is completely locked in the UI — the field, operator, and value cannot be modified, and the filter cannot be removed. It will always be applied exactly as defined in the model YAML.
