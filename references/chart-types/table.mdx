...
<Warning>
  Subtotals are only valid for `count` or `sum` metrics. Other aggregate metrics (such as `average`, `min`, `max`, and rates) will not produce correct subtotal values. We are working on a fix to hide subtotals for these metric types.
</Warning>
