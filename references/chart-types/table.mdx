...
- **Rows** accepts dimensions (which set the row grouping) and, when a pivot is configured, metrics and table calculations (which then render once per row instead of once per pivot column group).
- Drag fields between the lists to move them, and drag within a list to reorder. The order in the **Rows** list is the order the columns render on the left-hand row axis of the pivot.
