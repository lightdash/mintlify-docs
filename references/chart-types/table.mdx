...
When metrics are displayed as columns (the default behavior), the column limit is divided by the number of metrics. This is because each unique dimension value combination creates one column per metric.

For example, with a column limit of 60 and 3 metrics (`gross_revenue_sum`, `net_revenue_sum`, `margin`):
- Maximum unique dimension combinations: `floor(60 / 3)` = **20 combinations**
- Each combination creates 3 columns (one per metric)
- Total columns: 20 × 3 = 60

| created_date_month | partner_name | gross_revenue_sum | net_revenue_sum | margin |
|--------------------|--------------|-------------------|-----------------|--------|
| 2024-01            | Acme         | 1000              | 800             | 200    |
| 2024-01            | Beta         | 1500              | 1200            | 300    |
| ...                | ...          | ...               | ...             | ...    |
