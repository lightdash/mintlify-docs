...
You can sort a table chart directly from the column headers — no need to open the sort pill above the chart.

Click a column header to open the column menu, then pick a direction under **Sorting** (for example, `Sort A → Z` / `Sort Z → A`, or `Sort 1 → 9` / `Sort 9 → 1` for numeric columns). The active direction is marked with a check. To clear a sort, reopen the menu and choose `Remove sort`.

<Note>
  Header-click sorting is only available in edit mode. Viewers of a saved chart or dashboard tile see the sort that was saved with the chart but can't change it from the headers.
</Note>

The same rules from the [Explore sort menu](/get-started/exploring-data/using-explores#sort-results) apply: clicking a header replaces the existing single-column sort, and you can still open the sort pill to layer in multiple sorts or configure [NULL sorting](/get-started/exploring-data/using-explores#sorting-null-values).
...
Pivot tables support clickable header sorting, just like non-pivot tables. The column you click determines what gets sorted, and pivot tables allow a few options that aren't available in a flat table:

- **Click a row-dimension header** (the index columns on the left) to sort rows alphabetically or numerically by that dimension. This is the same behavior as a non-pivot table.
...
Each click opens the same menu used elsewhere in Lightdash, showing the available sort directions and a `Remove sort` option once a direction is active. Sorts applied through headers are saved with the chart configuration.

<Note>
  Pivot-header sorting is only available in edit mode. Saved charts and dashboard tiles render with the sort that was saved.
</Note>
