...
The pivot table configuration panel exposes three separate lists — **Columns**, **Rows**, and **Metrics** — so you can control exactly which fields appear on the row axis of a pivot and in what order.

- **Rows** accepts dimensions (which define the row axis of the pivot and the order of the left-hand columns) and, when a pivot is configured, metrics and table calculations. A metric or table calculation in **Rows** renders once per row, showing its value from the first pivot column group, instead of repeating in every column group.
- Drag dimensions between **Columns** and **Rows**, and drag metrics or table calculations between **Metrics** and **Rows**, to move them. Drag within **Columns** or **Rows** to reorder; the order of the **Rows** list is the order the row-axis columns render.
- Toggle a field's eye icon to hide it from the rendered pivot without removing it from the underlying query — see [Hidden dimensions and sorting](#hidden-dimensions-and-sorting).
- The configuration is persisted with the chart, so saved charts and dashboard tiles render with the same row layout you set up in edit mode.

Placing a metric or table calculation in **Rows** is the key to building cohort-style layouts: the metric shows once per row on the left, while the pivoted dimension (for example, months since a cohort started) spreads across the columns. Combined with [conditional formatting color ranges](#color-ranges), this produces retention heatmaps like the one below.

<img
  alt="Cohort retention pivot table with a metric configured on the row axis and months across the columns"
  className="block dark:hidden"
  src="/images/references/chart-types/table/pivot_rows_cohort_retention_light.png"
/>
<img
  alt="Cohort retention pivot table with a metric configured on the row axis and months across the columns"
  className="hidden dark:block"
  src="/images/references/chart-types/table/pivot_rows_cohort_retention_dark.png"
/>
...
<img
  ...
  className="block dark:hidden"
  src="/images/references/chart-types/table/pivot_column_limit_example_1_light.png"
/>
<img
  alt="Pivot column limit example"
  className="hidden dark:block"
  src="/images/references/chart-types/table/pivot_column_limit_example_1_dark.png"
/>
...
<img
  ...
  className="block dark:hidden"
  src="/images/references/chart-types/table/column_limit_exceeded_light.png"
/>
<img
  alt="Pivot column limit exceeded"
  className="hidden dark:block"
  src="/images/references/chart-types/table/column_limit_exceeded_dark.png"
/>
...
<img
  alt="Metrics as rows"
  className="block dark:hidden"
  src="/images/references/chart-types/table/metrics_as_rows_light.png"
/>
<img
  alt="Metrics as rows"
  className="hidden dark:block"
  src="/images/references/chart-types/table/metrics_as_rows_dark.png"
/>
