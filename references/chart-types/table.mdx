...
<Tip>
  If you have a Lightdash Pro or Enterprise account, this limit can be increased. Note that increasing the limit will have a negative impact on performance. [Contact us](https://www.lightdash.com/contact) to discuss your needs.
</Tip>
...
For example, if you pivot by `created_at_month` (with 12 months of data) and have 6 metrics:
- Total columns: 12 months × 6 metrics = **72 columns**

The column limit is divided by the number of metrics to determine how many dimension values you can display. With a column limit of 200 and 6 metrics:
- Maximum dimension values: `floor(200 / 6)` = **33 months**

Here's what a pivoted table looks like with `created_at_month` as a column and 3 metrics:

| partner_name | Jan 2024 - revenue | Jan 2024 - orders | Jan 2024 - margin | Feb 2024 - revenue | Feb 2024 - orders | Feb 2024 - margin | ... |
|--------------|-------------------|-------------------|-------------------|-------------------|-------------------|-------------------|-----|
| Acme         | 1000              | 50                | 200               | 1200              | 60                | 240               | ... |
| Beta         | 1500              | 75                | 300               | 1400              | 70                | 280               | ... |

With 12 months and 3 metrics, this creates 36 columns (12 × 3).

#### When metrics are rows

When you enable **Show metrics as rows**, the column limit applies directly to the number of unique dimension value combinations. Metrics appear as separate rows for each combination, so they don't count toward the column limit.

For example, with a column limit of 60 and 3 metrics:
- Maximum unique dimension combinations: **60 combinations**
- Each combination gets 3 rows (one per metric)

| created_date_month | partner_name | Metric            | Value |
|--------------------|--------------|-------------------|-------|
| 2024-01            | Acme         | gross_revenue_sum | 1000  |
| 2024-01            | Acme         | net_revenue_sum   | 800   |
| 2024-01            | Acme         | margin            | 200   |
| 2024-01            | Beta         | gross_revenue_sum | 1500  |
| ...                | ...          | ...               | ...   |

This means you can display more dimension combinations when using metrics as rows, which can be useful when you have many metrics to compare across fewer dimension values.
