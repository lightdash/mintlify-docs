...
- `--combine-manifest <path>`
  - Path to an additional dbt `manifest.json`. Models present in this file but missing from the preview-generated manifest are merged in so the preview reflects the full project even when dbt was only run on a subset of models. The preview-generated manifest always wins on conflicts.
- `--use-batched-deploy`
  - (default: false)
