...
Static `options` can be authored as either plain values or `{ label, value }` objects. Use the object form when the text you want users to see in the UI is different from the value that should be substituted into SQL — for example, when the underlying value is an ID:
