...
The `granularity_labels` setting lets you override how standard granularities are displayed across the project. This is useful when your business uses a non-default convention — for example, a week that starts on Monday — and you want the UI to reflect that everywhere a granularity is shown.
...
| Property                         | Required | Value  | Description                                                                       |
| :------------------------------- | :------- | :----- | :-------------------------------------------------------------------------------- |
| `<granularity>` (e.g. `week`)    | No       | string | Label appended to the standard granularity name. Keys are case-insensitive.        |

Supported keys are the standard granularities: `raw`, `millisecond`, `second`, `minute`, `hour`, `day`, `week`, `month`, `quarter`, `year`. Unknown keys are ignored with a warning at project compile time.

**Where the override is applied:**

* **Explorer dimension labels** — appended to the auto-generated label of each time-truncated dimension. For example, with `week: "starting Monday"`, the `Order date Week` dimension becomes `Order date Week starting Monday`.
* **Explorer sidebar tree** — the same relabeling is shown in the dimension tree.
* **Date zoom dropdown** — standard granularities are relabeled to match (e.g. `Week starting Monday`).
* **Chart axis labels** — axis labels that reflect the active grain pick up the override automatically.

**How it works:**

* **Append-only** — the override string is appended to the standard granularity name, not used as a full replacement. Use a fragment like `"starting Monday"`, not `"Week starting Monday"`.
