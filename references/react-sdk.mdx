...
You have two ways to manage the allow-list:

### Option 1: From the UI (recommended)

Organization admins (users with **manage Organization** permission) can configure allowed origins directly in Lightdash:

1. Go to **Project settings → Embed configuration → CORS**.
2. Toggle **Enable CORS** on for your organization.
3. Add one entry per row. Each entry can be either:
   - An exact origin, such as `https://app.example.com`, or a wildcard subdomain like `*.example.com`.
   - A regex pattern wrapped in slashes, such as `/^https:\/\/.*\.example\.com$/`, by switching the row to **regex mode**.
4. Save the changes. Entries are merged with the instance-level allow-list at runtime.

Origins must use `http://` or `https://` and cannot contain a path or query string. Invalid regex patterns are rejected.

### Option 2: From environment variables

For self-hosted instances, you can also (or instead) set CORS values via environment variables. Lightdash Cloud customers should use the UI above or contact the Lightdash team.
...
CORS is enabled by default. Set `LIGHTDASH_CORS_ENABLED=false` to disable CORS at the instance level.
