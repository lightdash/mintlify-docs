...
<Warning>
  If your project uses [personal warehouse connections](/references/personal-warehouse-connections) to manage data access, we do not recommend enabling caching since it's possible for data to be cached by one user and served to another user with different access.
</Warning>
