...
For more information about dbt connection types (Github, Gitlab, Bitbucket, etc.) and the fields required for each type, [read the dbt project section in our connection guide](https://docs.lightdash.com/get-started/setup-lightdash/connect-project#2-import-a-dbt-project).
...
dbt project and you want to speed up the sync process. Unlike [table selection](https://docs.lightdash.com/guides/adding-tables-to-lightdash#limiting-the-tables-in-lightdash-using-dbt-tags), this selector is applied to the dbt models, so it will skip the entire compilation process for the models that you don't want to see in Lightdash.
