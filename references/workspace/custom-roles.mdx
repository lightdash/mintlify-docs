...
Custom roles are either **project scoped** or **organization scoped** — not both. When creating a new custom role, you must select whether the role will be applied to a user or user group at the project level or at the organization level.

* **Organization custom roles** can include both organization-only scopes and project scopes, so an org-level custom role can mirror the built-in system org roles and apply project permissions across every project.
* **Project custom roles** can only include project scopes — organization-only scopes cannot be added to a project role.
