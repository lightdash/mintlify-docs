...
<Tip>
  Promoting content from a preview back to production still requires promote (or editor) rights on the destination project. The preview-edit scopes do not grant promotion — that gate is enforced separately so a reviewer remains in the loop.
</Tip>
