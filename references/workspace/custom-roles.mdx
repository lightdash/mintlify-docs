...
Custom roles are either **project scoped** or **organization scoped**. When you create a custom role, you choose whether it will be assigned at the project level or the organization level — this determines which scopes you can pick from and where the role can be assigned:

- **Project custom roles** can include project scopes (for example, `view:Dashboard`, `manage:SavedChart`, `manage:ContentAsCode`). They cannot include organization-only scopes like managing organization access or SCIM.
- **Organization custom roles** can include the same project scopes as project custom roles **plus** organization-only scopes. Project scopes on an organization custom role apply across every project in the organization, so an org custom role can mirror what a system organization role does while adding or restricting specific project-level abilities.

<Warning>
  Once a custom role type (organization or project) has been selected, you cannot change it. If you need a role with a different type, you will have to create a new role.
</Warning>
...
4. Select the **role type** — **Organization** or **Project**. This determines where the role can be assigned and which scopes are available. **Project** roles show project scopes only. **Organization** roles show both project scopes (applied across every project) and organization-only scopes. Once a role type has been selected, it cannot be changed — you'll need to create a new role if you want a different type.
...
### Content-verification scopes

Two scopes control access to [verified charts and dashboards](/guides/verified-content). They split discovery from write access so read-only users and service accounts can find verified content without gaining the ability to change verification state.

**View content verification** (`view:ContentVerification`) is a read-only scope. It lets a role list the verified charts and dashboards in a project and see the verified badge everywhere it appears — in the homepage tab, in the project settings verification panel, and in the `list_verified_content` MCP tool. This scope is included by default on the Interactive Viewer role (both project and organization) and on read-only service accounts, so editor-level custom roles and read-only integrations can discover verified content out of the box. The list is filtered by the caller's space access: users only see verified content in spaces they can otherwise access.

**Manage content verification** (`manage:ContentVerification`) is the write scope. It lets a role verify and unverify charts and dashboards, and it implies `view:ContentVerification`. This scope is included by default on Admin and Developer roles. Grant it to any custom role that should be able to sign off on content.

Use `view:ContentVerification` on its own when you want a role to discover trusted content (for example, an AI agent service account using the MCP `list_verified_content` tool) without granting the ability to change verification state. Grant `manage:ContentVerification` to reviewers who should be able to approve content.
