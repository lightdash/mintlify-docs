...
Exporting a dashboard to Google Sheets is bounded separately, because Google caps a single
spreadsheet at 10 million cells across all of its tabs. These limits are fixed and are not
configurable from this panel:
...
A tab trimmed by either limit is still created and still contains data — it's marked `Truncated`
in the export's summary tab, along with the number of rows that were written. The export as a
