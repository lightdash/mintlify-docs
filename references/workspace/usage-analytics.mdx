...
| Trino          | Sends metadata as comma-separated `key=value` pairs in the [`X-Trino-Client-Tags`](https://trino.io/docs/current/develop/client-protocol.html#client-request-headers) HTTP header on each submitted query. Tag keys and values are restricted to `a-z`, `A-Z`, `0-9`, `_`, and `-`, and truncated to 60 characters; unsafe characters are replaced with `_`. |
