...
The compilation history displays a list of all project compilations, showing when each compilation occurred, who triggered it, and the source. Sources include:

- **Refresh dbt**: A user clicked the `Refresh dbt` button in the UI.
- **CLI Deploy**: A compilation was triggered by `lightdash deploy` from the CLI or CI/CD.
- **Create Project**: A compilation ran when a new project was created.
- **Project connection form**: A compilation was triggered from the project connection settings (for example, when testing or saving connection changes).
