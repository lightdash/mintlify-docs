---
title: "User impersonation"
description: "Temporarily view Lightdash as another user to troubleshoot user-specific issues and permissions."
---

User impersonation allows organization admins to temporarily view Lightdash as another user in their organization. This helps troubleshoot user-specific issues like permission problems, data access questions, or UI concerns without requiring the user's credentials.

## How it works

When you impersonate a user:

- You see Lightdash exactly as that user sees it, including their permissions, spaces, and data access
- A banner appears at the top of the screen indicating you're impersonating someone
- All actions are logged for audit purposes—Lightdash logs the start and end of each impersonation session, and every request made while impersonating includes both the impersonating admin's `userUuid` and the impersonated user's `userUuid`
- Each impersonation session has a hard limit of **15 minutes**. After 15 minutes, the session automatically ends and you return to your normal admin view
- You can stop impersonation at any time by clicking the button in the banner

## Who can use impersonation

Only **organization admins** can impersonate other users. You cannot impersonate users outside your organization.

## Enabling impersonation

Organization admins can enable user impersonation from **Settings** > **Organization**. Toggle the **User impersonation** card on to allow admins to impersonate other users in the organization. This setting can be turned off at any time.

When you turn the toggle off, any active impersonation session is cleared on the next request.

<Frame>
  ![Clean Shot 2026 03 12 At 15 38 18@2x](/images/CleanShot2026-03-12at15.38.18@2x.png)
</Frame>

## Starting impersonation

1. Go to **Settings** \> **Users & Groups**
2. Find the user you want to impersonate
3. Click the **...** menu next to their name
4. Select **Impersonate user**

<Frame>
  ![Start impersonating a user](/images/references/workspace/user-impersonation/impersonate-menu.png)
</Frame>

You'll immediately switch to viewing Lightdash as that user, with a banner indicating the active impersonation session.

<Frame>
  ![Impersonating a user](/images/references/workspace/user-impersonation/impersonation-banner.png)
</Frame>

## Stopping impersonation

Click the **Stop impersonating** button in the banner at the top of the screen. You'll return to your normal admin view.