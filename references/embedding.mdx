...
```typescript
{
  content: {
    ...
    // Layout
    stickyHeader?: boolean,         // Pin tabs and filter bar while scrolling
  },
...
Pins dashboard tabs and the filter bar to the top of the embed scroll container so they stay visible as users scroll through tiles.

```typescript
stickyHeader?: boolean
```

When off (the default), the header scrolls away with the rest of the dashboard — the original embed behavior. When on, the header sticks to the top of the embed viewport, mirroring the regular dashboard view in Lightdash. Use it for tall, multi-tile dashboards where users need to keep filters or tabs in reach.

This option can also be toggled from the **Appearance** section of the embed preview form in your project's embed settings.
