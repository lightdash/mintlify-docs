...
Write actions let embedded users save new charts from the embedded Explore view. When the JWT includes a `writeActions` claim, Lightdash uses a configured actor (a service account or a regular user) to perform the write on behalf of the embedded viewer, and forces created content into a specific destination space.

This is useful when you want to let your customers explore a chart, change it, and save the result back to Lightdash — without giving them a Lightdash login.
