...
<Info>
If a dashboard filter is [locked](/guides/limiting-data-using-filters#locking-dashboard-filters), any embed override targeting that filter's field is ignored and the saved locked value is used instead. Use locked filters when you need to guarantee that an embedded dashboard always runs with a specific value — for example, scoping every query to the customer who owns the embed session.
</Info>
