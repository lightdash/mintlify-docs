...
<Info>
If a dashboard filter is [locked](/guides/limiting-data-using-filters#locking-dashboard-filters), any `externalFilters` value targeting that field is ignored and the dashboard's saved value is used instead. Lock filters that should never be overridden by the embedding application.
</Info>
