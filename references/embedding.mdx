...
- [Embedding quickstart](/guides/how-to-embed-content)
...
User attributes filter tables that have `sql_filter` configurations in dbt. See the complete [User attributes guide](/references/user-attributes).
...
This metadata appears in [query tags](/references/usage-analytics#query-tags) for usage analytics. If you don't provide an `externalId`, Lightdash automatically generates one based on the embed token.
...
See [User attributes reference](/references/user-attributes) for complete implementation guide.
