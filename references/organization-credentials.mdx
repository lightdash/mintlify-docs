...
3. Select **Organization credentials** from the sidebar
...
- **Use descriptive names**: Name credentials clearly (e.g., "Production Snowflake", "Staging Snowflake") so users know which to select
- **Document credential usage**: Keep internal documentation about which credentials should be used for different project types
- **Regular credential rotation**: Update credentials periodically following your organization's security policies
- **Limit credential scope**: Create separate credentials for different environments (production, staging, development) rather than one credential for all purposes
- **Monitor usage**: Regularly review which projects are using which credentials

### For project creators

- **Select the correct credential**: Make sure you're using the appropriate credential for your project's purpose
- **Contact admins for new credentials**: If you need a credential that doesn't exist, reach out to your organization admin rather than creating a project with manual credentials
- **Respect credential permissions**: Don't attempt to access data outside the scope intended for the credential you're using
...
### Can I use organization credentials with personal warehouse connections?

Yes, organization credentials and [personal warehouse connections](/references/personal-warehouse-connections) serve different purposes and can be used together. Organization credentials define the project-level warehouse connection, while personal warehouse connections allow individual users to query data using their own credentials in projects that require user credentials.

### What happens if an organization credential is deleted?

If you delete an organization credential that is actively being used by projects, those projects may lose their warehouse connection. Always verify which projects are using a credential before deleting it, and communicate with project owners to migrate them to a different credential first.

### Can I convert an existing project to use organization credentials?

Yes, organization admins can update a project's warehouse connection settings to use an organization credential instead of manually configured credentials. This can be done in the project settings under the warehouse connection section.

### Do organization credentials work with all warehouse types?

Currently, organization credentials are only supported for **Snowflake SSO connections**. Support for additional warehouse types including BigQuery, Redshift, Databricks, and PostgreSQL will be added in the future.