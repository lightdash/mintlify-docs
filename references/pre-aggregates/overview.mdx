...
<CardGroup cols={2}>
  ...
  <Card title="External pre-aggregates" icon="database" horizontal href="/references/pre-aggregates/external-tables">
...
* **Managed pre-aggregates** are the default. Lightdash materializes the rollup on your warehouse, stores the result, and serves matching queries from that stored copy using in-memory DuckDB workers. You only write the definition — Lightdash handles the rest.
* **External pre-aggregates** delegate the materialization to you. You point a pre-aggregate at a warehouse table you build and refresh yourself (with dbt, Airflow, a scheduled query, or a warehouse-native materialized view). Lightdash uses the definition only for matching and routing — matching queries run on your project warehouse against your table. See [External pre-aggregates](/references/pre-aggregates/external-tables) for the full workflow.

Managed and external pre-aggregates can coexist on the same model. Matching, filter compatibility, granularity rules, and supported metric types are identical on both paths — the difference is only where the data lives and who refreshes it.
...
- **Metrics re-aggregate cleanly** — all metrics are [supported types](#supported-metric-types). Non-additive metrics like `count_distinct`, `median`, and `percentile` can't be faithfully re-computed from stored rows, so they only match on an [exact match](#exact-match-queries) of the pre-aggregate. Anything resolved at runtime — raw SQL table calculations, [`sql_filter`](/references/tables#sql-filter-row-level-security) metrics, or SQL dependent on [Parameters](/references/lightdash-config-yml#parameters-configuration) and [user attributes](/references/workspace/user-attributes) — is never eligible.
...
- A dimension reached through a [custom bin](/guides/custom-fields#bin) does not count as selected either — bins collapse groups and break the one-row-per-result guarantee.
...
<Warning>
  Filter matching compares operators directly. A query filter only counts as "the same or narrower" when it uses the **same operator** as the pre-aggregate's filter — relative-to-relative (for example, `inThePast` compared against `inThePast`), or absolute-to-absolute (for example, `equals` compared against `equals`).

  Lightdash does not resolve a relative filter into a concrete date range at match time. An absolute date filter like `event_date is 2026-08-08` will not match a pre-aggregate filter like `event_date inThePast 30 days`, even when the selected date falls inside that window. The reverse is also true. Queries with a mismatched operator fall back to the warehouse.

  If a rolling window is the underlying need, filter the query with the same relative operator (for example, `inThePast N days`) to hit the pre-aggregate.
</Warning>
...
You can add exact-only metrics to any pre-aggregate. They are materialized (or expected in the [external table's column contract](/references/pre-aggregates/external-tables#2-get-the-column-contract)) and served whenever a query matches the pre-aggregate exactly. Non-exact queries that include an exact-only metric miss with the reason **`non_additive_metric_requires_exact_match`** — the fix is to select exactly the pre-aggregate's dimensions at exactly its granularity.

<Tip>
  Exact-only metrics are omitted from the generated pre-aggregate preview explore, since direct queries against that explore don't enforce the exact-match rule. Define and use them on your regular explore; matching still routes qualifying queries to the pre-aggregate.
</Tip>
...
For that reason, non-additive metrics are only served on [exact-match queries](#exact-match-queries), where no combining happens. A non-exact query that includes one falls back to the warehouse with the miss reason `non_additive_metric_requires_exact_match`.

We're investigating serving `count_distinct` on non-exact matches through approximation algorithms. [Follow this issue](https://github.com/lightdash/lightdash/issues/21536) for updates.

### Metrics that are never eligible

The following metrics can't be pre-aggregated at all, on exact or non-exact matches:

- `percent_of_total`, `percent_of_previous`
- `running_total`
- Custom SQL / post-calculation metrics, including `type: number` metrics that reference a non-additive metric — [Follow this issue](https://github.com/lightdash/lightdash/issues/21537)
...
| **Coverage**                 | All metric types, all query shapes                 | Re-aggregatable metrics (sum, count, min, max, average) on any match; non-additive metrics (count_distinct, median, percentile, etc.) on exact matches only |
...
- You need unsupported features listed above, such as [Parameters](/references/lightdash-config-yml#parameters-configuration), [`sql_filter`](/references/tables#sql-filter-row-level-security), or raw SQL table calculations, or you need `count_distinct` and similar non-additive metrics across query shapes that aren't exact matches
