...
- **Metrics re-aggregate cleanly** — all metrics are [supported types](#supported-metric-types). No non-additive metrics (like `count distinct`, `median`, etc. which require recalculation per grouping) and nothing resolved at runtime (raw SQL table calculations, [`sql_filter`](/references/tables#sql-filter-row-level-security) metrics, or SQL dependent on [Parameters](/references/lightdash-config-yml#parameters-configuration) and [user attributes](/references/workspace/user-attributes)). These can't be faithfully re-computed from stored rows.
