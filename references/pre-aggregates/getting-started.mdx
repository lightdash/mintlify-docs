...
- Queries including a non-additive metric like `count_distinct` unless they select exactly the pre-aggregate's dimensions at exactly its granularity (see [Exact match queries](/references/pre-aggregates/overview#exact-match-queries))
