...
| **Filter dimension not in pre-aggregate** | A filter references a dimension not in the pre-aggregate. This also fires when a model [`required_filters`](/references/pre-aggregates/getting-started#required-filters-and-pre-aggregates) target is missing from the pre-aggregate's dimensions. | Add the filter dimension to the `dimensions` list — even if it's only used for filtering, not grouping. |
| **Pre-aggregate filter not satisfied** | The pre-aggregate definition includes a static filter, but the query is missing it, uses a broader filter, or uses a different filter operator. Filter compatibility is only checked when both filters use the **same operator** — an absolute date filter (`is 2026-01-15`) will not match a relative pre-aggregate filter (`inThePast 30 days`) even if the date falls inside the window, and vice versa. This also fires when the pre-aggregate's `filters` block targets a model [`required_filters`](/references/pre-aggregates/getting-started#required-filters-and-pre-aggregates) field or a sibling grain of one — the two would conflict, so the pre-aggregate is treated as ineligible. | Match the pre-aggregate filter's operator in the query (for example, use `inThePast N days` against a relative pre-aggregate filter), add the matching filter to the query, or create another pre-aggregate for that query pattern. Remove any pre-aggregate `filters` on required-filter fields. |
