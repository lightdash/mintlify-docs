...
<Info>
  **Fanout handling is an experimental feature** currently only available in Lightdash Cloud. If you would like to use it, [reach out to the Lightdash team](/contact/contact_info).
</Info>
...
- **Metrics that reference a dimension from joined table** ([see below](#metrics-that-reference-a-dimension-from-joined-table))
- **Metrics that have a filter with a dimension from joined table** ([see below](#metrics-that-have-a-filter-with-a-dimension-from-joined-table))
- **Dimension that references a dimension from joined table** ([see below](#dimension-that-references-a-dimension-from-joined-table))
- **Metrics that have multiple levels of references** ([see below](#metrics-that-have-multiple-levels-of-references))
...
<Note>
  If you are being impacted by these limitations, please reach out to support as we are interested to understand your use cases.
</Note>
...
#### Dimension that references a dimension from joined table

Dimensions that reference dimensions from joined tables can cause issues with Lightdash's fanout handling.
