...
- **MIN/MAX functions** - `MIN(user_id)` ✅ and `MAX(user_id)` ✅ are safe because they return a single value regardless of duplication. These functions look at the entire set of values and return just one result.

- **Custom deduplication techniques** - For other metrics, you may need to implement specific deduplication strategies based on your data model.

### Examples

#### Single one-to-many join

When you join an organization table to a users table, each organization can have multiple users. In this scenario:
...
❌ **Organization metrics become inflated** - Each organization will appear multiple times in the result set (once for each associated user). This means organization-level metrics like "total revenue per organization" will be counted multiple times, leading to inflated results. For example, if "Acme Corp" has 5 users, its revenue would be counted 5 times in a simple SUM calculation.

#### Chained one-to-many joins

When you create a chain of one-to-many relationships by joining users to their orders, and then orders to their support tickets:
...
**Example:** Users → Orders AND Users → Tickets (both relate to users, but not directly to each other)

This scenario creates the most problematic data situation, when you join multiple one-to-many relationships that branch from the same table but aren't directly related to each other:
...
- **Order metrics get inflated** by the number of tickets the user has. Even if an order has no tickets itself, it will appear multiple times in the result set - once for each ticket the user has.

- **Ticket metrics get inflated** by the number of orders the user has. Similarly, each ticket will be duplicated for every order the user has placed.

For example, if Alice has 2 orders and 3 tickets, after joining users → orders → tickets, the result would look like:

```
Alice | Order 1 | Ticket 1
Alice | Order 1 | Ticket 2
Alice | Order 1 | Ticket 3
Alice | Order 2 | Ticket 1
Alice | Order 2 | Ticket 2
Alice | Order 2 | Ticket 3

Result: 6 rows instead of 1
```

This creates the worst-case scenario where virtually no aggregate metric (`SUM`, `COUNT`, `AVG`) is safe without explicit deduplication techniques.
