...
```sql
SELECT a.account_name, u.user_name, f.fee_type, f.per_user_amount
FROM accounts a
JOIN users u ON a.account_id = u.account_id
JOIN fees f ON a.account_id = f.account_id;
```
...
When you need fanouts for your business logic we recommend **creating a dedicated table**.
Instead of joining tables that cause unwanted fanouts, we strongly encourage creating a dedicated table that already contains the pre-joined data at the correct granularity. This is the most reliable approach to avoid conflicts with Lightdash's fanout handling.
