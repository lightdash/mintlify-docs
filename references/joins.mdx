...
3. **cte_unaffected**: Only needed when calculating metrics that could be affected by the join relationships. In some cases, this CTE isn't required if you're not calculating any metrics that would suffer from fanout.

**General Rule**: Dimensions go in the keys CTE, metrics get calculated in separate CTEs. The deduplication happening with the DISTINCT clause in the keys CTE is the critical piece that prevents the fanout issue, ensuring each record contributes exactly once to the aggregation.
...
cte_metrics_orders AS (
    ...
    LEFT JOIN "postgres"."jaffle"."orders" AS "orders" ON cte_keys_orders."pk_order_id" = "orders".order_id
...
# Step 4: Join the metrics ctes together
SELECT
  cte_unaffected.*,
