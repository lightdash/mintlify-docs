...
For example, if `products` joins `inventory` and `reviews` joins `products`, the `inventory` join is **not** carried over to the `reviews` explore. Any dimension or metric on `products` that references an `inventory` field (like `${ld_inventory.in_stock}`) will fail to compile when exploring from `reviews`.

```
reviews → products     ✅ works
reviews → products → inventory   ❌ not automatic
```
...
```sql
-- in your dbt model for products
SELECT
  *,
  COALESCE(inventory.in_stock, 0) > 0 AS inventory_in_stock
FROM products
LEFT JOIN inventory ON products.sku = inventory.sku
```
    ...
    models:
      - name: reviews
        meta:
          joins:
            - join: products
              sql_on: ${reviews.sku} = ${products.sku}
            - join: inventory
      ...
      - name: reviews
        meta:
          joins:
            - join: products
              sql_on: ${reviews.sku} = ${products.sku}
              fields: [sku, product_title]  # omit metrics that reference inventory
    ```
  </Tab>
  <Tab title="dbt v1.10+ and Fusion">
    ```yaml
