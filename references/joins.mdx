...
Only metrics using `DISTINCT`, `MAX` or `MIN` are protected from inflation:
...
**Example:** Organizations → Users (one org has many users)

**Impact:**

✅ **Metrics from the "many" side are accurate** (user metrics)
❌ **Metrics from the "one" side get inflated** (organization metrics get duplicated for each user)

#### Chained one-to-many joins

**Example:** Users → Orders → Tickets (chain relationship where tickets belong to specific orders)

**Impact:**

✅ **Final table metrics are accurate** (ticket metrics)
❌ **Upstream tables get inflated by downstream counts**
    - Order metrics: inflated by number of tickets per order
    - User metrics: inflated by total number of tickets
...
❌ **EVERY aggregate metric gets inflated** (cartesian product effect)
- User metrics: inflated by (orders × tickets)
- Order metrics: inflated by number of tickets
- Ticket metrics: inflated by number of orders

This creates the worst-case scenario where no aggregate metric (`SUM`, `COUNT`, `AVG`) is safe.

**Visual example:**

```
Alice has 2 orders and 3 tickets

After joining users → orders → tickets:
