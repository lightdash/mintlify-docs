...
Lightdash can automatically handle deduplicating metrics that are inflated due to fanouts in join relationships are specified in `YAML`. Developers can identify joins that are not a one-to-one match and specify the primary keys to join on. To do this

- Identify the join relationship: `many_to_one`, `one-many`, `many-many`
- Choose a key in each table for which duplicates will be removed
