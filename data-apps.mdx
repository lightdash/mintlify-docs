...
You can change the model on every prompt - including mid-iteration. The existing app source remains the starting point, so switching models doesn't reset the app. Self-hosted operators can learn how to select an agent and provider in [data apps on self-hosted Lightdash](/self-host/enterprise-features/data-apps).
...
Data apps are enabled by default on Lightdash Cloud. If you're running a self-hosted enterprise instance, see [data apps on self-hosted Lightdash](/self-host/enterprise-features/data-apps) for the configuration you'll need to add — a [sandbox provider](/self-host/customize-deployment/sandboxes), a Claude provider, and an S3 bucket for the built artifacts.
