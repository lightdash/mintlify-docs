...
<Info>
  **Public Beta:** Data apps are currently in the Beta phase. Learn what this means in our Feature Maturity Levels guide: [Feature Maturity Levels](/help/feature-maturity-levels).
...
Data apps are one of several AI surfaces in Lightdash. To see how they compare with AI agents, MCP, and dashboards as code — and pick the right one for a task — see [Choosing your AI surface](/agents#choosing-your-ai-surface).
...
The Queries tab has an **Inspect data** toggle (the database-search icon in the panel's title bar) that turns on a two-way link between the inspector and what's rendered in the app:

- **Hover a query row** to highlight every data point in the preview that was rendered from that query - individual bars, table rows, KPI values, chart segments, and cells all light up together, so you can see exactly where a query's results end up in the UI.
- **Click** a data point or a query row to pin the selection so you can scroll or interact with both sides without losing the highlight; click again (or press `Esc`) to cancel.

Inspect data relies on lineage metadata that the agent emits during the build. If the toggle is disabled with a tooltip that says *"Inspect data is not available in this app version"*, the app was generated before lineage was introduced - regenerate the app (send any follow-up prompt, or ask the agent to rebuild) to enable it.
...
See [External connections](/data-apps/external-connections) for details.
...
See [Themes](/data-apps/customize-themes) for details.

## Chart types

Data apps also power reusable custom chart types, which are built and managed in the chart type gallery rather than here. See [Custom app-based charts](/explore/chart-types/custom-app-based-charts).
...
Your [project role](/workspace-admin/roles#project-roles-and-permissions) determines what you're allowed to do with data apps anywhere in the project:
...
If you're building in a [preview project](/workflow/preview-projects), you can promote a data app into the preview's upstream (production) project once it's ready. This mirrors how chart and dashboard [promotion](/explore/promote-content) works: you iterate safely in preview, then push a snapshot to production.
...
If you embed a data app in a [dashboard](/explore/promote-content#promote-dashboards) and promote the dashboard, every referenced app is promoted alongside it. Each app is created or updated in the upstream project using the same rules as standalone app promotion, and the tile is remapped to point at the upstream app so the production dashboard renders the production copy.

## Data apps as code

Data apps join charts and dashboards in the [as-code workflow](/workflow/cli/reference#lightdash-download). You can download a data app's source, check it into git, edit it locally, and re-upload it — the destination Lightdash instance rebuilds the app in its sandbox and publishes a new version. This lets you version app source, review changes in pull requests, and move apps between projects or instances.
...
- `lightdash-app.yml` — the manifest, including the `appUuid` that identifies the app when you re-upload it, plus editable `name` and `description` fields. For [app-based chart types](/explore/chart-types/custom-app-based-charts), the manifest also carries a `vizSchema` field describing the declared fields (dimensions, metrics, series) the chart type needs — the schema is stored in the database, not the source tree, so the CLI persists it here to round-trip a viz through download and upload.
...
App-based chart types follow the same loop: the download writes the chart type's declared field schema into `lightdash-app.yml`, and the upload sends it back so it shows up in the chart type picker on the destination. If you edit the manifest's `vizSchema` directly, the upload validates it and rejects malformed schemas.
...
- **Enterprise only.** Both the source and destination instances must have data apps enabled, including the build sandbox. See [Self-hosting Data apps](/data-apps/self-hosting) for the configuration required on self-hosted instances.
...
Data apps are enabled by default on Lightdash Cloud. If you're running a self-hosted enterprise instance, see [Self-hosting Data apps](/data-apps/self-hosting) for the configuration you'll need to add — an E2B sandbox key, an Anthropic API key, and an S3 bucket for the built artifacts.
