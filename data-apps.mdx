...
<Info>
  **Public Beta:** Data apps are currently in the Beta phase. Learn what this means in our Feature Maturity Levels guide: [Feature Maturity Levels](/support/feature-maturity-levels).
...
You can change the model on every prompt - including mid-iteration. The existing app source remains the starting point, so switching models doesn't reset the app. Self-hosted operators can learn how to select an agent and provider in [data apps on self-hosted Lightdash](/self-host/data-apps).
...
Refreshing also **invalidates the warehouse cache** for that app, so every query stays fresh for the rest of the session. See [Caching](/semantic-layer/caching#chart-and-dashboard-results-caching) for how results caching works across Lightdash.
...
An app can be sent on a schedule as CSV, XLSX, or an image, and it can keep a Google Sheet up to date. Both are set up from the **⋯** menu in the app header, and both work differently from a chart or dashboard delivery in one important way: an app doesn't declare its queries, so Lightdash renders it, captures every query it actually runs, and turns each one into its own file or tab.

See [Scheduled deliveries and syncs](/data-apps/deliveries-and-syncs) for details.
...
Data apps also power reusable custom chart types, which are built and managed in the chart type gallery rather than here. See [Custom project charts](/explore/chart-types/custom-project-charts).
...
You can download a data app's source, edit it in your own editor, preview it locally against real data, and upload new versions with the Lightdash CLI. This lets you version app source in git, review changes in pull requests, and move apps between projects or instances.

See [Data apps as code](/data-apps/download-data-apps-as-code) for the full workflow.
...
Data apps are enabled by default on Lightdash Cloud. If you're running a self-hosted enterprise instance, see [data apps on self-hosted Lightdash](/self-host/data-apps) for the configuration you'll need to add — a [sandbox provider](/self-host/customize-deployment/sandboxes), a Claude provider, and an S3 bucket for the built artifacts.
