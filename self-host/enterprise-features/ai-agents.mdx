...
`MANAGED_AGENT_VALIDATED_MODELS` defaults to `[]`, so no model is qualified for cleanup flagging or deletion. Operators can register exact provider/model pairs with a reviewed qualification for `observe`, `flag`, or `cleanup`. Duplicate qualifications use the most restrictive mode; malformed configuration rejects startup. Qualify a model on disposable representative content before increasing its permissions; use the exact IDs shown in Autopilot, including Azure deployment names or Bedrock inference-profile prefixes.
