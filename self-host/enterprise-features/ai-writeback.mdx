...
<Note>
  On Lightdash versions below **0.3340.0**, AI writeback is additionally gated
  by the `ai-writeback` feature flag, which is off by default — enable it for
  your organization, or upgrade. From 0.3340.0 the flag no longer exists and
...
Check that `LIGHTDASH_LICENSE_KEY` is set and the project's dbt connection is GitHub or GitLab with the matching Lightdash app installed. On versions below 0.3340.0, also check the `ai-writeback` feature flag is enabled for the organization.
