...
Add the following environment variables to your Lightdash deployment. The
`E2B_*` variables apply when `SANDBOX_PROVIDER` is `e2b` (the default); on the
other providers the writeback image is selected by that provider's own image
variables — see [sandboxes](/self-host/customize-deployment/sandboxes).
