...
| --- | --- | --- |
| `LIGHTDASH_ENABLE_FEATURE_FLAGS` | (unset) | Add `data-app-prompt-cache-1h` to run the coding agent with a 1-hour prompt cache instead of the provider's 5-minute default. A build that starts within an hour of the previous one on the same app reuses the cached conversation. Applies only to Claude through the Anthropic API (direct or gateway); Bedrock and Codex builds ignore it. Cache writes cost 2x a normal input token, and reads within the hour cost 10x less than re-sending the transcript, so this pays off when your users iterate on apps 5 to 60 minutes apart. Remove the flag to return to the 5-minute default on the next build. |
