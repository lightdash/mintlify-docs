...
Okta users will automatically be assigned to the same groups in Lightdash as they are in Okta if you have [configured Okta to share groups with Lightdash](/workspace-admin/manage-groups#using-okta-to-manage-groups-in-lightdash). To enable this functionality, ensure the following environment variable is set:

| Variable                 | Description                               | Required? |
| :------------------------ | :----------------------------------------- | :---------: |
| `AUTH_ENABLE_GROUP_SYNC` | If `"true"` enables group sync from Okta. |           |

read more about [Using OKTA to manage groups in Lightdash](/workspace-admin/manage-groups#using-okta-to-manage-groups-in-lightdash)
