...
description: "Enable password login and SSO with Okta, Google, Azure AD, OneLogin, or any OIDC provider"
boost: 0.001
doc-type: tutorial
---
...
**Lightdash Cloud users**: configure SSO yourself from **Organization settings → Single Sign-On** — see [Configure SSO](/workspace-admin/sso/setup). Use the provider-side steps on this page (creating an OAuth app, configuring redirect URIs, etc.) to obtain the values, then enter them in the organization settings UI instead of setting environment variables.

Cloud SSO also requires a [verified domain](/workspace-admin/sso/verified-domains). Saving provider credentials without a verified domain will not route users to that provider during sign-in.
</Note>
...
<Tip>
  **Lightdash Cloud users**: instead of setting these environment variables, enter the client ID, client secret, Okta domain, and issuer URI in **Organization settings → Single Sign-On → Okta**. See [Configure SSO](/workspace-admin/sso/setup#okta).

  Make sure your organization has a [verified domain](/workspace-admin/sso/verified-domains), otherwise Okta will not appear for users during sign-in.
</Tip>
...
| :----------------------------------- | :-------------------------------------------------------------------------------------------------- | :---------: |
| `AUTH_OKTA_DOMAIN`                  | The `{{ okta_domain }}`. Should not include `https://`                                             | <Icon icon="check"/>         |
| `AUTH_OKTA_OAUTH_CLIENT_ID`         | The **Client ID** copied from the application settings in okta                                     | <Icon icon="check"/>         |
| `AUTH_OKTA_OAUTH_CLIENT_SECRET`     | The **Client secret** copied from the application settings in okta                                 | <Icon icon="check"/>         |
| `AUTH_OKTA_OAUTH_ISSUER`            | The **Issuer URI** copied from the authorization server. Should include `https://`                 | <Icon icon="check"/>         |
...
<Warning>
  **This feature is deprecated and will be removed in a future release.**
  ...
  For more information on how to provision users and groups in Lightdash, see the [SCIM integration](/workspace-admin/sso/scim) documentation.
</Warning>

Okta users will automatically be assigned to the same groups in Lightdash as they are in Okta if you have [configured Okta to share groups with Lightdash](/workspace-admin/sso/scim#group-role-provisioning). To enable this functionality, ensure the following environment variable is set:

| Variable                 | Description                               | Required? |
| :------------------------ | :----------------------------------------- | :---------: |
| `AUTH_ENABLE_GROUP_SYNC` | If `"true"` enables group sync from Okta. |           |

read more about [Using OKTA to manage groups in Lightdash](/workspace-admin/sso/scim#group-role-provisioning)
...
| Variable                           | Description                                 | Required? | Default |
| :---------------------------------- | :------------------------------------------- | :---------: | :-------: |
| `AUTH_GOOGLE_ENABLED`                  | Required to be set to `true` for Google SSO                                                                                            | <Icon icon="check"/>         |         |
| `AUTH_GOOGLE_OAUTH2_CLIENT_ID`         | Required see instructions above                                                                                                        | <Icon icon="check"/>         |         |
| `AUTH_GOOGLE_OAUTH2_CLIENT_SECRET`     | Required see instructions above                                                                                                        | <Icon icon="check"/>         |         |
...
If you use [BigQuery SSO](/workspace-admin/sso#warehouse-sso-enterprise-only) to give users per-user warehouse credentials, set `AUTH_GOOGLE_INCLUDE_BIGQUERY_SCOPE=true` to request the BigQuery scope during the initial Google login. Users will complete one consent screen that covers both Lightdash login and BigQuery warehouse access instead of two separate OAuth flows.
...
<Tip>
  **Lightdash Cloud users**: instead of setting these environment variables, enter the client ID, client secret, and issuer URL in **Organization settings → Single Sign-On → OneLogin**. See [Configure SSO](/workspace-admin/sso/setup#onelogin).

  Make sure your organization has a [verified domain](/workspace-admin/sso/verified-domains), otherwise OneLogin will not appear for users during sign-in.
</Tip>
...
| Variable                             | Description                | Required? | Default |
| :------------------------------------ | :-------------------------- | :---------: | :-------: |
| `AUTH_ONE_LOGIN_OAUTH_CLIENT_ID`     | Required for One Login SSO | <Icon icon="check"/>         |         |
| `AUTH_ONE_LOGIN_OAUTH_CLIENT_SECRET` | Required for One Login SSO | <Icon icon="check"/>         |         |
| `AUTH_ONE_LOGIN_OAUTH_ISSUER`        | Required for One Login SSO | <Icon icon="check"/>         |         |
...
<Tip>
  **Lightdash Cloud users**: instead of setting these environment variables, enter the client ID, client secret, and tenant ID in **Organization settings → Single Sign-On → Azure AD**. See [Configure SSO](/workspace-admin/sso/setup#azure-active-directory).

  Make sure your organization has a [verified domain](/workspace-admin/sso/verified-domains), otherwise Azure AD will not appear for users during sign-in.
</Tip>
...
| Variable                               | Description           | Required? | Default |
| :-------------------------------------- | :--------------------- | :---------: | :-------: |
| `AUTH_AZURE_AD_OAUTH_CLIENT_ID`        | Required for Azure AD | <Icon icon="check"/>         |         |
| `AUTH_AZURE_AD_OAUTH_CLIENT_SECRET`    | Required for Azure AD | <Icon icon="check"/>         |         |
| `AUTH_AZURE_AD_OAUTH_TENANT_ID`        | Required for Azure AD | <Icon icon="check"/>         |         |
...
<Tip>
  **Lightdash Cloud users**: instead of setting these environment variables, enter the client ID, client secret, and metadata document URL in **Organization settings → Single Sign-On → OpenID Connect**. See [Configure SSO](/workspace-admin/sso/setup#openid-connect).

  Make sure your organization has a [verified domain](/workspace-admin/sso/verified-domains), otherwise OpenID Connect will not appear for users during sign-in.
</Tip>
...
| Variable                          | Description                                                                                                                        | Required? | Default               |
| :--------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------- | :---------: | :---------------------: |
| `AUTH_OIDC_CLIENT_ID`             |                                                                                                                                    | <Icon icon="check"/>         |                       |
| `AUTH_OIDC_CLIENT_SECRET`         | Required unless AUTH\_METHOD is `private_key_jwt`                                                                                  |           |                       |
| `AUTH_OIDC_METADATA_DOCUMENT_URL` | [URL to OIDC metadata discovery endpoint](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfigurationRequest) | <Icon icon="check"/>         |                       |
