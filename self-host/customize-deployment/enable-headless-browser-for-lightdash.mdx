...
description: "Run the headless browser that renders chart and dashboard images for email and Slack deliveries"
...
<Frame>
  ![Diagram: scheduled deliveries and Slack unfurls trigger image generation in Lightdash, which opens a socket to the headless browser; the browser loads the chart page and returns a screenshot for email or Slack](/images/self-host/customize-deployment/enable-headless-browser-for-lightdash/headless-browser-schema-62b496e0d9f5f705ae823c7d4fdec946.png)
</Frame>
...
The default `ghcr.io/browserless/chromium` image does not trust self-signed certificates out of the box, so without this flag you will see `net::ERR_CERT_AUTHORITY_INVALID` from Playwright and `DEPTH_ZERO_SELF_SIGNED_CERT` from the backend's own internal fetch.
