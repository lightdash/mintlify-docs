...
By default, query results use the main S3 configuration. To store query results with a different S3-compatible provider, set the `RESULTS_S3_*` variables that differ from your main configuration:
...
RESULTS_S3_ACCESS_KEY=AKIA...
RESULTS_S3_SECRET_KEY=...
RESULTS_S3_FORCE_PATH_STYLE=false
```
