...
- **Data-apps bucket** (if using [data apps](/self-host/data-apps)) — persistent, with **no** delete lifecycle.

**Block all public access** on every bucket. Prefer **IAM roles** (IRSA on EKS, Workload Identity on GKE) over static keys — noting that signed URLs are then capped at 7 days.
