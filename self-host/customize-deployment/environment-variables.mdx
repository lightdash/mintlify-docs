...
| `S3_USE_CREDENTIALS_FROM` | Configures the credential provider chain for AWS S3 authentication if access key and secret is not provided. Supports: `env` (environment variables), `token_file` (token file credentials), `ini` (initialization file credentials), `ecs` (container credentials — covers both ECS task roles and EKS Pod Identity), `ec2` (instance metadata credentials). Multiple values can be specified in order of preference.                                                                                                                                                                                                                                                                                                                                                                                                                     |
