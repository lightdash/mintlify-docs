...
These variables enable you to configure the [AI Analyst functionality](/agents). Note that you will need an **Enterprise Licence Key** for this functionality.
...
| `SLACK_MULTI_AGENT_CHANNEL_ENABLED` | Enables the [multi-agent Slack channel (Beta)](/agents/set-up-agents#setting-up-a-multi-agent-slack-channel) feature for the whole instance (default=false) |
...
Configure the sandboxed runtime that powers [data apps](/self-host/data-apps). Sandboxes execute through [E2B](https://e2b.dev/).
...
Configure the Lightdash [Model Context Protocol](/self-host/customize-deployment/configure-mcp-for-lightdash) server.
