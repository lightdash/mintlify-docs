...
| `AUTH_ENABLE_OIDC_LINKING`             | Enables linking a new OIDC identity to an existing user if they already have another OIDC with the same email                                |           | `false` |
| `AUTH_ENABLE_OIDC_TO_EMAIL_LINKING`    | Enables linking OIDC identity to an existing user by email. Required when using [SCIM](/references/scim-integration) with SSO                |           | `false` |
