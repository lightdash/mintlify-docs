...
| `AUTH_ENABLE_OIDC_TO_EMAIL_LINKING`       | Enables linking OIDC identity to an existing user by matching their full email address. Required when using [SCIM](/workspace-admin/scim-integration) with SSO (default=false) |
...
Configure the sandboxed runtime that powers [data apps](/data-apps/self-hosting). Sandboxes execute through [E2B](https://e2b.dev/).
...
Group bindings are resolved **by group name** because group UUIDs aren't known at deploy time — groups provisioned via [SCIM](/workspace-admin/scim-integration) are created by your IdP, not by Lightdash.
...
This is especially useful for instances that provision groups via [SCIM](/workspace-admin/scim-integration): SCIM group UUIDs aren't known at deploy time, so this lets you declare role mappings by group name once and have them applied automatically as groups sync from your IdP.
...
- **Groups** are resolved by `groupName` because group UUIDs aren't known at deploy time when groups are provisioned via [SCIM](/workspace-admin/scim-integration).
