...
| `AUTH_ENABLE_OIDC_TO_EMAIL_LINKING`       | Enables linking OIDC identity to an existing user by matching their full email address. Required when using [SCIM](/references/workspace/scim-integration) with SSO (default=false) |
...
| `AI_COPILOT_THREAD_DUMP_ENABLED`          | Enables the [AI thread debug dump](/self-host/customize-deployment/ai-thread-debug-dump) download action for organization AI admins (default=false)      |
