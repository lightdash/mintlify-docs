...
| `MANAGED_AGENT_VALIDATED_MODELS` | JSON array of exact `{ "provider", "model", "mode" }` qualifications. Mode: `observe`, `flag`, or `cleanup`. Default: `[]` (no cleanup flagging or deletion qualification). See [model qualification](/self-host/enterprise-features/ai-agents#model-qualification-and-cleanup). |
