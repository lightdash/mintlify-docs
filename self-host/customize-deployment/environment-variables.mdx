...
When `PERSISTENT_DOWNLOAD_URLS_ENABLED` is set to `true`, CSV and dashboard ZIP exports return a stable Lightdash-hosted URL (e.g. `https://lightdash.example.com/api/v1/file/{id}`) Each time this URL is accessed, Lightdash generates a short-lived S3 signed URL and redirects to it. This means:
