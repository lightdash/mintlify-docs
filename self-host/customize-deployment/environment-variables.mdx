...
description: "Complete reference of the environment variables that configure a Lightdash deployment"
mode: "wide"
boost: 0.001
doc-type: reference
---
...
| `ALLOW_MISSING_MIGRATIONS` | When `true`, disables Knex's migration list validation so Lightdash can start against a database that has migrations applied which don't exist as files on disk. Use this during rollbacks (e.g. v2 → v1) when the database stays on the newer version. Pending migrations (code ahead of DB) are still detected. (default=false) |
...
| Variable                | Description                                                                                                                                                                                                                                                                                                              |
| :---------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LIGHTDASH_LICENSE_KEY` | License key for Lightdash Enterprise Edition. See [enterprise features](/self-host/enterprise-features) for details. [Get your license key](https://calendly.com/lightdash-cloud/enterprise?utm_source=docs&utm_medium=referral&utm_campaign=enterprise_licensing&utm_content=license_key_cta) |
...
| `SCHEDULER_SHUTDOWN_TIMEOUT`     | How many milliseconds a stopping scheduler worker waits for in-flight jobs to finish before releasing them for retry. Keep it below your pod's termination grace period. (default=60000, 1 minute) |
...
| `LIGHTDASH_ENABLE_TIMEZONE_SUPPORT`     | Enables per-query timezone support. On by default; set to `false` to turn it off instance-wide.   |
...
[Map charts](/visualizations/map) use CARTO for the Light, Dark, and Voyager backgrounds, including the Voyager fallback when OpenStreetMap tiles fail. Lightdash requests these tiles through its backend and keeps the API key out of the browser. OpenStreetMap and Satellite backgrounds do not use this key.

| Variable | Description |
| :--- | :--- |
| `CARTO_API_KEY` | CARTO **Basemaps** API key. Optional; unset by default. Without a key, Lightdash still requests anonymous tiles, which can display CARTO's "API key required" watermark. |

To configure a key:

1. [Request a free CARTO Basemaps API key](https://carto.com/basemaps/apikey/). Complete the form with your email and project details; CARTO emails the key to you. Review the usage limits and terms linked on that page.
2. Set `CARTO_API_KEY` in the environment of every Lightdash backend replica using your deployment's secret management. For example, `CARTO_API_KEY=your-carto-basemaps-api-key`. If you restrict the key, allow your backend's outbound public IP addresses: tile requests originate from the server, so website-only restrictions do not authorize these requests.
3. Restart or roll out the backend, then open a map with a Light, Dark, or Voyager background. The map should render without the watermark. If an old watermark remains, force-refresh the browser to discard cached tiles.

For Helm deployments, you can reference an existing Kubernetes Secret. Create a Secret named `lightdash-carto` in the release's namespace with a `CARTO_API_KEY` entry, then add this to your Helm values:
...
extraEnv:
  - name: CARTO_API_KEY
    valueFrom:
      secretKeyRef:
        name: lightdash-carto
        key: CARTO_API_KEY
```

Allow outbound HTTPS from the backend to `basemaps.cartocdn.com`. CARTO's usage limits still apply; the backend proxy does not remove provider quotas.
...
| `EXTENDED_USAGE_ANALYTICS`                   | When `true`, adds "Dashboard views (top 20)" and "Chart views (top 20)" tables to the [User Activity dashboard](/workspace-admin/usage-analytics#user-activity-dashboard) in Project Settings > Usage Analytics. (default=false) |
...
| `AUTH_ENABLE_OIDC_TO_EMAIL_LINKING`       | Enables linking OIDC identity to an existing user by matching their full email address. Required when using [SCIM](/workspace-admin/sso/scim) with SSO (default=false) |
...
| `S3_AUTH_MODE`            | How Lightdash authenticates requests to the bucket. `default` (the default value) signs requests with SigV4 and AWS credentials; use it for AWS S3, MinIO, and Google Cloud Storage HMAC keys. `gcp_oauth` sends a Google OAuth bearer token instead, which lets Lightdash reach Google Cloud Storage from a GKE workload identity service account with no static keys. See [external storage](/self-host/customize-deployment/configure-lightdash-to-use-external-object-storage#option-b-workload-identity-with-no-keys). One value applies to every storage bucket. |
| `S3_USE_CREDENTIALS_FROM` | Configures the credential provider chain for **AWS** authentication when you do not set an access key and secret. Accepts `env` (environment variables), `token_file` (token file credentials), `ini` (initialization file credentials), `ecs` (container metadata credentials), and `ec2` (instance metadata credentials). List several values in order of preference. Every source is an AWS credential provider, so none of them works on Google Cloud. To authenticate to Google Cloud Storage without keys, set `S3_AUTH_MODE` instead. |
...
| `RESULTS_S3_ENDPOINT`     | S3 endpoint for the query results bucket (default=S3_ENDPOINT)                                                                                                                                                                                                                                                                                                                |
...
| `RESULTS_S3_FORCE_PATH_STYLE` | Force path style addressing for the query results bucket (default=S3_FORCE_PATH_STYLE)                                                                                                                                                                                                                                                                                    |
...
The variables below act as instance-wide defaults. Organization admins can override them per organization (and per channel) from **Settings → Organization → Exporting**. See [Configuring download link expiration](/explore/create-scheduled-deliveries#configuring-download-link-expiration).
...
<a id="analytics--event-tracking" />

## Analytics & Event Tracking

RudderStack product telemetry is enabled by default on self-hosted deployments and points at `https://analytics.lightdash.com`. For the full payload, disable and redirect options, and the security-review checklist, see [Data flows and telemetry](/self-host/data-flows-and-telemetry).

| Variable                         | Description                                                                                        |
| :------------------------------- | :------------------------------------------------------------------------------------------------- |
| `RUDDERSTACK_WRITE_KEY`          | RudderStack key used to track events (by default Lightdash's key is used). See [redirect telemetry to your own RudderStack](/self-host/data-flows-and-telemetry#configuration-3-send-telemetry-to-your-own-rudderstack). |
...
| `RUDDERSTACK_ANALYTICS_DISABLED` | Set to `true` to stop the backend and browser from sending any RudderStack events. Takes precedence over `RUDDERSTACK_WRITE_KEY` and `RUDDERSTACK_DATA_PLANE_URL`. See [disable outbound telemetry](/self-host/data-flows-and-telemetry#configuration-2-disable-outbound-rudderstack-telemetry). |

## AI Analyst

Enterprise-only. For setup steps (which provider to pick, how to point at an LLM gateway, how to enable verified answers), see [AI agents](/self-host/enterprise-features/ai-agents). The tables below are the exhaustive variable reference.
...
| `AI_COPILOT_THREAD_DUMP_ENABLED`          | Enables the [AI thread debug dump](/support/ai-thread-debug-dump) download action for organization AI admins (default=false)                             |
...
| `AI_AGENT_MCP_ALLOW_PRIVATE_ADDRESSES`    | When `true`, allows AI agent [MCP server](/agents/connect-external-mcp-servers) URLs to point at private/internal network addresses (e.g. `127.0.0.1`, RFC1918 ranges). Useful for self-hosted deployments where MCP servers run inside the same private network. Leave unset or `false` to enforce public-address-only validation (default=false). |

The AI Analyst supports multiple providers. Provide credentials for the provider set in `AI_DEFAULT_PROVIDER`. OpenAI and Anthropic are the most tested. For which provider to pick and end-to-end setup, see [AI agents](/self-host/enterprise-features/ai-agents).
...
| `BEDROCK_USE_DEFAULT_CREDENTIALS` | Set to `true` to use the standard AWS credential chain instead of an API key or static credentials. |
...
| `BEDROCK_BASE_URL`          | Optional HTTP(S) base URL for a Bedrock-compatible gateway. Routes backend Bedrock calls and Bedrock-backed Data app coding agents through the gateway. |
| `CLAUDE_CODE_SKIP_BEDROCK_AUTH` | Set to `true` or `1` to omit AWS authentication from Claude Code when `BEDROCK_BASE_URL` points to a gateway that performs upstream authentication. The backend still requires a supported Bedrock credential mode. |
...
<Note>
  Exact model snapshots are automatically assigned (e.g., `gpt-5.1` → `gpt-5.1-2025-11-13`). For Bedrock, a region prefix is added on top of that based on `BEDROCK_REGION` (e.g., `claude-sonnet-4-5` → `us.anthropic.claude-sonnet-4-5-20250929-v1:0`).
</Note>

## Slack Integration

These variables enable you to configure the [Slack integration](/integrations/slack).
...
| `SLACK_MULTI_AGENT_CHANNEL_ENABLED` | Enables [multi-agent Slack channels](/self-host/enterprise-features#multi-agent-slack-channels) for the whole instance (default=false) |
| `SLACK_LINK_SHARE_IMAGE_PREVIEW_ENABLED` | Enables image previews when Lightdash links are unfurled in Slack. (default=true)                                                                              |

## dbt

Configure how Lightdash runs the dbt subprocess during project compilation.

| Variable | Description |
| --- | --- |
| `ALLOW_DBT_COMMANDS_ACCESS_TO_ENV_VARS` | Comma-separated list of environment variable names from the Lightdash server's environment to pass through to the dbt subprocess. Use this when your dbt project reads values with `env_var()` (for example in macros) that you want to source from the host machine instead of setting per-project in Lightdash. By default the dbt subprocess does not inherit the server's environment. Only the names on this allowlist are forwarded. |
...
Enterprise-only. For setup steps and how the allow-list of iframe origins works, see [embedding](/self-host/enterprise-features#embedding).
...
Enterprise-only. See [custom roles](/self-host/enterprise-features#custom-roles) for setup.
...
Enterprise-only. See [service accounts](/self-host/enterprise-features#service-accounts) for setup.
...
Enterprise-only. See [SCIM](/self-host/enterprise-features#scim) for setup, including the required `AUTH_ENABLE_OIDC_TO_EMAIL_LINKING` companion when combining SCIM with SSO.
...
Enterprise-only. For setup steps and the required pre-aggregate worker deployment, see [pre-aggregates](/self-host/enterprise-features/pre-aggregates).
...
Enterprise-only. Sandboxes execute through [E2B](https://e2b.dev/). For setup steps (coding-agent choice, model provider, S3 bucket, sandbox provider), see [data apps](/self-host/enterprise-features/data-apps).
...
| `APP_RUNTIME_EXTERNAL_CONNECTION_ALLOWED_PRIVATE_HOST_CIDRS` | Comma-separated `hostname@CIDR` exceptions allowing external connections, connection tests, and OAuth client-credentials token requests to reach trusted private addresses. Both the hostname and CIDR must match. Default: empty; private addresses remain blocked. Instance-wide; requires a backend restart. See [private-network external connections](/self-host/enterprise-features/data-apps#private-network-external-connections) for syntax, prerequisites, and security boundaries. |
...
Enterprise-only scheduled project maintenance. Enable AI Analyst, configure a provider at the instance or organization level, and enable the `ai-autopilot` feature flag. Apply the same environment configuration to API and scheduler processes and restart both after changes. See [Autopilot setup](/self-host/enterprise-features/ai-agents#autopilot).

| Variable | Description |
| :--- | :--- |
| `MANAGED_AGENT_SCHEDULE` | Fallback cron schedule when no project schedule is selected. Default: `0 0 * * *`. |
| `MANAGED_AGENT_SESSION_TIMEOUT_MS` | Per-run deadline in milliseconds. Default: `600000` (10 minutes). Cancellation is cooperative; an in-progress write may finish after the deadline. |
| `MANAGED_AGENT_MAX_STEPS` | Maximum model steps per run. Default: `120`. Reaching the cap ends the run with an error and preserves recorded actions and any captured summary. |
| `MANAGED_AGENT_VALIDATED_MODELS` | JSON array of exact `{ "provider", "model", "mode" }` qualifications. Mode: `observe`, `flag`, or `cleanup`. Default: a built-in list of recent Claude Opus, Claude Sonnet and GPT models qualified for `cleanup`. Setting the variable replaces that list; an empty value qualifies no model. See [model qualification](/self-host/enterprise-features/ai-agents#model-qualification-and-cleanup). |
| `MANAGED_AGENT_RUNTIME` | Releases 2.255.0 to 2.261.x only: `ai-sdk` (default) selects the AI SDK runner; `anthropic-managed` selects the hosted runner. Ignored from 2.262.0, which removed the hosted runtime and logs a startup warning when the variable is set. |

The hosted-runtime credential and remote skill-list settings are ignored on AI SDK-only releases. Autopilot uses the shared [AI provider settings](#ai-analyst) instead. See [upgrade guidance](/self-host/enterprise-features/ai-agents#upgrading-from-the-hosted-runtime) before changing runtime or removing rollback credentials.

## MCP

Enterprise-only. For setup steps, connecting AI assistants, and network requirements, see [MCP](/self-host/enterprise-features/mcp).

<McpEnv />

## Metrics SQL API

Postgres wire protocol endpoint for the semantic layer. See [Metrics SQL API](/integrations/metrics-sql-api).
...
| `PGWIRE_SSL_CERT_PATH` | Path to the PEM server certificate (leaf first, then chain) for the Metrics SQL API. Required when `PGWIRE_PORT` is set, unless TLS is explicitly disabled. The file is watched, so certificate renewals apply without a restart. |
| `PGWIRE_SSL_KEY_PATH` | Path to the PEM private key for the Metrics SQL API certificate. Required alongside `PGWIRE_SSL_CERT_PATH`. |
| `PGWIRE_SSL_MODE` | `require` (default) or `disabled`. With `require`, the server terminates TLS itself and rejects plaintext connections before any credentials are exchanged; Lightdash refuses to start if `PGWIRE_PORT` is set without a certificate. Set `disabled` only on fully trusted networks or behind a Postgres-aware TLS-terminating proxy — tokens are sent in plaintext. |

## NATS worker

Optional NATS-backed query workers. When enabled, Lightdash routes query execution to dedicated [NATS worker pods](/self-host/nats-workers) instead of running queries on the main API server. Enable at least one warehouse worker to process the routed queries.

| Variable           | Description                                                                                                                                        |
| :----------------- | :------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NATS_ENABLED`     | Routes query execution through NATS worker pods. Requires at least one [warehouse worker](/self-host/nats-workers/warehouse-workers) to process the queries. (default=false) |
...
## Initialize instance <Badge icon="building-plus" color="blue" size="sm" shape="pill">Enterprise</Badge>

When a new Lightdash instance is created, and there are no orgs and projects. You can initialize a new org and project using ENV variables to simplify the deployment process. See the [pricing page](https://www.lightdash.com/pricing) for what each plan includes. Only Databricks project types and GitHub dbt configuration are supported.

Initialization is skipped once an organization and project exist — on subsequent restarts, only the [update instance](#update-instance) call is made.

<Warning>
  If you have multiple organizations or projects, the update call fails and the instance will not start. Remove the variables listed in [Update instance](#update-instance) to restart safely.
</Warning>
...
Multi-project setup supports Databricks project types and GitHub dbt configuration.
...
Set `LD_SETUP_USER_ATTRIBUTES` to a JSON array to declare [user attributes](/workspace-admin/user-attributes) and their group-value mappings as instance config. Lightdash reconciles the array on every deploy, so you no longer need a manual post-deploy API or CLI step to (re)apply attributes after a release.
...
| `groups`           | No       | Array of group mappings. Each mapping is `{ "group": "<group name>", "values": ["<string>", ...] }` and assigns those values to every member of a [group](/workspace-admin/manage-groups). The legacy single-value form `{ "group": "...", "value": "<string>" }` is still accepted but deprecated. Defaults to `[]`. |
...
Group bindings are resolved **by group name** because group UUIDs aren't known at deploy time — groups provisioned via [SCIM](/workspace-admin/sso/scim) are created by your IdP, not by Lightdash.
...
<a id="declare-group--project-access" />

## Declare group → project access

Set `LD_SETUP_GROUP_PROJECT_ACCESS` to a JSON array to declare [group → project role assignments](/workspace-admin/roles) as instance config. Lightdash reconciles the array on every deploy, so you no longer need to re-apply group access through the UI or API after each release. This is the project-access equivalent of [`LD_SETUP_USER_ATTRIBUTES`](#declare-user-attributes).

<Info>
  **Group project access setup is only available on Lightdash Enterprise plans** and uses [groups](/workspace-admin/manage-groups) and (optionally) [custom roles](/workspace-admin/custom-roles), which are Enterprise features.
...
This is especially useful for instances that provision groups via [SCIM](/workspace-admin/sso/scim): SCIM group UUIDs aren't known at deploy time, so this lets you declare role mappings by group name once and have them applied automatically as groups sync from your IdP.
...
| Field         | Required                       | Description                                                                                                                                                                                            |
| :------------ | :----------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `groupName`   | Yes                            | Name of the [group](/workspace-admin/manage-groups) to grant access to. Resolved by name so it is compatible with SCIM-synced groups.                                                                    |
...
| `role`        | Yes                            | A [system role](/workspace-admin/roles) (`viewer`, `interactive_viewer`, `editor`, `developer`, or `admin`) or the name of a [custom role](/workspace-admin/custom-roles).                   |
...
- **Groups** are resolved by `groupName` because group UUIDs aren't known at deploy time when groups are provisioned via [SCIM](/workspace-admin/sso/scim).
- **Projects** are resolved by `projectUuid` when set, otherwise by `projectName`.
- **Roles** are resolved as a system role first; if the value doesn't match a system role, Lightdash looks up a [custom role](/workspace-admin/custom-roles) with that name.
