...
| `OPENAI_SUPPORTS_STREAMING` | Set to `false` to disable streaming completions for gateways that don't support SSE. Each agent step is sent as a single non-streaming request and replayed as a simulated stream. (default=true) |
...
If your gateway rejects streaming (SSE) completions, also set `OPENAI_SUPPORTS_STREAMING=false`. The AI Analyst will send each step as a single non-streaming request — the chat UI and SSE delivery to the browser are unaffected. Make sure your reverse-proxy idle timeouts (e.g. nginx `proxy_read_timeout`) are comfortably above the expected single-completion time, since no bytes are sent within a step until the request returns.
</Tip>
...
| `ANTHROPIC_SUPPORTS_STREAMING`   | Set to `false` to disable streaming completions for gateways that don't support SSE. (default=true) |
...
| `AZURE_AI_SUPPORTS_STREAMING`    | Set to `false` to disable streaming completions for Azure deployments that don't support SSE. (default=true) |
...
| `OPENROUTER_SUPPORTS_STREAMING` | Set to `false` to disable streaming completions for gateways that don't support SSE. (default=true) |
...
| `BEDROCK_SUPPORTS_STREAMING` | Set to `false` to disable streaming completions for Bedrock endpoints that don't support SSE. (default=true) |
