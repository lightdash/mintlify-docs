...
| `NODE_ENV`                                   | Node runtime environment (`production`, `development`, `test`). Affects logging and dev-only behavior.                                                                                                                                                                                                                      |
| `LIGHTDASH_INSTALL_ID`                       | Stable identifier reported with anonymous usage analytics so multiple instances aren't double-counted.                                                                                                                                                                                                                      |
| `LIGHTDASH_INSTALL_TYPE`                     | Free-form label for how the instance was installed (e.g. `docker-compose`, `helm`, `heroku`). Reported with anonymous usage analytics.                                                                                                                                                                                      |
...
| `EXPERIMENTAL_CACHE`                         | Enables experimental in-process cache for query metadata. Use only in dev/test. (default=false)                                                                                                                                                                                                                             |
| `ANALYTICS_EMBED_SECRET`                     | Secret used to sign tokens for the embedded built-in analytics dashboard.                                                                                                                                                                                                                                                   |
| `DEV_SCOPED_COOKIE_NAMES_ENABLED`            | When `true`, cookie names are suffixed per dev environment so multiple local instances on the same host don't collide. Dev only. (default=false)                                                                                                                                                                            |
| `ALLOW_MISSING_MIGRATIONS`                   | Skips the boot check that errors when applied DB migrations don't match the codebase. (default=false). Use only when intentionally running an older binary against a newer schema.                                                                                                                                          |
...
| `SENTRY_SPOTLIGHT`              | When `true`, forwards backend errors to a local [Spotlight](https://spotlightjs.com/) instance for development debugging. (default=false) |
...
| `E2B_TEMPLATE_NAME`               | E2B template used for general data app sandboxes. (default=`lightdash-data-app`)                                                                      |
