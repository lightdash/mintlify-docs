...
| `LIGHTDASH_CORS_ENABLED`         | Enables Cross-Origin Resource Sharing (CORS) (default=true). Set to `false` to disable CORS at the instance level.              |
| `LIGHTDASH_CORS_ALLOWED_DOMAINS` | Instance-wide list of domains allowed to make cross-origin requests. Values must be separated by commas. Admins can also manage allowed origins per organization from **Project settings → Embed configuration → CORS** in the UI. |
