...
| `LIGHTDASH_PROMETHEUS_HTTP_METRICS_ENABLED`      | Enables the OpenTelemetry `http.server.request.duration` histogram (semconv labels, route templates) on the `/metrics` endpoint. (default=false) |
