...
description: "Expose Prometheus metrics for instance health, plus custom counters driven by application events"
boost: 0.001
doc-type: reference
---
...
Operator-configurable counter metrics driven by application events are set through `LIGHTDASH_CUSTOM_METRICS_CONFIG_PATH` — see [Custom event metrics](#custom-event-metrics) below.
