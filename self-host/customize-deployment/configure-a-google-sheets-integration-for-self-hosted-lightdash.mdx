...
In this guide we will show you how you can [upload to Google Sheets in scheduled deliveries](/references/google-sheets) and upload results from Google Sheets on `charts` on your on self-hosted Lightdash server.

### Enable Google SSO[](#enable-google-sso "Direct link to Enable Google SSO")

To authenticate Google users, first you need to enable [Google SSO](self-host/customize-deployment/use-sso-login-for-self-hosted-lightdash#google) on your server.
...
### Create an API key for Google file picker[](#create-an-api-key-for-google-file-picker "Direct link to Create an API key for Google file picker")
