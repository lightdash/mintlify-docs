---
title: Sandboxes
description: "How Lightdash isolates LLM-generated code, and how to configure the sandbox provider"
...
[Data apps](/self-host/enterprise-features/data-apps) can route Claude or Codex model traffic through a corporate LLM gateway. The sandbox connects to that gateway independently from the Lightdash backend, so both network paths must be allowed.

- **E2B and Azure Sandboxes** receive the hostname from `ANTHROPIC_BASE_URL`, `BEDROCK_BASE_URL`, or `OPENAI_BASE_URL` in their per-sandbox egress allowlist.
- **AWS Lambda MicroVMs** cannot apply Lightdash's per-sandbox allowlist. If you use a restricted `LAMBDA_MICROVM_EGRESS_CONNECTOR_ARN`, configure that connector to permit the gateway hostname.
- **Docker and Google Cloud Run** use their existing runtime network path. Make sure its DNS and firewall policy can reach the gateway.

These settings affect Data app coding agents, not AI writeback. If AI writeback is enabled, its sandbox still needs its existing Anthropic and git-host egress.

See [Corporate LLM gateways](/self-host/enterprise-features/ai-agents#corporate-llm-gateways) for provider configuration and the required API paths.
...
- **Two MicroVM images** — one for data app generation and one for AI writeback (they
  ...
  below. The data app Dockerfile copies `lightdash-query-sdk.tgz`, a generated artifact
  that a fresh checkout does not contain — build and pack it into `sandboxes/data-apps/`
  first, as shown in [Build the gateway image](#2-build-the-gateway-image) under the
  Cloud Run provider. The writeback Dockerfile has no such prerequisite.
...
- **A disk image per group** — build the two images from the Dockerfiles in the Lightdash
  ...
  there is no in-VM agent to build — Sandboxes expose a native command/file API.) The data
  app Dockerfile copies `lightdash-query-sdk.tgz`, a generated artifact that a fresh
  checkout does not contain — build and pack it into `sandboxes/data-apps/` first, as
  shown in [Build the gateway image](#2-build-the-gateway-image) under the Cloud Run
  provider. The writeback Dockerfile has no such prerequisite.
...
- Node and pnpm at the repository-pinned versions (`.nvmrc` and the `packageManager`
  field in `package.json`), to build the query SDK.
...
<Note>
Newer Lightdash releases include a build script that does all of this in one step. If
`sandboxes/data-apps/build-cloud-run-image.sh` exists in your checkout, prefer it over
the manual recipe below — it prepares the SDK tarball and pins the gateway version for
you:

```bash
./sandboxes/data-apps/build-cloud-run-image.sh <image-tag> --push
```

If your copy of the script still assigns `/app` to `user` with `chown`, use the
corrected manual recipe below instead.
</Note>

The Docker build context copies `lightdash-query-sdk.tgz`, a generated artifact that a
fresh checkout does not contain, so the recipe builds and packs it first. Use the
repository-pinned toolchain: the Node version in `.nvmrc` and the pnpm version in the
`packageManager` field of `package.json` (for example, `nvm use` and `corepack enable`
...
# Build and pack @lightdash/query-sdk — the Dockerfile copies this tarball
pnpm -C ../../packages/query-sdk build
pnpm -C ../../packages/query-sdk pack --pack-destination "$(pwd)"
mv lightdash-query-sdk-*.tgz lightdash-query-sdk.tgz

# Stage the Docker context with the gateway
STAGE=$(mktemp -d)
...
# Create E2B's user account, but keep /app owned by the root gateway user
# so the Cloud Run sandbox can write to it.
...
Instead of building the SDK from your local source, you can download the published
tarball from npm. Pin it to the version in your checkout so the sandbox template and SDK
stay in step — don't use an unpinned `latest`. This replaces `pnpm install` and the two
`pnpm` commands above (run it from `sandboxes/data-apps`):

```bash
SDK_VERSION=$(node -p "require('../../packages/query-sdk/package.json').version")
npm pack "@lightdash/query-sdk@${SDK_VERSION}"
mv "lightdash-query-sdk-${SDK_VERSION}.tgz" lightdash-query-sdk.tgz
```

This downloads a prebuilt archive from the npm registry — it does not build your local
source, so any local changes under `packages/query-sdk/` are not included. If the version
in your checkout hasn't been published yet (for example, on an unreleased development
branch), build from source instead.
...
service. The gateway image must keep `/app` owned by root. If an older build recipe
assigned it to `user`, data app builds can fail with `Permission denied` for
`/app/src/design`. Rebuild with the corrected recipe above and redeploy the gateway;
upgrading Lightdash alone does not update the gateway image.
