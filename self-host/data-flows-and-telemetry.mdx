...
- **While telemetry is enabled, each user chooses between anonymized and standard telemetry.** Anonymized users are tracked under a random UUID with no email or name; standard users' events also carry their email, first name, and last name. See [User-level tracking anonymization](#user-level-tracking-anonymization).
...
The user identifier is the Lightdash `userId` (a random UUID, not derived from the user's name or email), the constant string `"embed"` for embedded viewers, or the anonymous ID `00000000-0000-0000-0000-000000000000` for events with no user attached. Users who enable tracking anonymization keep their `userId` on events; anonymization removes their email and name, not the identifier (see [User-level tracking anonymization](#user-level-tracking-anonymization)).
...
While telemetry is enabled, each user chooses one of two tracking modes:

- **Anonymized telemetry** — the user is tracked under their `userId` only, a random UUID that identifies the account without revealing who the person is. Usage events are still sent, but identify calls and the `user.updated`, `user.verified`, and `user.deleted` events drop `email`, `firstName`, and `lastName`, and carry the `is_tracking_anonymized: true` flag instead.
- **Standard telemetry** — identify calls and user events additionally include the user's email, first name, and last name.

The choice is per user, not per deployment:

- Users set it with the **Anonymize my usage data** checkbox during account setup.
- It can be changed at any time by setting `isTrackingAnonymized` on `PATCH /api/v1/user/me`.

Anonymization does not disable telemetry. The `userId` and resource identifiers (`organizationId`, `projectId`, chart/dashboard UUIDs) **are still sent**, and resource metadata on non-user events (chart and dashboard titles, error messages) is not scrubbed. To stop telemetry leaving the deployment for every user, set [`RUDDERSTACK_ANALYTICS_DISABLED=true`](#configuration-2-disable-outbound-rudderstack-telemetry).
