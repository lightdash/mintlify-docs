...
- **Optional integrations** (SSO, SMTP, Slack, GitHub, object storage, AI providers, MCP, sandbox providers, and Sentry) talk directly to the endpoints you configure. They are not proxied through Lightdash Cloud. The [organization roadmap](/self-host/customize-deployment/organization-roadmap) is on for every Enterprise instance whose license validates at boot and calls `https://roadmap.lightdash.com`.
...
Keygen is the only Lightdash-operated external service required for Enterprise features. Once the license validates, the backend also calls `https://roadmap.lightdash.com` to serve the [organization roadmap](/self-host/customize-deployment/organization-roadmap) — see the [egress matrix](#egress-matrix) below.
...
| `roadmap.lightdash.com` | [Organization roadmap](/self-host/customize-deployment/organization-roadmap) on every Enterprise instance whose license validated at boot, and optional license-validation proxy | Optional | Backend | Roadmap requests: organization UUID as a path segment and license key in the `lightdash-license-key` header. Proxy requests: license key in a JSON `key` field. No query text or business data. | Remove `LIGHTDASH_LICENSE_KEY` (Community only) to stop roadmap requests; leave `LIGHTDASH_LICENSE_VALIDATION_PROXY_ENABLED` unset so license validation goes directly to `api.keygen.sh` |
...
<Steps>
  ...
  <Step title="Allow-list the domains you actually need">
    ...
    - `roadmap.lightdash.com` — for the organization roadmap on Enterprise Edition.
