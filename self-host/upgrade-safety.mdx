...
{/* Before this PR leaves draft: verify the verb against the published @lightdash/cli (npm view @lightdash/cli version), replace the illustrative output below with verbatim real output (including whether the Direction line survives the reverse-span-rejection amendment), confirm the --json field list below matches the amended output shape, and state the carrier CLI version here if we want one. */}
