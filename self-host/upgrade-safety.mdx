...
The answer depends on exactly three inputs: the version you're running, the version you're targeting, and every release in between. Fetch the index once and check every release between your current version (exclusive) and your target (inclusive):
...
3. `rollingUpdateSafe` is `false` — the span crosses at least one release that is not proven rolling-safe, so **deploy with `Recreate`**: stop the old version, then start the new one.

If your current version isn't in the index at all, the command errors on purpose. That's the fail-safe default: a version the index can't see is a span it can't vouch for.

A CLI command that answers span checks (including for CI) is planned for a future release; until it ships, reading the index directly as above is the supported method.
...
- **All three change surfaces were checked and came back clean**: no breaking REST API changes, no breaking MCP tool changes, no environment variable removals, renames, or default changes.
...
Releases may attach additional informational counters on the API surfaces (for example `breakingCount` or `advisories`); the fields documented below are the stable contract.
...
<Note>
If you inspect the artifact *inside* a development build of the image you may see `version: "development"` and an epoch-0 `releaseDate` (`1970-01-01T00:00:00.000Z`). That's the committed development sentinel, not a bug — its fields are all `unknown`, so it fails safe. The release pipeline overwrites it with real values when a release is cut.
</Note>
...
| Field | Type | Meaning |
| --- | --- | --- |
| `api.rest`, `api.mcp` | object | One entry per API surface: `checked` (did the diff run), `breaking` (boolean or `"unknown"`), and `changes` (human-readable, endpoint- or tool-specific detail) |
...
The safety signal reads the same in both directions: rolling back from B to A is gated by the same span rule as upgrading from A to B.

- **Running an older image against a newer database is a supported state.** If a new version misbehaves, redeploying the previous image is the sanctioned emergency path — the database schema stays where it is.
- **Database down-migrations are not supported.** Never attempt to roll the schema back in production.
- **Past that window, restore from backup.** If you need the *database* back at an older state — not just the application — the answer is your database backup, which is why [point-in-time recovery](/self-host/production-deployment-checklist) belongs in your production setup.

<Warning>
Before rolling back across multiple releases, check the span in the index just as you would for an upgrade — required stops and unproven (`unknown`) releases gate rollbacks too.
</Warning>
...
- Confirm your database backup (and point-in-time recovery, if configured) is current.
- Know your rollback plan before you start: which image tag you'd redeploy, and which backup you'd restore if the database itself had to come back. Test it before you need it.
