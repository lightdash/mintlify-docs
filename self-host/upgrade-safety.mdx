...
description: "Read the release-safety artifact, check an upgrade span, and pick the right deployment strategy"
tag: "Beta"
doc-type: reference
---
...
When `migrationJob.enabled` is enabled, `Recreate` alone does not stop old pods before migrations. Helm runs the pre-upgrade migration hook before it applies the Deployment strategy. For a `false` or `unknown` span, [scale Lightdash application workloads to zero and verify that they have terminated](/self-host/upgrade-runbook#scale-application-workloads-to-zero-when-required) before `helm upgrade`.
</Warning>
...
| Anything `false` or `unknown` | Use `Recreate`. With `migrationJob.enabled`, first [scale application workloads to zero](/self-host/upgrade-runbook#scale-application-workloads-to-zero-when-required) | `docker compose down`, update the tag, `docker compose up -d` |

`Recreate` means a short window of downtime. Without a migration Job, it stops old replicas before new pods start and migrate. With a migration Job, use the [runbook's scale-to-zero procedure](/self-host/upgrade-runbook#scale-application-workloads-to-zero-when-required), because old replicas otherwise keep serving while the pre-upgrade hook changes the schema. A rolling update is only appropriate when the verdict certifies that behaviour as safe.
...
- If the span verdict is anything other than `true`, plan a maintenance window and use `Recreate`. With `migrationJob.enabled`, follow the [scale-to-zero procedure](/self-host/upgrade-runbook#scale-application-workloads-to-zero-when-required) before the Helm upgrade.
