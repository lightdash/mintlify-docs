...
description: "How to read the release-safety artifact Lightdash publishes with every release, check an upgrade span, and pick the right deployment strategy."
tag: "Beta"
doc-type: reference
---

<Info>
  **Release-safety artifacts are in [Beta](/support/feature-maturity-levels).** The artifacts are published for every release and are safe to read today, but we're still refining how we classify risk and what the artifact carries, so fields and verdicts may change.
