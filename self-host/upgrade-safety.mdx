...
The answer depends on exactly three inputs: the version you're running, the version you're targeting, and every release in between. The [Lightdash CLI](/guides/cli/how-to-install-the-lightdash-cli) answers it in one command:

{/* Before this PR leaves draft: verify the verb against the published @lightdash/cli (npm view @lightdash/cli version), replace the illustrative output below with verbatim real output, and state the carrier CLI version here if we want one. */}

```bash
lightdash upgrade-check --from 1.111.0 --to 1.115.0
```

Example output (illustrative — format shown against the live index at the time of writing):
...
For automation that needs the detail, `--json` prints a stable machine-readable object (`safe`, `verdict`, `direction`, `requiredStops`, `minPreviousVersion`, `coveredVersions`, `missingRanges`):
...
`upgrade-check` also accepts a `--from` newer than `--to` and reports `Direction: rollback`, evaluating the same span with the same fail-closed semantics:

```bash
lightdash upgrade-check --from 1.123.0 --to 1.121.0
```

Versions `1.123.0` and later tolerate running an older image against a newer database natively; on older versions, going back needs additional steps — full rollback guidance is coming in an upgrade runbook.
