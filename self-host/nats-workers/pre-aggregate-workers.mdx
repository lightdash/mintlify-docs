...
import NatsWorkerConfig from '/snippets/self-host/nats-worker-config.mdx';
...
- **Too many rows** — Materializations should not contain very large datasets. We recommend keeping materializations under 100,000 rows for optimal performance. You can use `max_rows` in your [pre-aggregate definition](/semantic-layer/pre-aggregates/getting-started#row-limits) or the `PRE_AGGREGATES_MAX_ROWS` environment variable to enforce a limit.
