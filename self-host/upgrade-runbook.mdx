...
description: "Upgrade sequences for Kubernetes, docker compose, and automated deployments"
tag: "Beta"
doc-type: reference
---
...
The automatic barrier requires a chart version whose values reference includes `upgrade.mode`. If the field is absent, use the manual fallback in the next step.
</Step>

<Step title="Run the automatic shutdown barrier">
With `migrationJob.enabled: true` and `upgrade.mode: Recreate`, a chart that supports `upgrade.mode` automatically removes the release-managed HPA, scales all database-capable Lightdash workloads to zero, waits for their pods to terminate, and then runs the pre-upgrade migration Job. Do not scale workloads manually for this path.
...
Use this fallback for chart versions that do not support `upgrade.mode` and for custom or manual deployments. For a `false` or `unknown` verdict with `migrationJob.enabled: true`, scale every Lightdash application workload to zero before you run `helm upgrade`.
...
With `migrationJob.enabled: true`, the chart runs migrations in a `pre-install,pre-upgrade` hook Job and the backend pods then start without migrating, so replicas never race for the lock. This is the recommended setup for any multi-replica deployment. For a `false` or `unknown` verdict, the [automatic shutdown barrier](#run-the-automatic-shutdown-barrier) runs when the chart supports `upgrade.mode`; otherwise use the [manual fallback](#scale-application-workloads-manually-when-required). Without the Job, the pods migrate at startup and the lease runtime arbitrates between them: one pod wins and migrates, the rest wait.
</Step>
...
INTERMEDIATE_STOPS="$(jq -r --arg target "$TARGET_VERSION" '[.requiredStops[] | select(. != $target)] | join(", ")' <<<"$CHECK_JSON")"
if [[ -n "$INTERMEDIATE_STOPS" ]]; then
...
if [[ "$REQUIRED_STOP_COUNT" != "0" ]]; then
  STOP_TARGET_IS_SAFE="$(jq -r --arg target "$TARGET_VERSION" '
    .verdict == true and
    (.missingRanges | length == 0) and
    (.requiredStops | length == 1 and .[0] == $target)
  ' <<<"$CHECK_JSON")"
  if [[ "$STOP_TARGET_IS_SAFE:$CHECK_EXIT" == "true:1" ]]; then
    UPGRADE_MODE=RollingUpdate
  else
    printf '%s\n' 'The required-stop target does not meet the reviewed-hop conditions; aborting.' >&2
    exit 1
  fi
else
