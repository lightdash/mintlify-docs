...
<Step title="Scale application workloads to zero when required">
For a `false` or `unknown` verdict with `migrationJob.enabled: true`, scale every Lightdash application workload to zero before you run `helm upgrade`. Helm runs the pre-upgrade migration Job before it applies the Deployment strategy, so `Recreate` alone does not stop existing pods before the Job migrates the database.
...
With `migrationJob.enabled: true`, the chart runs migrations in a `pre-install,pre-upgrade` hook Job and the backend pods then start without migrating, so replicas never race for the lock. This is the recommended setup for any multi-replica deployment. For a `false` or `unknown` verdict, complete the [scale-to-zero step](#scale-application-workloads-to-zero-when-required) first. Without the Job, the pods migrate at startup and the lease runtime arbitrates between them: one pod wins and migrates, the rest wait.
</Step>
