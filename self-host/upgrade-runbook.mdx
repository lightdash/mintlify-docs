...
FIRST_STOP="$(jq -r '.requiredStops[0] // empty' <<<"$CHECK_JSON")"
...
`--set-string` overrides any static `upgrade.mode` in the values file. Derive the mode for every upgrade. A required stop is a separate target, not a direct hop. Malformed output, an execution failure, or a fetch failure has no usable verdict and aborts the pipeline.
