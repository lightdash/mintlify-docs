...
Upgrade mechanics are in [update Lightdash](/self-host/upgrading) and the version policy in [versioning](/self-host/upgrading). Operational best practice:
...
For async warehouse queries, see the [NATS workers overview](/self-host/nats-workers) and [warehouse workers](/self-host/nats-workers/warehouse-workers) — including the critical rule: **never enable `nats.enabled` without `warehouseNatsWorker.enabled`**.
...
Sandbox providers (E2B, AWS Lambda MicroVMs, Azure) and their security model are covered in [sandboxes](/self-host/customize-deployment/sandboxes); model configuration in [self-hosting data apps](/data-apps/self-hosting). Production checklist on top:
