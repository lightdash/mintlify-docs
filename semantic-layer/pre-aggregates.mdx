...
Pre-aggregates can be materialized through three different triggers:
...
- Queries with [Parameters](/semantic-layer/parameters), or with [user attributes](/workspace-admin/user-attributes) referenced from a dimension or metric SQL expression (user attributes in [`sql_filter`](#sql_filter-and-pre-aggregates) are supported)
