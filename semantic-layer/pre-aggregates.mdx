...
description: "Speed up dashboards and cut warehouse costs with pre-computed, materialized summaries"
tag: "Beta"
---
...
### Parent metrics deduplicated across a join

A re-aggregatable metric becomes exact-only when the pre-aggregate deduplicates it. If a pre-aggregate groups by dimensions from the "many" side of a one-to-many join (order lines, or an unnested repeated column) and carries a `count`, `sum` or `average` from the "one" side (orders), each stored row holds the value for the distinct parents in that group, computed through the parent's `primary_key`. That value is correct at the pre-aggregate's own grain, but summing rows across the many-side dimension would count every parent once per child. Lightdash serves such a metric only on an [exact-match query](#exact-match-queries) and otherwise falls back to the warehouse with the miss reason "Metric deduplicated across a join". Metrics defined on the many side, and inflation-proof metrics such as `min` and `max`, re-aggregate as usual.
