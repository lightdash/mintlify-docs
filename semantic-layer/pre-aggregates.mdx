...
- [Parameters](/semantic-layer/lightdash-config-yml#parameters-configuration) — parameter values are picked at query time, so they cannot be resolved during materialization. Queries that use parameters fall back to the warehouse.
...
- SQL table calculations ([Formula table calculations](/explore/table-calculations/formulas#formula-table-calculations) are supported)
...
- **At materialization time**, the filter is evaluated against your warehouse. If the SQL references [Parameters](/semantic-layer/lightdash-config-yml#parameters-configuration) or [user attributes](/workspace-admin/user-attributes), the values injected come from the materialization context — you can pin this to a fixed identity or attribute set with [`materialization_role`](/semantic-layer/pre-aggregates/getting-started#materialization-role) so the materialization captures the rows you need.
...
See the [caching guide](/semantic-layer/caching) for details.
...
- You need unsupported features listed above, such as `count_distinct`, [Parameters](/semantic-layer/lightdash-config-yml#parameters-configuration), [`sql_filter`](/semantic-layer/tables#sql-filter-row-level-security), or raw SQL table calculations
