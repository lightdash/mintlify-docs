...
<Info>
  If you're new to lightdash.config.yml, check out our [getting started guide](/semantic-layer/lightdash-config-yml#getting-started-with-lightdashconfigyml) to learn how to create and set up this file.
</Info>
...
Resolves to the currently selected [date zoom](/explore/date-zoom) granularity, lowercased (for example `day`, `week`, `month`, `quarter`, `year`, or a custom granularity name). When no date zoom is selected, the value is an empty string.
