...
description: "Parameters are variables users set at runtime to build dynamic, reusable queries without writing SQL"
doc-type: reference
---
...
<Info>
  If you're new to lightdash.config.yml, check out our [getting started guide](/semantic-layer/lightdash-config-yml#getting-started) to learn how to create and set up this file.
</Info>
...
1. **Dimension SQL**: Use parameters in the SQL definition of a [dimension](/semantic-layer/dimensions)
2. **Metric SQL**: Use parameters in the SQL definition of a [metric](/semantic-layer/metrics)
3. **Table SQL**: Use parameters in [sql_from](/semantic-layer/tables#sql-from) and [sql_filter](/semantic-layer/tables#sql-filter-row-level-security) definitions
4. **Table Joins**: Use parameters in [join](/semantic-layer/joins) conditions
5. **SQL Runner**: Use parameters in the [SQL Runner](/explore/sql-runner) query
6. **Table Calculations**: Use parameters in [table calculations](/explore/table-calculations)
7. **Additional Dimensions**: Use parameters in the SQL definition of an [additional dimension](/semantic-layer/dimensions#additional-dimensions)
8. **Custom Dimensions**: Use parameters in [custom dimension](/explore/create-custom-fields#custom-sql) definitions
...
For the full list of parameter properties, see [Parameters configuration](#parameters-configuration) below.
...
parameters:
  ...
  # Date parameter that defaults to the current date at query time
  period_to:
    label: "Period to"
    description: "As-of date for stock and pricing metrics"
    type: "date"
    default: today
  ...
  # Parameter with labeled options — display text differs from the underlying value
  status:
    label: "Status"
...
For the full list of project-level parameter properties, see [Parameters configuration](#parameters-configuration) below.
...
For the full list of model-level parameter properties, see [Parameters configuration](#parameters-configuration) below.


## Parameters configuration

Each parameter is a key-value pair: the key is the parameter name (alphanumeric with underscores or hyphens) and the value is an object with the following properties. Project-level and model-level parameters share this schema.
...
| `options`                | No       | Array of strings, numbers, or `{ label, value }` objects | A list of possible values for the parameter.                                                                       |
| `default`                | No       | string, number, or array of strings/numbers              | The default value(s) for the parameter. When using labeled options, the default must match a `value`, not a `label`. For `type: date`, `today` resolves to the current date at query time (see [Dynamic date defaults](#dynamic-date-defaults)). |
...
Static `options` can be plain values or `{ label, value }` objects when the display text should differ from the underlying value — see the `status` parameter in the [project-level example](#project-level-parameters) above. Plain string or number arrays remain the simplest form when the label and value are the same. With labeled options, `default` must match a `value`, not a `label`.

### Dynamic date defaults

A `type: date` parameter accepts a static `YYYY-MM-DD` default or the keyword `today`:
...
`today` is resolved every time a query runs, so dashboards, scheduled deliveries, and API calls that do not set the parameter always use the current date. The date picker opens pre-filled with today's date and stays fully editable, so viewers can still pick any other date.

<Note>
  `today` is evaluated in the project's [query timezone](/workspace-admin/set-project-timezone). Projects with intraday data close to midnight should check that this timezone matches the one their metrics are reported in.
</Note>

Other dynamic defaults, such as expressions or references to other parameters, are not supported yet. Follow [this issue](https://github.com/lightdash/lightdash/issues/25809) for progress.
...
<Frame>
  <img src="/images/semantic-layer/parameters/reserved-parameter-autocomplete-light.png" alt="Custom SQL autocomplete showing the date_zoom reserved parameter labelled as a System variable" className="block dark:hidden" />
  <img src="/images/semantic-layer/parameters/reserved-parameter-autocomplete-dark.png" alt="Custom SQL autocomplete showing the date_zoom reserved parameter labelled as a System variable" className="hidden dark:block" />
</Frame>
...
Resolves to the currently selected [date zoom](/explore/dashboards/date-zoom) granularity, lowercased (for example `day`, `week`, `month`, `quarter`, `year`, or a custom granularity name). When no date zoom is selected, the value is an empty string.
...
The value tracks the granularity actually used to compile the query, so it stays in sync with what users see in the date zoom dropdown, including [custom granularities](/semantic-layer/lightdash-config-yml#custom-granularities-configuration).
