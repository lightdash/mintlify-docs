...
description: "Configure project-wide settings like the Metrics Catalog, parameters, and defaults in one YAML file"
...
- **Organize metrics in the [Metrics Catalog](/explore/metrics-catalog)** with custom categories and visibility settings
...
1. ✅ A working Lightdash project (completed the [getting started guide](/get-started/quickstart))
2. ✅ A local dbt project (not connected to dbt Cloud)
3. ✅ The [Lightdash CLI](/get-started/quickstart/prepare-your-dbt-project#step-1-install-the-lightdash-cli-tool) installed and configured
...
The `spotlight` section allows you to configure project-wide Metrics Catalog settings. `spotlight` is the YAML namespace that covers everything about how your metrics show up across the [Metrics experience](/explore/metrics-catalog#the-spotlight-config) — here you set the project-wide defaults, and you can override them per model or metric in your model `.yml` files. This section is required in the lightdash.config.yml file.
...
The `parameters` section defines project-wide parameters that can be referenced across your Lightdash project. Project-level parameters use the same property schema as model-level parameters.
...
Reference a project-level parameter in SQL with `${lightdash.parameters.parameter_name}` (or the shorter `${ld.parameters.parameter_name}`).

See the [Parameters reference](/semantic-layer/parameters#parameters-configuration) for the full property schema, and the [Parameters guide](/semantic-layer/parameters) for where and how to use parameters across your project.

### Reserved parameter names

A small set of parameter names — currently `date_zoom` — are reserved by Lightdash and resolve automatically from the query context, so you don't declare them in `lightdash.config.yml`. Defining a user parameter with a reserved name shadows the reserved one. See [Reserved parameters](/semantic-layer/parameters#reserved-parameters) for the full list and usage.
...
| `pre_aggregate_execution_fallback`  | No       | boolean                  | Whether a matched query whose pre-aggregate execution fails is retried against the source warehouse. When `false`, the query errors instead of transparently running on the warehouse. Defaults to `true`. See [Pre-aggregate execution fallback](#pre-aggregate-execution-fallback) below. |
...
### Pre-aggregate execution fallback

When a query matches a [pre-aggregate](/semantic-layer/pre-aggregates) but the pre-aggregate execution itself fails — for example, an unreadable materialization file, a DuckDB error, or a missing external table — Lightdash retries the query against the source warehouse by default. This keeps dashboards available when a materialization breaks.
...
You can also toggle this default via the API:
...
<Info>
  See [Tables reference](/semantic-layer/tables#case-sensitive) for explore-level configuration and [Dimensions reference](/semantic-layer/dimensions#case-sensitive) for dimension-level configuration.
</Info>
...
<Info>
  See [Dimensions reference](/semantic-layer/dimensions#time-intervals) for per-dimension configuration and the [Custom granularities](#custom-granularities-configuration) section below for defining custom granularity keys.
</Info>
...
See the [date zoom guide](/explore/dashboards/date-zoom) for how to configure which granularities appear in the dropdown and how to set defaults.
...
See the [dimensions reference](/semantic-layer/dimensions#time-intervals) for configuring time intervals on dimensions, including how to reference custom granularities.
...
<Info>
  See [Tables reference](/semantic-layer/tables#groups) for more information on grouping tables in the sidebar.
</Info>