...
<Info>
  **Experimental:** The `convert_timezone` dimension override is currently in the [Experimental](/help/feature-maturity-levels) phase. Contact support to enable it for your organization. Has no effect unless the [project query timezone](/workspace-admin/set-project-timezone#project-query-timezone-ui-setting) is set.
</Info>
...
See [Per-dimension timezone opt-out](/workspace-admin/set-project-timezone#per-dimension-timezone-opt-out) in the Timezones guide for how this interacts with other timezone settings and filter behavior.
...
Regardless of which option you use, filter autocomplete always returns `{ value, label }` pairs. The `value` is what gets used in the filter, and the `label` is what's displayed in the dropdown. Parameters that source their options from the same dimension via [`options_from_dimension`](/semantic-layer/tables#parameter-configuration) automatically pick up these labels too.
...
The referenced dimension must live in the same table. Any [parameter](/semantic-layer/tables#parameter-configuration) that sources its options from `customer_id` via `options_from_dimension` also picks up the customer name labels automatically.
