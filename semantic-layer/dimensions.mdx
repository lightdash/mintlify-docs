...
In the example below, only users with `is_admin` attribute `true` can use the `salary` dimension on `user` table. Users without access to this dimension will not see it, or the metrics defined under it, on the Explore page. Model-level metrics that reference the dimension are not restricted — see [metrics and column access control](/workspace-admin/user-attributes#metrics-and-column-access-control).
