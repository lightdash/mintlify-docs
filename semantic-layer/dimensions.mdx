...
description: "The columns of your tables, and every option for defining and configuring them in YAML"
...
| [required_attributes](#required-attributes) | No    | Object with `user_attribute`, `value`  | Limits access to users with those attributes (AND logic - all must match) |
| [any_attributes](#any-attributes) | No    | Object with `user_attribute`, `value`  | Limits access to users with those attributes (OR logic - at least one must match) |
...
<Frame>
  ![Dimensions list with an info icon beside "User id" showing the tooltip "Unique identifier for a user."](/images/semantic-layer/dimensions/screenshot-column-descriptions-3498d7b3c09c38fda5b755ce21358b2c.png)
</Frame>
...
<Frame>
  ![screenshot-default-intervals](/images/semantic-layer/dimensions/screenshot-default-intervals-1fb6e04e2d4f617bb22f9fea86fa52c6.png)
</Frame>
...
<Frame>
  ![screenshot-intervals-off](/images/semantic-layer/dimensions/screenshot-intervals-off-e786a3290b6c8b8454fe7f223a5682d8.png)
</Frame>
...
<Info>
<Badge icon="flask" color="purple" size="sm" shape="pill">Beta</Badge> Custom granularities are available on all plans. [What Beta means](/support/feature-maturity-levels).
</Info>
...
<Frame>
  ![Dimensions sidebar with a Product Details group expanded to a nested Item Details group of basket item fields](/images/semantic-layer/dimensions/groups-757097385f03095824be4746f585d50b.png)
</Frame>
...
<Frame>
  ![Results table with a Path cell open, showing an "Open site page" link above "View underlying data"](/images/semantic-layer/dimensions/clickable-dimension-link-70554874d4e468900e9d8fadf3b53cd2.jpg)
</Frame>
...
<Frame>
![Rich text examples rendered in a Table chart](/images/semantic-layer/dimensions/rich-text-hero.png)
</Frame>
...
<Frame>
![Cells rendered with a bold "Customer ID:" label, a code-style ID, and two inline links per row](/images/semantic-layer/dimensions/rich-text-basic-markdown-links.png)
</Frame>
...
<Frame>
![Conditional tiers rendered with emoji, bold and italic text, and a grey "No data" fallback for null values](/images/semantic-layer/dimensions/rich-text-conditional-tiers.png)
</Frame>
...
<Frame>
![Avatar image with colored initials, bold name, and a LinkedIn link rendered per row](/images/semantic-layer/dimensions/rich-text-html-avatars.png)
</Frame>
...
You can combine both `required_attributes` and `any_attributes` on the same dimension. When both are set, both checks must pass. See [user attributes](/workspace-admin/user-attributes#combining-required_attributes-and-any_attributes) for details.
...
See [Per-dimension timezone opt-out](/workspace-admin/set-project-timezone#opt-a-column-out-of-timezone-conversion) in the Timezones guide for how this interacts with other timezone settings and filter behavior.

## Wall clock timezone

Set `wall_clock_timezone` on a naive `timestamp` dimension (one with no timezone in its warehouse type) to declare the zone its values are actually stored in, when your connection's data timezone doesn't cover it.

<CodeGroup>
```yaml dbt v1.10+
columns:
  - name: created_at_sast
    config:
      meta:
        dimension:
          type: timestamp
          wall_clock_timezone: Africa/Johannesburg
```
...
Lightdash reads the column's values as wall clock times in that zone instead of the connection's data timezone. Display, grouping, and filters all use it, so two naive columns stored in different zones on the same connection both come out right.

Setting `wall_clock_timezone` declares the column naive. If the warehouse catalog reports it as timezone-aware, the declaration wins and a project validation warning is shown.

The value must be an IANA zone name, such as `Europe/Berlin`. Any other value is ignored, with a warning. It's only valid on `timestamp` dimensions.

It can't be combined with `convert_timezone: false`. If both are set on the same dimension, `convert_timezone: false` applies and the wall clock zone is ignored, with a warning.

Time-interval children of the dimension (`_day`, `_month`, `_day_of_week_index`, ...) inherit it.

Most columns don't need this. The connection's data timezone already covers a warehouse where every naive column is in the same zone. Reach for `wall_clock_timezone` for the exceptions, like a column pre-converted to local time sitting next to columns in UTC.

See [Declare a column's stored timezone](/workspace-admin/set-project-timezone#declare-a-columns-stored-timezone) in the Timezones guide for when to use this.

## Timestamp domain

Lightdash reads whether a `timestamp` column stores an instant (`aware`) or a bare wall clock (`naive`) from your warehouse catalog, so you don't usually need to set this. Override it when the catalog type doesn't match the data, for example a `TIMESTAMP_NTZ` column that already holds UTC instants.
...
The YAML value wins over the catalog. It's ignored on dimensions with custom `sql` and on additional dimensions. Interval children (`_day`, `_month`, ...) inherit from their base.

See [Override a column timestamp domain](/workspace-admin/set-project-timezone#override-a-column-timestamp-domain) in the Timezones guide.
...
<Frame>
  ![Pie chart Series config panel listing order-status series, each with a colour swatch matching its slice](/images/semantic-layer/dimensions/pie-chart-color-b5c761ad3dc797717c6397920572b1c4.png)
</Frame>
...
<Info>
  Dimension-level `case_sensitive` settings override explore-level and project-level settings. See [Tables reference](/semantic-layer/tables#case-sensitive) for explore-level configuration and [lightdash.config.yml reference](/semantic-layer/lightdash-config-yml#defaults-configuration) for project-level defaults.
</Info>
...
- Source autocomplete values from a dimension in another model (for example, drive a `flight.airline_code` filter from the canonical list in `dim_airlines`).
...
Regardless of which option you use, filter autocomplete always returns `{ value, label }` pairs. The `value` is what gets used in the filter, and the `label` is what's displayed in the dropdown. Parameters that source their options from the same dimension via [`options_from_dimension`](/semantic-layer/parameters#parameters-configuration) automatically pick up these labels too.

### Properties

| Property | Required | Value | Description |
| :------- | :------- | :---- | :---------- |
| values | No | Array of `value`, `label` | A static, hard-coded list of suggested filter values. `value` is the raw value used in the filter; `label` is the optional display name shown in the autocomplete dropdown. Duplicate `value`s are ignored (the first occurrence wins). Use this when the set of values is small and known ahead of time. |
...
### Labelling warehouse-fetched values with another dimension
...
The referenced dimension must live in the same table. Any [parameter](/semantic-layer/parameters#parameters-configuration) that sources its options from `customer_id` via `options_from_dimension` also picks up the customer name labels automatically.
...
Properties inside `options_from_dimension`:
...
- The dimension-level `label_dimension` on the current dimension is ignored when `options_from_dimension` is set. Set `label_dimension` inside `options_from_dimension` instead — labels always come from the model the values come from.
- If `fetch_from_warehouse: false` is also set on this dimension, curated `values` are served and `options_from_dimension` is ignored. Lightdash surfaces a project warning when both are configured together.
...
Tags are useful for:
- **AI agent access control** — restrict which dimensions an [AI agent](/agents/data-access) can see
...
BigQuery `STRUCT` and `ARRAY` fields use the configuration described in [Nested and repeated columns](/semantic-layer/nested-and-repeated-columns).
