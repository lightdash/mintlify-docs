...
description: "Connect models to each other so people can query more than one table at a time"
sidebarTitle: "Joins"
doc-type: reference
---
...
<Frame>
  ![screenshot-joined-table](/images/semantic-layer/joins/screenshot-joined-table-1eaca01a7819ce13488b565e04c90cd8.png)
</Frame>
...
<Tip>
  ...
  - The `sql_on` statement can include references to [user attributes](/workspace-admin/user-attributes) for row-level filtering.
</Tip>
...
If you get the following error: `Model "TABLE" has a metric reference: ${TABLE.DIMENSION} which matches no metric` - you likely have a mismatch in metric category. Read more about metric categories [here](/semantic-layer/metrics#metric-categories).
...
<Warning>
  **Every joined field that you reference in a YAML file adds a dependency that you'll have to include in all future references to that model.**
  ...
  Check out our [dimensions](/semantic-layer/dimensions) and [metrics](/semantic-layer/metrics) reference docs to see all of the other configurations you can use with your joined fields.
</Warning>
...
<Frame>
  ![Two sidebars side by side: the hidden joined table is absent from dimensions and metrics but still listed under Tables](/images/semantic-layer/joins/hide-join-table-columns-5527cee18c448653280c238838cf71fa.jpg)
</Frame>

You can adjust which tables show up in the Tables list with [Tables configuration](/semantic-layer/tables#choosing-which-tables-appear-in-lightdash) in your project settings.
...
<AccordionGroup>
  <Accordion title="Two repeated columns unnested together">
    For the `Repeated columns … are unnested together` query warning, see [Nested and repeated columns](/semantic-layer/nested-and-repeated-columns#query-warnings).
  </Accordion>
