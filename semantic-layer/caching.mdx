...
You can also read and update the setting through the API:

- `GET /api/v1/projects/{projectUuid}/results-cache-config` returns `cacheTtlSeconds` (`null` when the project is using the instance default) and `instanceDefaultTtlSeconds`.
- `PATCH /api/v1/projects/{projectUuid}/results-cache-config` accepts `{ "cacheTtlSeconds": <60–2592000> }` to set a project-specific duration, or `{ "cacheTtlSeconds": null }` to fall back to the instance default.
