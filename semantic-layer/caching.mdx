...
Results caching and [pre-aggregates](/semantic-layer/pre-aggregates) are complementary. Caching stores query results after the first warehouse hit; pre-aggregates materialize summary tables in advance so the warehouse is never hit at query time.

For a detailed comparison, see [Pre-aggregates vs results caching](/semantic-layer/pre-aggregates#pre-aggregates-vs-results-caching).