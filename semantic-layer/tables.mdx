...
description: "How dbt models become queryable Tables, and every option for configuring them"
sidebarTitle: "Tables"
doc-type: reference
---
...
The [`build your semantic layer` track](/get-started/build-your-semantic-layer) walks through generating this YAML with the Lightdash CLI.

## Choosing which Tables appear in Lightdash

Not every dbt model with a YAML file belongs in Lightdash — staging and intermediate models usually shouldn't be queryable by end users. **Tables configuration** in your project settings controls which models become Tables.

To reach it, click the gear icon in the top-right navigation bar, choose **Project settings**, then select **Tables configuration** in the left sidebar. The setting is only visible to users whose [role](/workspace-admin/roles) lets them edit project settings.

| Option | Behavior |
| --- | --- |
| Show entire project | Every model with a YAML file in your dbt project appears as a Table. |
| Show models with any of these tags | Only models carrying one of the [dbt tags](#limiting-tables-with-dbt-tags) you list appear as Tables. |
| Show models in this list | Only the models you select by name appear as Tables. |

<Frame>
  <img src="/images/semantic-layer/tables/tables-config-settings.png" alt="Tables configuration in Lightdash project settings" />
</Frame>

Tables configuration filters models after Lightdash compiles them. To skip compilation for models entirely — which speeds up syncing on large projects — use the [dbt selector](/integrations/dbt/projects#dbt-selector) in your project settings instead.

### Limiting Tables with dbt tags

Tagging is the most maintainable way to control which models appear, because the decision lives in your dbt project alongside the model rather than in a list someone has to remember to update. Use a set of tags you already maintain, or create a Lightdash-specific one.
...
<Frame>
  <img src="/images/semantic-layer/tables/table-selection-prod.png" alt="Selecting models by dbt tag in Tables configuration" />
</Frame>

### Tables configuration vs. dbt selector

Two settings can filter models by dbt tag, and they run at different points in the sync.

**Tables configuration** lives in Project settings and filters *after* Lightdash has compiled your dbt project. Every model is still compiled, and Lightdash still needs warehouse access to the datasets behind them. The tag list only controls what users see in the UI. Use it when you want to tidy up the Tables list without changing what Lightdash pulls from the warehouse.

**[dbt selector](/integrations/dbt/projects#dbt-selector)** lives under the dbt connection's advanced configuration and filters *before* compilation. Anything that doesn't match the selector is never compiled, so its datasets are never queried. It takes full [dbt selection syntax](https://docs.getdbt.com/reference/node-selection/syntax#combining-state-and-result-selectors), for example `tag:lightdash`. Use it when you want to reduce what Lightdash compiles and needs access to. This is useful on large projects where compilation is slow, or when you want to keep whole subsets of models out of Lightdash entirely.

The two can be combined: the dbt selector decides which models Lightdash compiles, and Tables configuration then decides which of those models appear as Tables.
...
| [joins](/semantic-layer/joins)                                | array              | Join logic to join other data models to the Table. [Read about joins.](/semantic-layer/joins)                                               |
| [metrics](/semantic-layer/metrics#2-using-the-model-meta-tag) | object             | Model metrics. [Read about model metrics](/semantic-layer/metrics#2-using-the-model-meta-tag)                                               |
| [groups](#groups)                                         | array of strings   | Group tables in the sidebar, with up to 5 levels of nesting. [Read about groups.](#groups)                                              |
| group_label                                               | string             | <Badge icon="haze-moon" color="gray" size="sm" shape="pill">Deprecated</Badge> Use [`groups`](#groups) instead. Group tables in the sidebar.                                     |
| [sql_from](#sql-from)                                     | string             | Overrides dbt model relation_name                                                                                                       |
| [sql_filter](#sql-filter-row-level-security)              | string             | A permanent filter that will always be applied when querying this table directly. [`Read about sql_filter.`](#sql-filter-row-level-security)               |
| [sql_where](#sql-filter-row-level-security)               | string             | Alias for `sql_filter`                                                                                                                  |
| [required_attributes](#required-attributes)               | object             | Limits access to users with those attributes (AND logic). [Read about user attributes](/workspace-admin/user-attributes)                                 |
| [any_attributes](#any-attributes)                         | object             | Limits access to users with those attributes (OR logic). [Read about user attributes](/workspace-admin/user-attributes)                                 |
...
Group keys are referenced project-wide. Define their display labels and (optional) descriptions in your [`lightdash.config.yml`](/semantic-layer/lightdash-config-yml#table-groups-configuration) file under `table_groups`:
...
<Frame>
  ![Tables sidebar where a Staging group expands to Stg customers, Stg orders, and Stg payments above ungrouped tables](/images/semantic-layer/tables/table-group-label-a6d42e6d72a08f15c392fb7cb03a2e6a.png)
</Frame>
...
Using `sql_filter` with [user attributes](/workspace-admin/user-attributes) allows you to set up [row-level security in your tables](/workspace-admin/user-attributes#row-filtering-with-sql%5Ffilter). You can reference user attributes in your `sql_filter` using `${lightdash.attributes.my_attribute_name}`
...
Lightdash can use [user attributes](/workspace-admin/user-attributes) to limit some tables to some users.
...
You can combine `required_attributes` and `any_attributes` on the same table. When both are set, both checks must pass. See [user attributes](/workspace-admin/user-attributes#combining-required_attributes-and-any_attributes) for details.
...
<Info>
  Note: Required filters are not a security feature. For secure data access control, use [user attributes](/workspace-admin/user-attributes).
</Info>
...
<Info>
  Table-level `case_sensitive` settings override the project-level default. Dimension-level settings override both. See [Dimensions reference](/semantic-layer/dimensions#case-sensitive) for dimension-level configuration and [lightdash.config.yml reference](/semantic-layer/lightdash-config-yml#defaults-configuration) for project-level defaults.
</Info>

## Parameters configuration

Model-level parameters are defined under the model's `meta.parameters` key (in Lightdash YAML, the top-level `parameters` key). They are scoped to the model where you define them and use the same property schema as project-wide parameters — reference them in the model's SQL properties with the model name included, `${lightdash.parameters.model_name.parameter_name}` (or the shorter `${ld.parameters.model_name.parameter_name}`).

See the [Parameters reference](/semantic-layer/parameters#parameters-configuration) for the full property schema, and the [Parameters guide](/semantic-layer/parameters) for syntax and examples, including referencing model-level parameters from joined tables.
...
[Read this guide to learn more about explores](/semantic-layer/explores)
