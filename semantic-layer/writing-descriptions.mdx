...
AI hints can be added at four levels: model, group, dimension, and metric. An `ai_hint` accepts a single string or a list of strings — the examples below use lists.
...
If you organize related dimensions and metrics into groups via `group_details`, a single `ai_hint` on the group applies to every field that references it through its `groups` property — context shared across related fields without repeating it on each one:
