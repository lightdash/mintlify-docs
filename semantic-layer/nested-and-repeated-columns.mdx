...
<Info>
  <Badge icon="flask" color="purple" size="sm" shape="pill">Beta</Badge> Available for BigQuery and Databricks projects. On Lightdash Cloud, ask Lightdash support to enable it for your organization. On self-hosted deployments, add `unnest-repeated-columns` to `LIGHTDASH_ENABLE_FEATURE_FLAGS`; see [Feature flags](/self-host/customize-deployment/environment-variables#feature-flags). [What Beta means](/support/feature-maturity-levels).
</Info>
...
Type names follow your warehouse: BigQuery calls these `RECORD` and `REPEATED`, Databricks `STRUCT` and `ARRAY`.

<Note>
  Databricks supports nested and repeated columns when connected to a SQL warehouse or an all-purpose cluster running Databricks Runtime 16.2 or later.
</Note>
...
- Column types with no addressable fields, such as Databricks `MAP` and `VARIANT`, are exposed as plain string dimensions.
