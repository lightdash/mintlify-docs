...
Add one YAML entry for each leaf you want to expose. Write its full path from the top-level column, separating each level with a dot—for example, `customer.address.city`. Lightdash gets the structure from your BigQuery table, so you do not need to specify whether each level is a `STRUCT` or `ARRAY` in YAML.
...
- Only BigQuery is supported at the moment.
- To access an array element by index, such as `line_items[0].sku`, define a dimension with custom `sql`.
- Selecting a struct or array container directly is not supported.
