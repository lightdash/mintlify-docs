...
Use [Define nested columns in YAML](#define-nested-columns-in-yaml) for the complete configuration, then refer to [Virtual tables](#virtual-tables), [Query behavior and grain](#query-behavior-and-grain), [Query warnings](#query-warnings), [Pre-aggregates](#pre-aggregates), and [Known limitations](#known-limitations) as needed.
...
Only leaves listed in YAML become fields. The following example uses the public `bigquery-public-data.google_analytics_sample.ga_sessions_20170801` table, which has one row per session:
...
models:
  - name: ga_sessions
    ...
    columns:
      ...
      # REPEATED RECORD: becomes the virtual table ga_sessions__hits
      - name: hits
      ...
      # REPEATED RECORD inside hits: becomes ga_sessions__hits__product
...
The container entry, such as `- name: hits`, is optional. When it is present, Lightdash uses its `description` and `meta.dimension.label` for the virtual table. `lightdash generate` does not write container entries or entries below a repeated column, so add those leaves by hand.

### Non-repeated STRUCT leaves

A leaf below a non-repeated struct is an ordinary dimension on the model with its dotted name. For example, `- name: customer.address.city` compiles to `` `orders`.customer.address.city `` and has the field ID `orders_customer__address__city`.

Lightdash gets the leaf's type from the warehouse catalog, so `meta.dimension.type` is optional. An explicitly declared type takes precedence over the catalog type.
...
<Frame>
  <img src="/images/semantic-layer/nested-and-repeated-columns/virtual-tables-sidebar.png" alt="Explore sidebar showing Ga sessions, Ga sessions: Hits, and Ga sessions: Hits: Product tables with their dimensions and metrics" />
</Frame>
...
<Frame>
  <img src="/images/semantic-layer/nested-and-repeated-columns/aliased-virtual-tables.png" alt="Explore query using the Same session alias and fields from its Hits and Hits: Product virtual tables" />
</Frame>

<Frame>
  <img src="/images/semantic-layer/nested-and-repeated-columns/aliased-unnest-sql.png" alt="Compiled SQL joining the Ga sessions model as Same session and unnesting its Hits and Product arrays under the alias" />
</Frame>
...
<Frame>
  <img src="/images/semantic-layer/nested-and-repeated-columns/compiled-unnest-sql.png" alt="Explore results and compiled BigQuery SQL with chained left outer joins that unnest Hits and Product arrays" />
</Frame>
...
<Frame>
  <img src="/images/semantic-layer/nested-and-repeated-columns/query-results.png" alt="Bar chart and results table grouped by product name with product revenue, hit count, and deduplicated session count" />
</Frame>
...
<Frame>
  <img src="/images/semantic-layer/nested-and-repeated-columns/cross-product-warning.png" alt="Query warnings popover explaining that Hits and Custom dimensions are unnested together and may inflate metrics" />
</Frame>
