...
- [Authenticate](/workflow/cli/authenticate) with `lightdash login`
...
The miss labels printed by the CLI are the same ones documented in [Why a query misses a pre-aggregate](/semantic-layer/pre-aggregates/monitoring#why-a-query-misses-a-pre-aggregate) — that table maps each reason to the YAML or configuration change that fixes it.

A typical iteration loop:

1. `lightdash pre-aggregate-audit --dashboard <slug>` to see the current misses
2. Edit your dbt model YAML to address each miss (use the [miss-reason table](/semantic-layer/pre-aggregates/monitoring#why-a-query-misses-a-pre-aggregate) to know which field to change)
