...
| **Metric deduplicated across a join: select exactly the pre-aggregate dimensions** | The query includes a `count`, `sum` or `average` from the "one" side of a one-to-many join, and the pre-aggregate groups by dimensions from the "many" side. The stored value is deduplicated by the parent's `primary_key`, so it is right at the pre-aggregate's own grain but can't be summed across the many-side dimensions. | Select every pre-aggregate dimension, or define a pre-aggregate at the grain you query. Metrics on the many side, and `min` / `max`, re-aggregate normally. |
