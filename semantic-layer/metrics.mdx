...
Model-level metrics don't inherit settings from the columns they reference. That includes access control: a model-level metric that reads a column with `required_attributes` is not restricted with it. See [metrics and column access control](/workspace-admin/user-attributes#metrics-and-column-access-control).
