...
| AI coding agent *(Beta)* | Agent-proposed changes to any connected repository, not just the dbt project | Developer, analytics engineer | Feature flag, org's Git App install + user access intersection, denied-path commit gate | [AI coding agent](/agents/ai-coding-agent) |
