...
<Info>
  <Badge icon="building-plus" color="blue" size="sm" shape="pill">Enterprise</Badge> Direct access is available with a valid Enterprise Edition (EE) license. No feature flag or organization-level enablement is required. Recipients must be active members of the organization with access to the project.
</Info>
...
For dashboards, **Can edit** includes adding and removing tiles, but does not grant permission to delete the dashboard itself. **Full access** also grants dashboard deletion, subject to existing project permissions and content protections. Neither role grants separate permissions to restore or permanently delete content from **Recently deleted**.

This distinction applies to direct dashboard assignments. Existing space-editor permissions are unchanged: a user with direct **Can edit** can still delete a dashboard if another applicable role grants deletion. Existing direct editors without another qualifying permission need **Full access** to continue deleting dashboards.
...
The project-scoped v2 API manages the same direct assignments as the Share dialog. It requires a valid EE license and permission to manage direct access on the target item, including when listing assignments.
...
Without a valid EE license, direct-access sharing controls and **Shared with me** are hidden, and the direct-access API rejects requests.

If someone cannot open shared content, check that the EE license is valid, their account is active, and they still have project access. For a group assignment, check both their group membership and the group's project access. For a failed tile or linked query, check the independent dependency's access too.
