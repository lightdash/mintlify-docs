...
## Declare a column's stored timezone

When the data timezone you set in [Configure your connection](#configure-your-connection) doesn't cover every naive column, for example a `timestamp` already converted to local time sitting next to columns in UTC, declare that column's own zone with `wall_clock_timezone`:
...
See [Wall clock timezone](/semantic-layer/dimensions#wall-clock-timezone) in the dimensions reference for the full rules.
