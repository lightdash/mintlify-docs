...
description: "Customize what charts and dashboards show based on attributes of the logged-in user"
doc-type: reference
---
...
User attributes can be assigned to users or [groups](/workspace-admin/manage-groups) by navigating to **Organization Settings** \> **User Attributes** and clicking on the user attribute you'd like to assign. Select a user by email address, or a group by group name and set their value.

Each user, group, and default supports **multiple values**. When you reference the attribute in SQL or `required_attributes` / `any_attributes`, Lightdash treats all assigned values as a set — see [How user and group attribute values interact](#how-user-and-group-attribute-values-interact).

<Tip>
  Self-hosted Enterprise instances can declare user attributes and group-value mappings as instance config with the [`LD_SETUP_USER_ATTRIBUTES`](/self-host/customize-deployment/environment-variables#declare-user-attributes) environment variable, so they're reconciled on every deploy.
</Tip>
...
<Frame>
  ![The edit panel for a user attribute, with default values set alongside the per-user assignments](/images/workspace-admin/user-attributes/user-attribute-default-value-5618464d7ec3e8f71a78d0af4d409229.png)
</Frame>
...
When referencing user attributes in SQL you can use the following [SQL variables](/semantic-layer/sql-variables):
...
<Info>
  **The user email attribute is only available when the email is verified.**
  ...
  <Frame>
    ![A query error reading that the user attribute email is not available because the email is unverified](/images/user-attributes-info.png)
  </Frame>
  <CodeGroup>
...
If a referenced attribute is not set for the current user, the query returns a `Forbidden` error.

<Frame>
![Dashboard filter using a user attribute as its value](/images/workspace-admin/user-attributes/user-attribute-dashboard-filter.png)
</Frame>
...
To reference a user attribute in your sql, use the special lightdash reference`${lightdash.attributes.<attribute_name> }`. You should use the `IN` operator since the attribute might have multiple values.
...
<Note>
  Users without access to this dimension will not see it, or the metrics defined under that column. Model-level metrics that reference the column are not hidden — see [metrics and column access control](#metrics-and-column-access-control).
</Note>
...
If a user without access to this dimension runs a query that contains this dimension, they will get a `Forbidden` error. Metrics defined under this column's `metrics` are hidden and blocked the same way.
...
### Metrics and column access control

Metrics do not accept `required_attributes` or `any_attributes`. A metric's access control comes from where it's defined:

- **Column-level metrics**, defined under a column's `metrics`, inherit the `required_attributes` and `any_attributes` of that column's dimension.
- **Model-level metrics**, defined under the model's `metrics`, inherit nothing from the columns they reference. A model-level metric whose `sql` reads a restricted column stays visible and queryable.

To restrict a model-level metric, either:

- set `required_attributes` or `any_attributes` on the [table](#table-filtering-with-required_attributes), or
- define the metric under one of the restricted columns instead. If the metric reads several columns, give them all the same requirement.
...
models:
  - name: orders
    config:
      meta:
        metrics:
          revenue_per_order: # model-level: not restricted by the revenue column
            type: number
          ...
          meta:
            dimension:
              required_attributes:
                is_finance: "true"
            metrics:
              total_revenue: # column-level: restricted with the revenue column
                type: sum
```
...
metrics:
  revenue_per_order: # model-level: not restricted by the revenue dimension
    type: number
    sql: ${total_revenue} / ${order_count}
...
dimensions:
  - name: revenue
    ...
    required_attributes:
      is_finance: "true"
    metrics:
      total_revenue: # dimension-level: restricted with the revenue dimension
        type: sum
```
</CodeGroup>
