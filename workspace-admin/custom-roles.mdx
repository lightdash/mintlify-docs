---
title: "Custom Roles"
description: "Build granular permission sets by picking exactly which scopes a role grants"
doc-type: reference
---
...
<Frame>
![scope-dependencies.png](/images/workspace-admin/custom-roles/scope-dependencies.png)
</Frame>
...
<Frame>
![duplicate-role.png](/images/workspace-admin/custom-roles/duplicate-role.png)
</Frame>
...
To use the **Embedding** scopes with dashboard or AI agent tokens, see [Embed permission modes](/embed/reference#permission-modes) for configuration, scope names, and how role grants interact with JWT flags.
...
### Verified-content scopes

Two scopes control [verified content](/explore/verified-content), and they do different jobs:

**Manage content verification** (`manage:ContentVerification`) lets a user verify or unverify charts and dashboards — toggling the badge itself, in the UI or via `lightdash upload`. Admins and developers hold this by default.

**Edit or delete verified charts and dashboards** (`manage:VerifiedContent`) lets a user edit or delete content that is *currently verified*. Without it, verified content is read-only even for users with general edit rights — the original verifier is the only exception. Admins and developers hold this by default.

The two are deliberately separate: a role can be allowed to edit verified content without being able to keep or apply its badge (saving then drops verification), and a role can be allowed to verify content it can't otherwise edit. Granting neither gives editors a create → review → verify → lock workflow, where shared sources of truth can't be changed by the general editor population.
