...
<Info>
  <Badge icon="cloud-bolt" color="blue" size="sm" shape="pill">Cloud</Badge> Cloud Pro and above. Verified domains are part of per-organization SSO settings — contact the Lightdash team if you don't see the **Verified domains** panel. [What each availability level means](/support/feature-availability).
</Info>
...
<Note>
  Verified domains control **SSO routing only**. They are separate from [allowed email domains](/get-started/quickstart/invite-new-users), which control which users can auto-join your organization.
</Note>
...
- [SSO providers](/workspace-admin/sso)
- [Self-hosted SSO configuration](/self-host/customize-deployment/use-sso-login-for-self-hosted-lightdash)
- [Allowing users to auto-join your organization](/get-started/quickstart/invite-new-users)
