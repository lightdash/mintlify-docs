...
<Info>
  <Badge icon="building-plus" color="blue" size="sm" shape="pill">Enterprise</Badge> Self-hosted instances also need `SCIM_ENABLED=true`, see [enterprise features](/self-host/enterprise-features#scim). [What each availability level means](/support/feature-availability). [View pricing](https://www.lightdash.com/pricing).
</Info>

## Summary

SCIM (System for Cross-domain Identity Management) is an open standard protocol for automating user provisioning. Lightdash supports any identity provider (IdP) that implements the SCIM 2.0 standard, including Azure, Okta, OneLogin, and others.
...
If you are self-hosting Lightdash, see [SCIM self-hosting](/self-host/enterprise-features#scim) for the environment variables to set, including the required `AUTH_ENABLE_OIDC_TO_EMAIL_LINKING` companion when combining SCIM with SSO.
...
<Frame>
  ![The account menu opened from the user initials in the top-right corner, with User Settings highlighted](/images/workspace-admin/sso/scim/scim-lightdash-user-settings-9f554881763a721af6463096fe8e1d59.png)
</Frame>

2. In the sidebar, select **SCIM Access Tokens**.

<Frame>
  ![The SCIM Access Tokens page in user settings, with an empty token list and a Generate new token button](/images/workspace-admin/sso/scim/scim-lightdash-tokens-list-b060b94f0b68fc67274186809043c175.png)
</Frame>
...
<Frame>
  ![The generate token dialog with fields for a token name and an optional expiration date](/images/workspace-admin/sso/scim/scim-lightdash-generate-token-fd03c005c465260741db9f144782d7e4.png)
</Frame>

4. Once generated, copy and save it in a safe place, as it cannot be viewed again once the modal is closed.

<Frame>
  ![The generated SCIM token shown once in a modal, with a button to copy it](/images/workspace-admin/sso/scim/scim-lightdash-generated-token-851f5ec6141dbd3dcb35910687f579fa.png)
</Frame>
...
<Frame>
  ![The SCIM URL displayed at the top of the SCIM Access Tokens page above the list of tokens](/images/workspace-admin/sso/scim/scim-lightdash-view-url-3cfc6eb4588ba52111f287b6a5f08603.png)
</Frame>
...
<AccordionGroup>
  <Accordion title="Okta">
    ...
    <Frame>
      ![The Okta app catalog with SCIM 2.0 Test App (Header Auth) selected and its Add Integration button](/images/workspace-admin/sso/scim/scim-okta-create-integration-b666384a8b1a307b610ccdef67667dcd.png)
    </Frame>
    5. Give it a friendly name and click **Next**.

    <Frame>
      ![The Okta general settings step for the new integration, with the application label field filled in](/images/workspace-admin/sso/scim/scim-okta-name-app-0c844947069855ee82176c7893a4291c.png)
    </Frame>
    ...
    <Frame>
      ![The Provisioning tab of the Okta application before any API integration is configured](/images/workspace-admin/sso/scim/scim-okta-provisioning-3bacb4dce7b8a7229d5a93d71b89f016.png)
    </Frame>
    3. Select **Configure API Integration**.

    <Frame>
      ![The Okta Configure API Integration screen with fields for the SCIM base URL and API token](/images/workspace-admin/sso/scim/scim-okta-configure-api-integration-78a059f5ef6167aa33206c2c8ec2b2d0.png)
    </Frame>
    ...
    <Frame>
      ![The Okta Assignments tab with a user assigned to the application](/images/workspace-admin/sso/scim/scim-okta-assign-users-4cbb63bebcfc37077bbbdca39552e001.png)
    </Frame>
    <Info>
    ...
    **For Organization Role:**
    ...
    <Frame>
      ![The Okta profile editor add-attribute form filled in for the org_role attribute](/images/workspace-admin/sso/scim/scim-okta-org-role-attribute.png)
    </Frame>
    ...
    **For Project Roles:**
    ...
    <Frame>
      ![The Okta profile editor add-attribute form filled in for the project_role attribute](/images/workspace-admin/sso/scim/scim-okta-project-role-attribute.png)
    </Frame>
    ...
    **For Project Role Mapping:**
    ...
    <Frame>
      ![The Okta User to Lightdash mappings screen with a group-membership expression assigning role values](/images/workspace-admin/sso/scim/scim-okta-role-mapping.png)
    </Frame>
  </Accordion>
  <Accordion title="Azure">
    ...
    <Warning>
      The `email` attribute sent from Microsoft / Azure AD / Entra ID via SCIM is **not trusted**, since users can freely change it. Before configuring provisioning, review [Can the IdP-provided SCIM email be trusted?](#can-the-idp-provided-scim-email-be-trusted) and configure your IdP to use the immutable UPN (`userPrincipalName`) as the email claim.
    </Warning>
    ...
    <Frame>
      ![The Entra ID create-your-own-application panel with a name entered and the non-gallery option selected](/images/workspace-admin/sso/scim/scim-azure-create-app-3bb17d51662b8667f958aefbd2fea4ef.png)
    </Frame>
    ...
    <Frame>
      ![The Users and groups page of the Entra ID application, listing the assigned users and groups](/images/workspace-admin/sso/scim/scim-azure-edit-app-2b718ab6803faea2bae1f5ed40877256.png)
    </Frame>
    ...
    <Frame>
      ![The Entra ID provisioning setup with the Tenant URL and secret token fields and a Test connection button](/images/workspace-admin/sso/scim/scim-azure-configure-provisioning-0913ad7db2b504ecbee342922e84da98.png)
    </Frame>
  ...
  <Accordion title="Google Workspace">
    ...
    See the [SCIM API reference](/api-reference/scim/list-users) for all endpoints and payloads.
  </Accordion>
</AccordionGroup>

## API docs

You can find the full API docs and examples for SCIM [here](/api-reference/scim/list-users).
...
Lightdash defines two role scopes: Organization and Project. Both scopes support system roles and [custom roles](/workspace-admin/custom-roles).
...
<Warning>
  This section describes a legacy Lightdash-specific SCIM extension and is deprecated. New integrations should use the standard SCIM `roles` attribute [RFC 7643](https://datatracker.ietf.org/doc/html/rfc7643#section-7). The extension will be removed in a future release.
</Warning>
...
2. Manage group project roles in Lightdash and provision group memberships via SCIM
...
Use the following curl command to rotate a SCIM access token. Replace `<scim-access-token-uuid>` with the UUID of the token you wish to rotate, and `<personal-access-token>` with your [personal access token](/personal-settings/create-personal-tokens). This process will invalidate the previous token and return a new token with the specified expiration date.
