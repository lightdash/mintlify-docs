---
title: "SSO setup"
description: Configure SSO providers from admin settings, with domain routing and account linking
doc-type: reference
---
...
<Info>
  Self-serve SSO configuration is available on paid Lightdash plans and must be enabled per organization. If you don't see **Single Sign-On** in your Organization settings and need to configure a provider like Okta or Azure AD, contact [Lightdash support](mailto:support@lightdash.com) to enable it. Which providers you can configure depends on your plan — see the [SSO providers matrix](/workspace-admin/sso#sso-providers-by-plan). Self-hosted instances can configure SSO from this panel **or** with environment variables — see [Self-hosted alternative](#self-hosted-alternative-environment-variables).
</Info>

## Prerequisites

Before a provider can route users, your organization needs at least one [verified domain](/workspace-admin/sso/verified-domains). Verify your domains first.

## Provider options

Beyond its credentials, each configured provider has a few settings:

- **Enabled toggle** — turn a configured provider on or off without deleting it. A disabled provider is ignored by routing.
- **Password sign-in** — each provider has an *Allow password sign-in* option. The password field is hidden only when **every** provider matching a user's domain disallows it; if any matching provider allows it, the field stays visible. Disallow it everywhere to enforce SSO-only sign-in.
...
A provider serves users by email domain. By default it routes all of your organization's verified domains; to restrict it to a subset, see [Routing SSO methods to verified domains](/workspace-admin/sso/verified-domains#routing-sso-methods-to-verified-domains).
...
<Note>
  When you save an Okta or Generic OIDC configuration, Lightdash validates that the provider URL it must fetch (the Okta domain / metadata document URL) resolves to a public `https://` address. See [URL requirements for organization-managed SSO](/workspace-admin/sso#url-requirements-for-organization-managed-sso).
</Note>
...
| Field | Required? |
| :--- | :---: |
| Issuer URI | <Icon icon="check"/> |
| Okta domain | <Icon icon="check"/> |
| Client ID | <Icon icon="check"/> |
| Client secret | <Icon icon="check"/> |
...
| Field | Required? |
| :--- | :---: |
| Application (client) ID | <Icon icon="check"/> |
| Directory (tenant) ID | <Icon icon="check"/> |
| Client secret | <Icon icon="check"/> |
...
| Client ID | <Icon icon="check"/> |
| Client secret | <Icon icon="check"/> |
| Metadata document URL | <Icon icon="check"/> |
| Scopes | |
...
- **Link SSO logins to existing accounts by email** — when a user signs in via OIDC with an email that matches an existing Lightdash user, attach the SSO identity to that user instead of failing the login. Required when using [SCIM](/workspace-admin/sso/scim) with SSO so SCIM-provisioned users can sign in.
...
- [SSO providers overview](/workspace-admin/sso)
- [Configure SSO for self-hosted Lightdash](/self-host/customize-deployment/use-sso-login-for-self-hosted-lightdash)
- [SCIM integration](/workspace-admin/sso/scim)
- [Groups and access control](/workspace-admin/manage-groups)
