...
- **Link SSO logins to existing accounts by email** — when a user signs in via OIDC with an email that matches an existing Lightdash user, attach the SSO identity to that user instead of failing the login. Required when using [SCIM](/workspace-admin/scim-integration) with SSO so SCIM-provisioned users can sign in.
...
- [SCIM integration](/workspace-admin/scim-integration)
