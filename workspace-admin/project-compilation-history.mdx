...
description: "Review past project compilations to monitor deployments and troubleshoot failures"
doc-type: reference
---
...
<Frame>
  ![The compilation history entry in the project settings menu](/images/workspace-admin/project-compilation-history/compilation-history-settings.png)
</Frame>
...
<Frame>
  ![An expanded compilation log entry showing the user who triggered it and counts of metrics, dimensions, and explores](/images/workspace-admin/project-compilation-history/compilation-history-log-details.png)
</Frame>
...
<Frame>
  ![A dbt refresh job notification with the compilation log shown as the job finalizes](/images/workspace-admin/project-compilation-history/compilation-history-job-status.png)
</Frame>
...
Access to compilation logs is gated on two scopes for the target project:

- `update:Project`
- `view:JobStatus`

Both scopes are granted to Project Developer and Project Admin (as well as Organization Admin) by default. Project Viewer, Project Interactive Viewer, and Project Editor do not have them.

If you are shaping custom roles, grant both `update:Project` and `view:JobStatus` on the project to any role that needs to read compilation logs. See [roles and permissions](/workspace-admin/roles) for the full permission matrix.
