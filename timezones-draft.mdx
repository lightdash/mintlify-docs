...
See [Choosing a chart's timezone](#choosing-a-charts-timezone) for how to set this.
...
Read it. If it's not the zone you expect, change it with the [timezone picker](#choosing-a-charts-timezone).
...
To keep these filters consistent, a project admin can turn on **Project time zone in filter inputs** in Project settings → Timezone. Set a project timezone first; the toggle is disabled until one is set. When it's on:
