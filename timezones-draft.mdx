...
If a column represents a calendar date, such as a birthday, a fiscal period start, or an `effective_date` on a contract, store it as `DATE`. Lightdash treats `DATE` columns as fixed calendar dates and never shifts them. No matter the project timezone, `2024-03-15` stays `2024-03-15`.
...
Every chart in Explore and on dashboards shows a small badge with its resolved timezone, for example `(UTC +01:00) Europe/London`. This is the zone the chart settled on after the rules in [Which timezone wins](#which-timezone-wins), and it tells you exactly what zone the chart's filters, buckets, and rendered values are using.
...
The default is **Project timezone**. It is not frozen at save time: it always resolves to the current project timezone, so the chart stays consistent for its whole audience and tracks any later change to the project setting. Pick **User timezone** only when you deliberately want each viewer's own day boundaries, and **A specific timezone** when the zone must never change.
...
Two viewers in different timezones may see different numbers on a daily chart if it is set to User timezone. This is correct (each viewer sees their own calendar) but can be surprising. See [Choosing a chart timezone](#choosing-a-chart-timezone) for how to control it.
...
The hourly charts below count one event per hour in America/New_York. On the fall-back day (3 November), every hour has one event except 1 AM, which holds two: the 1 AM that ran on summer time and the 1 AM that ran again on winter time fall into the same bar.
...
Embedded dashboards have no signed-in viewer, so there is no profile timezone to fall back to.

A direct iframe or shareable-URL embed can set a per-session timezone with the `timezone` URL parameter. It applies to the whole embedded session and overrides each chart's own setting. See [embedding URL parameters](/references/iframe-embedding#timezone). The React SDK does not support this parameter.

Without it, each chart resolves to its own setting: the project timezone, or a specific timezone if one was chosen. A chart set to User timezone falls back to the project timezone, because an embed has no viewer profile.
