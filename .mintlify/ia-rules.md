...
12. **Every directory node lands on a root page.** A node is any group with children, at any depth — `agents` and `semantic-layer/pre-aggregates` are both roots, and a root can sit inside another root. Each sets `root` (the bare slug) and `directory: "card"` in `docs.json`, which renders its children automatically. The landing page is a short orientation plus that auto-rendered directory — **never a hand-built `<CardGroup>`**, which duplicates the listing and drifts the moment a child is added. Name the root page's `sidebarTitle` for what the page covers, or omit it; "Overview" is not a sidebar item anywhere. Sizing signals cut both ways: a node that can't sustain a meaningful landing page is too thin — merge it up; a node over ~12 children triggers a sectional review.
