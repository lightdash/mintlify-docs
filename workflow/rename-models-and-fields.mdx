...
description: "Rename a model, metric, or dimension and update every reference across your project"
doc-type: tutorial
---

<Info>
  Renaming is restricted to admin users, because it updates content project-wide, including charts and dashboards that other users may not have access to modify.
</Info>
...
Content can also be renamed using [Dashboards as code](/workflow/content-as-code), but the rename methods in this doc require fewer steps and automatically respect the right syntax for different field types. Neither rename workflow updates chart names and descriptions — to change those in bulk, use Dashboards as code.

## Rename in Lightdash

If you change field or table names in dbt that are being used in Lightdash, you will receive validation errors. You can fix these directly on the validation page by renaming references in the broken charts. You can easily access the validation page by clicking on the bell on the navigation bar. [Read the validation docs here](/workflow/validating-your-content).

<Frame>
<img
  src="/images/workflow/rename-models-and-fields/rename-validation-errors.png"
...
<Frame>
![rename fix button](/images/workflow/rename-models-and-fields/rename_fix_button.png)
</Frame>
...
<Frame>
<img
  src="/images/workflow/rename-models-and-fields/rename_modal.png"
  alt="rename modal"
  className="mx-auto"
  style={{ width:"50%" }}
/>
...
8. If you have [charts/dashboards as code](/workflow/content-as-code) in your dbt project, run `lightdash download` to update them with the renamed fields.
...
<Frame>
![rename CLI list](/images/workflow/rename-models-and-fields/rename_cli_list.png)
</Frame>

[Read the CLI reference doc](/workflow/cli/reference#lightdash-rename) for all arguments and more examples.

## Reverting changes

When renaming, a new chart version is created with the new changes. If you make a mistake during renaming, you can restore individual charts by using [version history](/explore/version-history#chart-version-history).