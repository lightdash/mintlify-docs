...
description: Find the content in your project that is broken, and the details of each error
doc-type: tutorial
---
...
<Frame>
  <img src="/images/workflow/cli/cli-lightdash-help-86d9d11f57ca516a722c0dfcf8f2919a.png" alt="The lightdash CLI help output listing global options, every command from login to generate-exposures, and usage examples" />
</Frame>
...
By default, `lightdash validate` reports errors only. Chart configuration warnings are hidden, the output notes how many were hidden, and they never affect the exit code.

Add `--severity warning` to show chart configuration warnings and treat them as errors:

```bash
lightdash validate --severity warning
```

With `--severity warning`, the command exits with return code 1 if any warnings or errors exist. Use this in CI to block merges on warnings instead of letting them go unnoticed.
