...
<Note>
**Recommended: Install Lightdash Skills first.** Before manually configuring your AI tool, run `lightdash install-skills` to install the [Lightdash Skills](/get-started/build-your-semantic-layer/install-skills). The skills already know to use `lightdash lint` for validation, so you don't need to configure anything manually.
</Note>
