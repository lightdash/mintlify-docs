...
<Note>
**Recommended: Install Lightdash Skills first.** Before manually configuring your AI tool, run `lightdash install-skills` to install the [Lightdash Skills](/workflow/install-agent-skills). The skills already know to use `lightdash lint` for validation, so you don't need to configure anything manually.
</Note>
  ...
  lint:
    ...
    steps:
      ...
      - name: Install Lightdash CLI
        run: brew tap lightdash/lightdash && brew install lightdash
...
- [Lightdash YAML](/semantic-layer/yaml) - Define your semantic layer in YAML without dbt
- [Dashboards as code](/workflow/content-as-code) - Manage charts and dashboards as YAML files
- [Lightdash CLI reference](/workflow/cli/reference#lightdash-lint) - Full command reference
