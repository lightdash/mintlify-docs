...
<Warning>
  ...
  **Use [`lightdash preview`](#lightdash-preview) for local development and testing.** Preview creates a temporary, isolated project that won't affect production.
  ...
  For production deploys, we recommend using [CI/CD with GitHub Actions](/workflow/cli/deploy#automatically-deploy-your-changes-to-lightdash-using-a-github-action).
</Warning>
...
- `--virtual-views <slugs...>`
  - opt-in flag to download specific [virtual views](/workflow/content-as-code#virtual-views) as code. Pass one or more virtual-view slugs. Each view is written to `lightdash/virtual-views/<slug>.yml`. Virtual views are not included in a bare `lightdash download` — use this flag or `--include-virtual-views` to opt in.
- `--include-virtual-views`
  - (default: false)
  - opt-in flag to download every [virtual view](/workflow/content-as-code#virtual-views) in the project as code. Use `--virtual-views <slugs...>` instead when you only want specific views.
- `--apps <appUuids...>`
...
- `--force`
  - required to create new content as code that doesn't exist in your Lightdash project yet — without it, `lightdash upload` only updates existing content. Also required to apply destructive [virtual view](/workflow/content-as-code#virtual-views) column changes (removing a column or changing its type).
- `--charts` or `-c`
...
- `--virtual-views <slugs...>`
  - specify [virtual view](/workflow/content-as-code#virtual-views) slugs to upload from `lightdash/virtual-views/`. Combine with `--force` to apply destructive column changes.
- `--skip-virtual-views`
  - (default: false)
