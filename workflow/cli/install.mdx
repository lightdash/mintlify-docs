...
description: Install the Lightdash CLI with Homebrew, NPM, WSL, or a direct binary download
doc-type: tutorial
---
...
brew trust --formula lightdash/lightdash/lightdash
brew install lightdash
```

<Note>
  Newer Homebrew versions may block formulas from third-party taps unless you trust them first. If the install fails, run the `brew trust --formula lightdash/lightdash/lightdash` line above and try again.
</Note>
...
<Info>
  Prebuilt binaries are macOS only (Apple Silicon and Intel). If you're on Mac, [Homebrew](#install-via-homebrew) is the easier no-Node option — it wraps the same binary and handles `PATH` and updates for you. Linux users should use [npm](#install-via-npm); Windows users should use [WSL](#install-on-windows-wsl-recommended).
</Info>
