...
description: "Temporary copies of a project where you can experiment without touching production"
---
...
- make changes to important content in production without any downtime by [promoting the content](/explore/promote-content) you've changed in your preview project when it's ready.
...
You can use the [Lightdash CLI tool](/workflow/cli/install) to generate preview projects in Lightdash from your command line.
...
You can see more information about the [lightdash preview CLI command here](/workflow/cli/reference#lightdash-preview).
...
<Frame>
  ![Create Preview Jp](/images/workflow/preview-projects/create-preview.jpg)
</Frame>
...
You can leave out the **branch** and **schema** fields and you'll get a preview project with a copy of all your content. You can create or update charts and dashboards, then [promote the content](/explore/promote-content) back to your main project when it's ready.
...
For a [native Lightdash YAML project](/semantic-layer/yaml#connect-through-github), the preview keeps the native format and compiles the selected branch without dbt. Its `sql_from` definitions still determine which physical tables are queried; selecting another branch does not create warehouse tables.
...
Learn more about setting up a Lightdash preview project for pull requests using GitHub Actions [here](/workflow/set-up-ci-cd#add-previews-to-pull-requests).
...
These flags work with both `lightdash preview` and `lightdash start-preview`, and `--ignore-errors` still lets preview creation continue after strict compilation errors:

```bash
lightdash start-preview --name "strict-preview" --no-partial-compilation --validate-warehouse-columns
```

### Open and work in your preview

After `lightdash preview` or `lightdash start-preview` spins up the project, `cmd` + `click` to open the preview link from your terminal. Once you're in Lightdash go to `Explore` --> `Tables`, then click on the model(s) you just updated to see your changes and play around with them.

While your preview is running, you can run `dbt compile` in a separate terminal session to update your dbt models. After compiling, refresh your browser to see the changes in Lightdash. This is not a hot reload — you need to refresh the page yourself.

**If `LIGHTDASH_PROJECT` is set in your shell** (for example from a `.env` file), it takes precedence over your active preview. Commands such as `lightdash deploy`, `lightdash upload`, and `lightdash download` target that project and warn that the preview was ignored. Pass `--project <preview uuid>` or unset the variable to work against the preview.
...
<Note>
This applies to ephemeral preview projects created from your local dbt profile. For a durable, persistent project connection to Redshift with IAM, use a role the Lightdash backend assumes (`assumeRoleArn`). See [Connect Redshift](/get-started/quickstart/connect-project#redshift).
</Note>
...
If you haven't changed anything from our default [GitHub action script](https://github.com/lightdash/cli-actions/blob/main/start-preview.yml), then the connection settings used for your preview project should be whatever you've specified in your [DBT_PROFILES secret](/workflow/set-up-ci-cd#use-profile-targets).

You have three options for setting up project connections in your GitHub Actions:

1. Set up a single project connection that's used in every pull request. E.g. always use the `prod` target. You can [follow the instructions here](/workflow/set-up-ci-cd#use-profile-targets) for your data warehouse
2. If you're using dbt Cloud, you can [use the dbt Cloud schema](/workflow/set-up-ci-cd#use-dbt-cloud-schema) to manage the connection used in the pull request.
3. [Create a project connection per developer in GitHub](/workflow/set-up-ci-cd#use-developer-credentials) (similar to `dev` profiles in your dbt project) so the connection changes depending on who opened the pull request. This is useful if you use different development schemas for team members developing in dbt.
...
Learn more about promoting content from preview projects [here](/explore/promote-content).
...
Preview projects created using any method can be deleted in the [Lightdash App](/workflow/preview-projects#lightdash-app-2).
...
For more details on the `lightdash preview` CLI command, check out the docs [here](/workflow/cli/reference#lightdash-preview).
...
<Frame>
  ![The All projects settings page listing preview projects, each row with a red delete button at the end](/images/workflow/preview-projects/delete-preview-4a4c1516a007ea4641ffd2683ae7d94e.jpg)
</Frame>
...
If you've added both the [start-preview and stop-preview workflows](/workflow/set-up-ci-cd#add-previews-to-pull-requests) in GitHub, then the preview environments you've created in your pull requests should automatically be deleted when the pull request gets merged.
