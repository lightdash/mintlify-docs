...
These flags work with both `lightdash preview` and `lightdash start-preview`. Other generic compilation warnings remain non-blocking, and `--ignore-errors` allows preview creation to continue after strict compilation errors. See the [CLI compilation options](/workflow/cli/reference#partial-compilation) for details.
