...
Selecting virtual views or data apps on their own, rather than through a dashboard, is opt-in on both commands — see [`lightdash download`](/workflow/cli/reference#lightdash-download) and [`lightdash upload`](/workflow/cli/reference#lightdash-upload) for those flags.
